...
本主題將處理與執行 [!INCLUDE[btsCoName](../../includes/btsconame-md.md)]® [!INCLUDE[BTARN_CurrentVersion_FirstRef](../../includes/btarn-currentversion-firstref-md.md)] 有關的問題。 每個問題的說明都會詳細描述其特殊徵狀、可能的原因以及解決方式。  
...
### <a name="solution"></a>方案  
 請檢閱錯誤訊息所提供的詳細資訊，可取得額外的協助。 重新啟動[!INCLUDE[btsCoName](../../includes/btsconame-md.md)] [!INCLUDE[btsSQLServerNoVersion](../../includes/btssqlservernoversion-md.md)]™ 可能可以解決這個問題。  
...
### <a name="possible-cause"></a>可能的原因  
 若您用 [!INCLUDE[btaBTARN3.3abbrevnonumber](../../includes/btabtarn3-3abbrevnonumber-md.md)]® 管理主控台 (MMC) 刪除協議與交易夥伴之前，先執行了 [!INCLUDE[btsCoName](../../includes/btsconame-md.md)]Clean 公用程式，則 BtarnClean 公用程式就無法取消登錄所有的成品，因為此時成品仍在使用中。  
...
### <a name="symptom"></a>徵兆  
 ...
 終止 BAM 訊息活動識別碼的錯誤*\<識別碼 >*。  
  
### <a name="possible-cause"></a>可能的原因  
 未安裝商務活動監控 (BAM) 追蹤工具。  
  
### <a name="solution"></a>方案  
 安裝 BAM 追蹤工具使用**自訂安裝**選項。 如果您不需要 BAM 功能，可以忽略這些訊息，或利用 [!INCLUDE[btaBTARN3.3abbrevnonumber](../../includes/btabtarn3-3abbrevnonumber-md.md)] 管理主控台停用追蹤功能。 停用追蹤功能之後，必須重新啟動 [!INCLUDE[btsBizTalkServerNoVersion](../../includes/btsbiztalkservernoversion-md.md)] 與 Internet Information Services (IIS)。  
...
2.  展開**\<電腦名稱 > （本機電腦）**，然後按一下 **網頁服務延伸**。  
  
3.  在**網頁服務延伸** 窗格中，確認是否允許 HwsMessages HttpReceive 的狀態。 如果沒有，請以滑鼠右鍵按一下**HwsMessages HttpReceive**，然後按一下 **允許**。  
...
2.  展開**\<電腦名稱 > （本機電腦）**，以滑鼠右鍵按一下**網頁服務延伸**，然後按一下 **新增新的 Web 服務延伸**。  
  
3.  在**新增網頁服務延伸**對話方塊中，於**延伸模組名稱**方塊中，輸入**BizTalk ISAPI 擴充程式**，然後按一下 **新增**。  
  
4.  在**Add File**對話方塊中，於**檔案路徑**方塊中，輸入**\<磁碟機 >: \Program Files\Microsoft BizTalk Server\<版本 > \HttpReceive\BTSHttpReceive.dll**，然後按一下 **確定**。  
...
### <a name="possible-cause"></a>可能的原因  
 若要執行 「 組態精靈 」，使用者應該設定為\<*機器名稱*> \hostsvc'，不是 '\hostsvc'。  
  
### <a name="solution"></a>方案  
 開啟 BizTalk 管理主控台中，並變更 '\hostsvc'，在帳戶下執行的主機，讓他們的帳戶下執行 '\<*機器名稱*> \hostsvc'。  
...
1.  按一下**啟動**，指向 **所有程式**，指向  **Microsoft SQL Server**，然後按一下  **Enterprise Manager**。  
  
2.  在 Enterprise Manager 中，展開**Microsoft SQL Server**， **SQL 伺服器群組**， **（本機） (Windows NT)**，和**Data Transformation Services**.  
...
6.  在命令提示字元中，輸入要部署的追蹤檔案中，下列程式碼，然後按一下**確定**。  
...
2.  註解的那一行 「 127.0.0.1 localhost 」 將"#"放在一行的開頭。 將變更後的 Hosts 檔案存檔。  
...
### <a name="symptom"></a>徵兆  
 之後[!INCLUDE[BTARN_CurrentVersion_abbrev](../../includes/btarn-currentversion-abbrev-md.md)]完成設定，當您嘗試存取[http://localhost](http://localhost/)或[http://localhost/BtarnApp/RnifSend.aspx](http://localhost/BtarnApp/RnifSend.aspx)，您可能會收到下列或類似的錯誤：  
...
2.  展開**\<電腦名稱 > （本機電腦）**，以滑鼠右鍵按一下**網站**，然後按一下 **屬性**。  
...
6.  按一下**啟動**，按一下 **執行**，型別**cmd**，然後按一下**確定**。  
...
> [!NOTE]
>  執行以上步驟之後，再連上 http://localhost 或 http://localhost/BtarnApp/RnifSend.aspx，此時 Internet Explorer 會收到 HTTP 400 訊息，這就代表 IIS 已經正常運作。  
...
### <a name="symptom"></a>徵兆  
 當您執行 setup.bat *\<磁碟機 >*: \Program Files\\ [!INCLUDE[btsCoName](../../includes/btsconame-md.md)] BizTalk\<版本 > Accelerator for RosettaNet\SDK\HubScenario 設定 HubScenario 範例中，作業會失敗。  
...
## <a name="run-setupx64bat-to-set-up-the-double-action-pipautomation-orchestration-sample-on-sql-server-2008-r22008-sp1"></a>執行 setupx64.bat，以設定 SQL Server 2008 R2/2008 SP1 上雙向動作 PIPAutomation 協調流程範例  
  
### <a name="symptom"></a>徵兆  
 執行 setup.bat 以建置和初始化設定「雙向動作 PIPAutomation 協調流程」範例時，BTARNData 資料庫中的 PipAutomationGetAction 預存程序未建立。  
  
### <a name="possible-cause"></a>可能的原因  
 您是在 64 位元的電腦或在執行 SQL Server 2008 R2/2008 SP1 的 [!INCLUDE[btsBizTalkServer2006r3](../../includes/btsbiztalkserver2006r3-md.md)] 安裝上執行 setup.bat。 這兩個執行個體都需要您執行 setupx64.bat。  
  
### <a name="solution"></a>方案  
 請執行 setupx64.bat 以建立預存程序。 如需詳細資訊，請參閱[雙向動作 PIPAutomation 協調流程](../../adapters-and-accelerators/accelerator-rosettanet/double-action-pipautomation-orchestration.md)。  
