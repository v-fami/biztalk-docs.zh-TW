...
## <a name="location-in-sdk"></a>SDK 中的位置  
 \<*磁碟機*> \Program Files\Microsoft BizTalk\<版本 > Accelerator for RosettaNet\SDK\SchemaValidator  
...
2.  移至\<*磁碟機*> \Program Files\Microsoft BizTalk\<版本 > Accelerator for RosettaNet\SDK\SchemaValidator。  
...
6.  移至\<*磁碟機*> \Program Files\Microsoft BizTalk\<版本 > Accelerator for RosettaNet\SDK\SchemaValidator，選取**SchemaValidator.sln**，然後按一下按一下**開啟**。  
...
13. 修改訊息執行個體至您想要藉由移除測試\< \!DOCTYPE … > 標記指定 DTD 檔案，從 XML 執行個體的標頭。  
  
14. 在訊息執行個體的根節點中，加入將要進行驗證之結構描述的 XML 命名空間。  
  
    > [!NOTE]
    >  如需可以由 SchemaValidator 公用程式進行驗證的結構描述的範例，請參閱中的 Sample3A4.xml \<*磁碟機*> \Program Files\Microsoft BizTalk\<版本 > Accelerator for RosettaNet\SDK\SchemaValidator。  
