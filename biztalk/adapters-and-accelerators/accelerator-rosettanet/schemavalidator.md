...
## <a name="location-in-sdk"></a>SDK 中的位置  
 \<*磁碟機*\>\Program 檔案 (x86) \Microsoft BizTalk\<版本\>Accelerator for RosettaNet\SDK\SchemaValidator  
...
2. 移至\<*磁碟機*\>\Program 檔案 (x86) \Microsoft BizTalk\<版本\>Accelerator for RosettaNet\SDK\SchemaValidator。  
  
3. 在命令提示字元中，輸入**sn-k SchemaValidator.snk**，然後按 ENTER 鍵。  
  
4. 開始**Microsoft Visual Studio 2012**。  
  
5. 在上**檔案**功能表上，指向**開啟**，然後按一下**開啟的方案**。  
  
6. 移至\<*磁碟機*\>\Program 檔案 (x86) \Microsoft BizTalk\<版本\>Accelerator for RosettaNet\SDK\SchemaValidator，選取**SchemaValidator.sln**，然後按一下**開啟**。  
  
7. 在 [方案總管] 中，以滑鼠右鍵按一下**SchemaValidator**，然後按一下**屬性**。  
  
8. 在  **MessageInspector 屬性**頁面上，按一下**簽署**索引標籤，然後再按**簽署組件**核取方塊。 選取  **SchemaValidator.snk**中**選擇強式名稱金鑰檔**。  
...
13. 修改訊息執行個體到您想要藉由移除測試\< \!DOCTYPE...\>指定從 XML 執行個體之標頭的 DTD 檔案的標記。  
  
14. 在訊息執行個體的根節點中，加入將要進行驗證之結構描述的 XML 命名空間。  
  
    > [!NOTE]
    >  如需可以由 SchemaValidator 公用程式進行驗證的結構描述的範例，請參閱中的 Sample3A4.xml \<*磁碟機*\>\Program 檔案 (x86) \Microsoft BizTalk\<版本\>Accelerator for RosettaNet\SDK\SchemaValidator。  
  
15. 在  [!INCLUDE[btsVStudioNoVersion](../../includes/btsvstudionoversion-md.md)]，按一下**SchemaValidator.cs**，然後按下 ctrl 鍵並按 F5 執行公用程式。  
