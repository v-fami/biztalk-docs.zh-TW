...
---
# <a name="stopping-and-starting-orchestrations-send-ports-and-receive-locations-programmatically"></a><span data-ttu-id="664a3-102">停止和啟動協調流程、 傳送埠和接收位置，以程式設計的方式</span><span class="sxs-lookup"><span data-stu-id="664a3-102">Stopping and Starting Orchestrations, Send Ports, and Receive Locations Programmatically</span></span>
<span data-ttu-id="664a3-103">本主題提供以程式控制方式停止及啟動協調流程、傳送埠和接收位置的範例程式碼。</span><span class="sxs-lookup"><span data-stu-id="664a3-103">This topic provides sample code for programmatically stopping and starting orchestrations, send ports, and receive locations.</span></span> <span data-ttu-id="664a3-104">您可以將所有的協調流程、傳送埠和接收位置當做群組或個別地執行這些動作。</span><span class="sxs-lookup"><span data-stu-id="664a3-104">You can perform these actions on all orchestrations, send ports, and receive locations as a group or individually.</span></span> <span data-ttu-id="664a3-105">只要將這個程式碼包含在程式中，即可動態地執行這些動作。</span><span class="sxs-lookup"><span data-stu-id="664a3-105">You can include this code in a program to perform these actions dynamically.</span></span> <span data-ttu-id="664a3-106">在圖形化使用者介面中執行這些動作在設計階段於 Microsoft [!INCLUDE[BTARN_CurrentVersion_FirstRef](../../includes/btarn-currentversion-firstref-md.md)]，或在 BizTalk 管理主控台中的執行階段。</span><span class="sxs-lookup"><span data-stu-id="664a3-106">You perform these actions in the graphical user interface at design time in Microsoft [!INCLUDE[BTARN_CurrentVersion_FirstRef](../../includes/btarn-currentversion-firstref-md.md)], or at run time in the BizTalk Administration console.</span></span>  
  
> [!NOTE]
>  <span data-ttu-id="664a3-107">對於用來啟動和停止協調流程的程式碼，您不必指定協調流程、傳送埠或接收位置。</span><span class="sxs-lookup"><span data-stu-id="664a3-107">For the code to start and stop orchestrations, you do not have to designate the orchestrations, send ports, or receive locations.</span></span> <span data-ttu-id="664a3-108">範例程式碼上所有的協調流程執行動作、 傳送埠和接收位置，[!INCLUDE[BTARN_CurrentVersion_abbrev](../../includes/btarn-currentversion-abbrev-md.md)]安裝的。</span><span class="sxs-lookup"><span data-stu-id="664a3-108">The sample code performs the action on all orchestrations, send ports, and receive locations that [!INCLUDE[BTARN_CurrentVersion_abbrev](../../includes/btarn-currentversion-abbrev-md.md)] installed at set up.</span></span> <span data-ttu-id="664a3-109">對於在單一協調流程、傳送埠或接收位置執行動作的程式碼，則請新增指示程式碼執行所在之協調流程、傳送埠或接收位置的參數。</span><span class="sxs-lookup"><span data-stu-id="664a3-109">For the code that acts on a single orchestration, send port, or receive location, add a parameter indicating on which orchestration, send port, or receive location you want the code to run.</span></span>  
  
## <a name="demonstrates"></a><span data-ttu-id="664a3-110">示範</span><span class="sxs-lookup"><span data-stu-id="664a3-110">Demonstrates</span></span>  
 <span data-ttu-id="664a3-111">本主題中的範例程式碼包含不同的程式碼區段，可以執行下列動作：</span><span class="sxs-lookup"><span data-stu-id="664a3-111">The sample code in this topic includes separate code sections to do the following:</span></span>  
  
-   <span data-ttu-id="664a3-112">啟動協調流程：啟動所有傳送埠和接收位置，並登錄及啟動所有協調流程</span><span class="sxs-lookup"><span data-stu-id="664a3-112">Start orchestrations—start all the send ports and receive locations, and enlist and start all orchestrations</span></span>  
  
-   <span data-ttu-id="664a3-113">停止協調流程：取消登錄所有協調流程和傳送埠，並停用所有接收位置</span><span class="sxs-lookup"><span data-stu-id="664a3-113">Stop orchestrations—unenlist all orchestrations, unenlist all send ports, and disable all receive locations</span></span>  
  
-   <span data-ttu-id="664a3-114">啟動單一傳送埠</span><span class="sxs-lookup"><span data-stu-id="664a3-114">Start a single send port</span></span>  
  
-   <span data-ttu-id="664a3-115">啟用單一接收位置</span><span class="sxs-lookup"><span data-stu-id="664a3-115">Enable a single receive location</span></span>  
  
-   <span data-ttu-id="664a3-116">取消登錄單一傳送埠</span><span class="sxs-lookup"><span data-stu-id="664a3-116">Unenlist a single send port</span></span>  
  
-   <span data-ttu-id="664a3-117">停用單一接收位置</span><span class="sxs-lookup"><span data-stu-id="664a3-117">Disable a single receive location</span></span>  
  
-   <span data-ttu-id="664a3-118">啟動單一協調流程</span><span class="sxs-lookup"><span data-stu-id="664a3-118">Start a single orchestration</span></span>  
  
-   <span data-ttu-id="664a3-119">取消登錄單一協調流程</span><span class="sxs-lookup"><span data-stu-id="664a3-119">Unenlist a single orchestration</span></span>  
  
## <a name="example"></a><span data-ttu-id="664a3-120">範例</span><span class="sxs-lookup"><span data-stu-id="664a3-120">Example</span></span>  
 <span data-ttu-id="664a3-121">本主題中的範例程式碼包含不同的程式碼區段，可以執行＜展示＞一節所列的功能。</span><span class="sxs-lookup"><span data-stu-id="664a3-121">The sample code in this topic includes separate code sections to do the functions listed in the "Demonstrates" section.</span></span>  
...
## <a name="see-also"></a><span data-ttu-id="664a3-122">另請參閱</span><span class="sxs-lookup"><span data-stu-id="664a3-122">See Also</span></span>  
 [<span data-ttu-id="664a3-123">範例</span><span class="sxs-lookup"><span data-stu-id="664a3-123">Samples</span></span>](../../adapters-and-accelerators/accelerator-rosettanet/samples3.md)