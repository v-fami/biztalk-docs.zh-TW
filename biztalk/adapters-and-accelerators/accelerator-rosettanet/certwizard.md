...
您可以使用 CertWizard 公用程式從.pfx 或.cer 檔案匯入憑證到私用或公用存放區，以搭配 Microsoft® [!INCLUDE[BTARN_CurrentVersion_FirstRef](../../includes/btarn-currentversion-firstref-md.md)]。  
...
|**Useridentity**|一或多個 BizTalk 主控件使用的服務識別。 如果您不想指定主控件，但想在使用者帳戶下匯入憑證，請輸入使用者帳戶。 **注意︰** 如果您沒有加入**Useridentity**切換，公用程式匯入和設定所有使用者的憑證。 **注意︰** 如果您新增**Useridentity**參數，但不是輸入值，WMI 會自動產生使用者識別。|  
|**密碼**|服務識別使用者的密碼。|  
|**憑證指紋**|特定憑證的指紋，以防檔案包含一個以上的憑證。 **注意：** 公開憑證檔案，如果檔案包含一個以上的憑證，而且您未指定指紋，此公用程式匯入檔案中的所有憑證。 對於私用憑證檔案，此公用程式會提示您選取要匯入的憑證。|  
|**Usage**|匯入之私用憑證的用途。 可為 sign (對於簽章憑證)、decrypt (對於解密憑證)、both (對於同時是簽章憑證又是解密憑證的憑證)，或 none (也是對於同時是簽章憑證又是解密憑證的憑證)。 **注意︰** 如果您設定 **/Usage**參數設為 none，精靈不會在 BizTalk 主控件或 BizTalk 群組上設定憑證的指紋。|  
