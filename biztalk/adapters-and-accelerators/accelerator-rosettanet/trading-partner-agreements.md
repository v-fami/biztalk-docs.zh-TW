...
[!INCLUDE[btsCoName](../../includes/btsconame-md.md)][!INCLUDE[BTARN_CurrentVersion_FirstRef](../../includes/btarn-currentversion-firstref-md.md)]交易夥伴協議 (TPA)，以處理與夥伴的訊息交換。 TPA 定義兩個交易夥伴之間訊息處理及驗證的規格。 它定義了這些交易夥伴實作相關交易夥伴介面程序 (PIP) 的方式，指定所有特定訊息類型實作的訊息內容。 TPA 也定義交易夥伴在網際網路上交換訊息的規格。  
