...
RosettaNet 組織定義 RosettaNet 實作架構 (RNIF) 規格中的訊息交換。 RNIF 定義整合系統傳輸訊息的方式。 [!INCLUDE[btaBTARN3.3abbrevnonumber](../../includes/btabtarn3-3abbrevnonumber-md.md)] 完整實作 RNIF 規格，將該功能新增至哪些 Microsoft[!INCLUDE[BTARN_CurrentVersion_FirstRef](../../includes/btarn-currentversion-firstref-md.md)]原生提供的立即可用。  
  
RNIF 通訊相當複雜。 執行 RNIF 處理的公開程序包含各種不同的驗證檢查和複雜的工作流程邏輯。 [!INCLUDE[btaBTARN3.3abbrevnonumber](../../includes/btabtarn3-3abbrevnonumber-md.md)] 原生提供這項功能。 這可讓您不需要從頭開發或維護 RNIF 邏輯，就能使用與 RosettaNet 相容的系統。  
...
整合系統執行 RNIF 傳送透過 HTTP/HTTPS 及 SMTP;不過，[!INCLUDE[btaBTARN3.3abbrevnonumber](../../includes/btabtarn3-3abbrevnonumber-md.md)]實作只有 HTTP/HTTPS。 [!INCLUDE[btaBTARN3.3abbrevnonumber](../../includes/btabtarn3-3abbrevnonumber-md.md)] 在 RNIF 1.1 不支援附件和同步交易。  
...
在程序組態設定檔中個別設定服務內容及通知的不可否認性需求。 如果您設定一或兩個**來源與內容的不可否認**並**不可否認性所需**選項，以`True`，然後[!INCLUDE[btaBTARN3.3abbrevnonumber](../../includes/btabtarn3-3abbrevnonumber-md.md)]會儲存下列資料：  
