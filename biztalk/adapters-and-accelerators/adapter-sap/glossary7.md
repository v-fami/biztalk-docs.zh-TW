...
下列詞彙和定義會用於[!INCLUDE[adaptersap](../../includes/adaptersap-md.md)]。  
...
**Bapi （商務應用程式開發介面）**  
SAP 系統交換 SAP 與其他系統之間的商務所公開的標準介面。 Bapi 用來整合不同的元件，透過本機網路或網際網路上彼此連接。
...
**商務物件**  
封裝的 SAP 資料和物件導向的模型中的程序。 外部用戶端叫用方法呼叫商務物件的應用程式介面 (Bapi)，以在物件上執行 （和其基礎的 SAP 成品）。 根據預設，SAP 配接器所公開的商務物件來取用應用程式。 商務物件只可供用戶端操作。

**商務物件存放庫 (BOR)**  
包含所有的 SAP 商業物件類型和 SAP 介面型別，以及其元件 （例如方法和屬性） 常駐在 SAP / 3 系統上。  
...
**字碼頁**基本上字碼指標的矩陣。 在資料庫中，包括字元的所有資料都會都儲存為位元組序列。 字元資料的字碼頁是位元組序列和字元之間的對應。
...
**函式模組**  
在 SAP 系統中定義為 ABAP 程式的程序。 這些是函式群組包含函式模組的邏輯群組的一部分。 函式的模組可讓 SAP 用戶端，以重複使用和封裝全域函式。
...
**中繼文件 (IDOC)**  
使用 / 3、 R/2 和非 SAP 系統之間交換資料的結構化文件。  
  
## <a name="m"></a>M  
  
**中繼資料**  
在 WCF 中，指的服務所公開的合約描述。 這就所謂的服務描述，並表示 WSDL 文件中。 配接器所公開的中繼資料描述 （介面），它可以在基礎的特定業務 (LOB) 系統上執行的作業。
...
**程式識別碼**  
唯一的通道傳送 RFC 中設定的閘道。 程式識別碼或主機名稱和 SAP 閘道服務，提供的程式識別碼必須相符 （透過 RfcAccept) 的註冊參數。
...
**遠端函式呼叫 (RFC)**  
用戶端和伺服器之間交換資料的程序。 用戶端通常會呼叫伺服器程式和伺服器會傳回結果，透過 TCP/IP 連線。
...
**交易識別碼 (TID)**  
SAP 將每個邏輯單元的工作 (LUW) 與相關聯的全域唯一識別碼。  
