...
5.  指定 SELECT 陳述式中的**CommandText**屬性**SAPCommand**。 如果有必要，您可以指定參數使用**SAPParameter**物件。 如需如何查詢使用 SELECT 陳述式的 SAP 成品的詳細資訊，請參閱[SAP 中的 SELECT 陳述式的語法](../../adapters-and-accelerators/adapter-sap/syntax-for-a-select-statement-in-sap.md)。 如需如何指定 RFC 或 BAPI 的範例，請參閱[SELECT 陳述式的範例](../../adapters-and-accelerators/adapter-sap/examples-for-select-statement.md)。  
...
## <a name="example"></a>範例  
 下列範例會寫到主控台的參數化的內部聯結陳述式 select 的結果。  
