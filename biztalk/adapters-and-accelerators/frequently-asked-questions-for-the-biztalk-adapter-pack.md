...
關於常見問題集 (Faq) [!INCLUDE[adapterpacknoversion](../includes/adapterpacknoversion-md.md)]。  
...
### <a name="what-are-the-supported-biztalk-versions-for-the-biztalk-adapter-pack"></a>BizTalk Adapter Pack 支援的 BizTalk 版本有哪些？  
 [!INCLUDE[adapterpacknoversion](../includes/adapterpacknoversion-md.md)]隨附於 Microsoft [!INCLUDE[btsBizTalkServerNoVersion_md](../includes/btsbiztalkservernoversion-md.md)]。 安裝隨附的版本您[!INCLUDE[btsBizTalkServerNoVersion_md](../includes/btsbiztalkservernoversion-md.md)]版本。 安裝[!INCLUDE[adapterpacknoversion](../includes/adapterpacknoversion-md.md)]從另一個[!INCLUDE[btsBizTalkServerNoVersion_md](../includes/btsbiztalkservernoversion-md.md)]不支援版本。 
  
### <a name="in-which-user-context-should-the-setup-be-run"></a>哪些使用者內容應該在安裝程式執行？  
執行[!INCLUDE[adapterpacknoversion](../includes/adapterpacknoversion-md.md)]使用屬於本機 administrators 群組的成員帳戶的安裝和[!INCLUDE[btsBizTalkServerNoVersion](../includes/btsbiztalkservernoversion-md.md)]Administrators 群組。 
...
如果您是新手[!INCLUDE[btsBizTalkServerNoVersion_md](../includes/btsbiztalkservernoversion-md.md)]或[!INCLUDE[adapterpacknoversion](../includes/adapterpacknoversion-md.md)]，看看 get 啟動主題，並逐步執行教學課程： 
...
是的。 [!INCLUDE[adapterpacknoversion](../includes/adapterpacknoversion-md.md)]提供**LOB 的時間 （累計）**效能計數器來測量時間 （毫秒），以完成某個動作，起始配接器所採用的 LOB 用戶端程式庫。  您可以藉由設定啟用效能計數器`EnablePerformanceCounters`內容繫結至**True**。 若要停用效能計數器，請設定`EnablePerformanceCounters`至**False** （預設值）。   

## <a name="biztalk-server-questions"></a>BizTalk Server 的問題

### <a name="which-biztalk-server-tools-are-used-while-working-with-adapters-where-can-i-find-more-information-about-these-tools"></a>使用配接器時，要使用哪些 BizTalk Server 工具？ 何處可以找到這些工具的詳細資訊？  
有數個工具，可協助使用這些介面卡的成品： 
...
是的。 繫結之間建立對應的邏輯端點，例如協調流程連接埠或角色連結，以及實體端點，例如傳送和接收埠。 這讓通訊能在不同的 BizTalk 商務方案元件之間進行。 繫結資訊會儲存在 XML 檔案，其中包含每個 BizTalk 協調流程範圍中的 BizTalk 組件、 應用程式或群組的繫結資訊。 您可以匯出 BizTalk 組件、 應用程式或群組中，繫結，並重複使用將它匯入至 BizTalk 應用程式或群組。 如需詳細資訊，請參閱 

[重複使用 Oracle 資料庫配接器繫結](../adapters-and-accelerators/adapter-oracle-database/reuse-oracle-database-adapter-bindings.md)  
...
### <a name="what-is-the-transaction-isolation-level-how-can-i-configure-it"></a>什麼是 「 交易隔離層級 」？ 如何設定它？  
 交易隔離等級會決定交易是與其他交易所進行的資料變更互相隔離的程度。 它定義的特定業務 (LOB) 系統的連接所發出的 TRANSACT-SQL 命令的鎖定行為。 
  
這是可配接器的某些設定。 請參閱： 

[Oracle 資料庫： 設定交易隔離等級和交易逾時](../adapters-and-accelerators/adapter-oracle-database/configure-transaction-isolation-level-and-transaction-timeout-with-oracle-db.md)  
[Oracle E-business Suite： 設定交易隔離等級和交易逾時](../adapters-and-accelerators/adapter-oracle-ebs/configure-transaction-isolation-level-and-transaction-timeout-with-oracle-ebs.md)  
[SQL： 設定交易隔離等級和交易逾時](../adapters-and-accelerators/adapter-sql/configure-transaction-isolation-level-and-transaction-timeout-with-sql.md)

[SQL Database Engine 中的隔離等級](https://technet.microsoft.com/library/ms189122.aspx)描述在 SQL 中的不同層級。 

## <a name="wcf-based-adapter-faqs"></a>WCF 架構的配接器常見問題集

### <a name="what-is-wcf"></a>WCF 是什麼？
 WCF 代表 Windows Communication Foundation。 WCF 是 Microsoft 開發的建置服務導向應用程式的程式設計架構。 WCF 是.NET framework 的一部分，並可讓開發人員建置安全、 可靠及交易性的方案會跨平台整合，並與現有投資交互操作。  
...
### <a name="what-is-wcf-lob-adapter-sdk"></a>WCF LOB 配接器 SDK 是什麼？  
 WCF LOB 配接器 SDK 是工具和元件，提供一致的架構開發可重複使用、 豐富的中繼資料的特定業務系統的配接器的集合。 配接器使用 WCF LOB 配接器 SDK 撰寫當成自訂 WCF 繫結，並可供 WCF 支援的用戶端。 
 
相關的連結： [WCF 線條的商務配接器 SDK 文件](../adapters-and-accelerators/wcf-lob-adapter-sdk/microsoft-wcf-line-of-business-adapter-sdk-documentation.md) 

### <a name="what-is-the-wcf-service-model"></a>WCF 服務模型是什麼？  
 WCF 服務模型是在其中部署 LOB 系統 （例如 Oracle 或 SAP） 會公開為 WCF 服務的 WCF 提供的程式設計模型。 用戶端和服務之間存在的服務合約以.NET 介面和作業會表示為此介面上的方法。 WCF 服務模型會產生 proxy 類別，WCF 用戶端類別，透過此程式碼可以叫用作業，並接收使用配接器的資料。 
 ...
 WCF 通道模型是抽象的低層級的用戶端與服務之間的 SOAP 訊息交換。 它提供介面和類型，可讓您傳送和接收訊息所使用的是稱為通道堆疊的多層式通訊協定堆疊。 堆疊的每個圖層由通道時，所組成，且每個通道會建立從 WCF 繫結。 每個配接器是 WCF 自訂傳輸繫結會公開為 WCF 服務的 LOB 系統。 
...
### <a name="when-should-i-use-the-wcf-service-model-or-the-wcf-channel-model"></a>何時應該使用 WCF 服務模型或 WCF 通道模型？  
 WCF 服務模型會顯示模型，熟悉.NET 程式設計人員，並透過通道隱藏基礎 SOAP 訊息交換的複雜性。 此外，加入的配接器服務參考外掛程式與 Visual Studio 設計體驗，整合，而且會呈現標準的 Microsoft Windows 介面提供強大瀏覽和搜尋功能，對配接器所公開的作業。 因此，WCF 服務模型通常是最好的選擇來開發任何 WCF 架構的配接器程式設計方案。  

 您想要使用透過 WCF 服務的 WCF 通道模型建立模型時：  
  
-   WCF 通道模型提供更細微的控制，因為在 WCF 通道模型中，您直接控制您透過通道傳送訊息的內容部署 LOB 系統執行的作業移轉。  
  
-   WCF 通道模型提供更廣泛支援端對端的資料流的大型物件 (LOB) 資料類型，與 WCF 服務模型。 這是因為在 WCF 通道模型中，您直接控制如何於外寄訊息提供訊息內文和您要如何處理內送訊息的訊息本文。 
  
### <a name="how-do-i-get-started-with-the-wcf-service-model"></a>如何開始使用 WCF 服務模型？  
 提供根據 WCF 服務模型來產生 WCF 用戶端類別或 WCF 服務合約和相關聯的協助程式程式碼取自服務中繼資料公開由各個配接器，您可以使用下列工具之一：  
  
-   ServiceModel Metadata Utility Tool (svcutil.exe)，隨附 WCF。  
  
-   加入配接器服務參考 Visual Studio 外掛程式，隨附[!INCLUDE[adapterpacknoversion_md](../includes/adapterpacknoversion-md.md)]。  
  
### <a name="how-do-i-get-started-with-the-wcf-channel-model"></a>如何開始使用 WCF 通道模型？  
 使用 WCF 通道模型，可以叫用作業，並藉由使用配接器的 SOAP 訊息交換 WCF 通道上收到輪詢查詢的結果。 若要開始，您必須建立輸出 （用戶端） 和輸入 （服務） 通道。
