...
下列詞彙和定義會用於[!INCLUDE[adapteroracle](../../includes/adapteroracle-md.md)]。    
...
**BFILE**  
Oracle 資料類型可讓您存取二進位檔案會儲存在檔案系統外部到 Oracle 資料庫中的 Lob。 BFILE 資料行儲存 BFILE 定位器，代表的目錄名稱和檔案名稱，其中包含伺服器檔案系統上的資料。

**二進位大型物件**  
1.  大型資料片段的例如點陣圖、 大型的欄位值、 無法預測資料表大小和是從應用程式的觀點來看不規則的資料特性。
2.  關鍵字指定 BLOB 結構，其中包含的資料區塊的相關資訊。
...
**資料操作語言 (DML)**  
用於擷取和操作資料的 SQL 陳述式子集。 此外，DML 陳述式通常會以 SELECT、 INSERT、 UPDATE 或 DELETE 開頭。
...
**區域命名方法**  
Oracle 命名 Oracle 資料庫配接器所支援的方法。 在這個命名方法，對 Oracle 用戶端的網路服務名稱解析本機 tnsnames.ora 檔案中的項目。  
  
## <a name="m"></a>M  
  
**中繼資料**  
在 WCF 中，指的服務所公開的合約描述。 這就所謂的服務描述，並表示 WSDL 文件中。 配接器所公開的中繼資料描述 （介面），它可以在基礎 LOB 系統上執行的作業。
...
**命名的方法**  
Oracle 命名的方法會決定對 Oracle 用戶端如何取得 Oracle 資料庫服務 （執行個體） 的連接資訊。 您可以設定 Oracle 用戶端使用特定的命名方法，方法是使用 Oracle Net Configuration Assistant。 Oracle 資料庫配接器支援本機命名的方法。

**網路服務名稱**  
別名由 Oracle 用戶端用來取得 Oracle 資料庫的連接資訊。 您可以提供網路服務名稱做為其中一個連線 URI 的連線屬性。  
...
**輪詢**  
裝置驅動程式用來找出從多個裝置它們是否包含資料傳輸技術。 裝置會輪詢一次一個。
...
**REF CURSOR**  
Oracle 的 PL/SQL 資料類型表示的結果集的 Oracle 資料庫中的指標。 REF CURSOR 類型啟用輸入和輸出資料流的資料，而且適合用來傳輸大量資料的 PL/SQL 程式碼區塊。
...
**WS 中繼資料交換 (MEX) 端點**  
它會實作 WCF 服務，例如配接器，所公開的端點**IMetadataExchange**介面。 WS 中繼資料交換端點可用來擷取服務描述 (WSDL) 的目標系統上的配接器所公開的作業。
