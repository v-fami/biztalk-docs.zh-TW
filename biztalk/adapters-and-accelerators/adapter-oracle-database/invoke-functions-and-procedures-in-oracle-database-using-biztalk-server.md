...
-   多載函式和預存程序。 請參閱[叫用多載函式和 Oracle 資料庫使用 BizTalk Server 中的程序](../../adapters-and-accelerators/adapter-oracle-database/run-overloaded-functions-and-procedures-in-oracle-database-using-biztalk-server.md)。  
  
-   函式和程序逾時，需要在和在 OUT REF CURSOR 參數。 請參閱[叫用的函數和程序與使用 BizTalk Server 的 Oracle 資料庫中的 REF CURSOR](../../adapters-and-accelerators/adapter-oracle-database/run-functions-and-procedures-with-ref-cursors-in-oracle-db-using-biztalk-server.md)。  
  
-   函數和程序需要 IN、 OUT，以及在出記錄型別參數。 請參閱[叫用的函數和程序與使用 BizTalk Server 的 Oracle 資料庫中的記錄類型](../../adapters-and-accelerators/adapter-oracle-database/run-functions-and-procedures-with-record-types-in-oracle-db-with-biztalk-server.md)。  
  
 如需有關使用函式和預存程序使用[!INCLUDE[adapteroracle_short](../../includes/adapteroracle-short-md.md)]，請參閱[函式和 Oracle 資料庫中的預存程序上的作業](../../adapters-and-accelerators/adapter-oracle-database/operations-on-functions-and-stored-procedures-in-oracle-database.md)。 叫用函數和程序的 SOAP 訊息結構的相關資訊，請參閱[函數和程序的訊息結構描述](../../adapters-and-accelerators/adapter-oracle-database/message-schemas-for-functions-and-procedures.md)。  
...
## <a name="sample-based-on-this-topic"></a>根據本主題的範例  
 基礎的範例，InvokeFunction，本主題也會提供[!INCLUDE[adapterpacknoversion](../../includes/adapterpacknoversion-md.md)]。 如需詳細資訊，請參閱[配接器範例](../../adapters-and-accelerators/accelerator-rosettanet/adapter-samples.md)。  
  
## <a name="generating-schema"></a>產生結構描述  
 本主題中，以示範如何叫用函數時，我們會叫用 SCOTT\Package\ACCOUNT_PKG 結構描述在 CREATE_ACCOUNT 函數。 此套件會建立下 SCOTT 結構描述，執行下列 SQL 指令碼提供範例。 CREATE_ACCOUNT 函數接受帳戶和位址做為輸入的資訊並建立帳戶和客戶資料表中的記錄。 如果記錄已經存在，此函數會傳回零;否則，函數會傳回帳戶識別碼。 若要深入了解範例和 SQL 指令碼，請參閱[結構描述範例](../../adapters-and-accelerators/accelerator-rosettanet/schema-samples.md)。  
  
 要叫用 CREATE_ACCOUNT 函式，我們必須產生相同的函式下 SCOTT\Package\ACCOUNT_PKG 結構描述的結構描述。 請參閱[擷取 Visual Studio 中的 Oracle 資料庫作業的中繼資料](../../adapters-and-accelerators/adapter-oracle-database/get-metadata-for-oracle-database-operations-in-visual-studio.md)如需有關如何產生結構描述。  
...
4.  在**屬性**窗格**Message_1**，執行下列動作：  
    ...
    |--------------|----------------|  
    |識別碼|型別**要求**。|  
    |訊息類型|從下拉式清單中，展開 **結構描述**，然後選取*InvokeFunction.OracleDBBindingSchema.CREATE_ACCOUNT*，其中*InvokeFunction*名稱您BizTalk 專案。 *OracleDBBindingSchema*是 CREATE_ACCOUNT 函式所產生的結構描述。|  
...
5.  重複上述步驟，建立新的訊息。 在**屬性**窗格新訊息中，執行下列動作：  
    ...
    |--------------|----------------|  
    |識別碼|型別**回應**。|  
    |訊息類型|從下拉式清單中，展開 **結構描述**，然後選取*InvokeFunction.OracleDBBindingSchema.CREATE_ACCOUNTResponse*。|  
  
## <a name="setting-up-the-orchestration"></a>設定協調流程  
 您必須建立 BizTalk 協調流程使用[!INCLUDE[btsBizTalkServerNoVersion](../../includes/btsbiztalkservernoversion-md.md)]叫用 Oracle 資料庫上的函式。 在此協調流程中，卸除要求訊息已定義的接收位置。 [!INCLUDE[adapteroracle_short](../../includes/adapteroracle-short-md.md)]會取用這個訊息，並將它傳遞到 Oracle 資料庫中，透過 ODP。 從 Oracle 資料庫的回應會儲存到另一個位置。 典型的協調流程叫用函式和 Oracle 資料庫中的程序就會包含：  
...
-   單向傳送埠，以便從 Oracle 資料庫的回應傳送到資料夾。  
 ...
 ![要叫用 Oracle 中的函式的協調流程](../../adapters-and-accelerators/adapter-oracle-database/media/f102c81b-be2b-4e3c-89aa-25a4af5e6739.gif "f102c81b-be2b-4e3c-89aa-25a4af5e6739")  
...
|形狀圖|屬性|  
|-----------|----------------|  
|ReceiveMessage|-設定**訊息**至*要求*<br />-設定**作業**至*FileIn.Function.Request*|  
|SendMessage|-設定**訊息**至*要求*<br />-設定**作業**至*LOBPort.Function.Request*|  
|ReceiveResponse|-設定**訊息**至*回應*<br />-設定**作業**至*LOBPort.Function.Response*|  
|SendResponse|-設定**訊息**至*回應*<br />-設定**作業**至*SaveResponse.Function.Request*|  
...
## <a name="executing-the-operation"></a>執行作業  
 執行應用程式之後，您必須先卸除要求訊息到檔案接收位置。 要求訊息的結構描述必須符合您先前產生的函式的結構描述。 請參閱[函數和程序的訊息結構描述](../../adapters-and-accelerators/adapter-oracle-database/message-schemas-for-functions-and-procedures.md)的詳細資訊的要求訊息結構描述叫用函式使用[!INCLUDE[adapteroracle_short](../../includes/adapteroracle-short-md.md)]。  
