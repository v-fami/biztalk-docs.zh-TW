...
繫結會建立邏輯端點 （例如協調流程連接埠或角色連結） 與實體端點之間的對應 (例如傳送和接收埠)。 這讓通訊能在不同的 BizTalk 商務方案元件之間進行。 您可以使用，以建立繫結[!INCLUDE[btsBizTalkServerNoVersion](../../includes/btsbiztalkservernoversion-md.md)]管理主控台。  
...
指定使用者名稱和密碼的相關資訊，請參閱[適用於 SQL Server，設定登入認證](../../adapters-and-accelerators/adapter-sql/configure-the-sign-in-credentials-for-the-sql-adapter.md)。
