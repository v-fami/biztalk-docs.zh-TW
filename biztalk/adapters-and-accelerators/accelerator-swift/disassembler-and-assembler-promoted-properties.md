...
|**A4SWIFT_PosInBatch**|表示來自傳入的批次訊息的序數位置。 批次包含 *n* 訊息， **A4SWIFT_PosInBatch**接受從 1 到值 *n* 、 對應至訊息的序數將批次中。<br /><br /> 設定為**0**訊息是否為批次標頭。<br /><br /> 設定為**n + 1**訊息是否為批次結尾。<br /><br /> 設定為**1**如果訊息本身就是整個批次 （停用批次片段）。<br /><br /> 設定為**-1**單一訊息 （不來自傳入的批次）。|數值|>= -1|從相同的傳入批次成為原始它們到達的順序排序訊息。|  
