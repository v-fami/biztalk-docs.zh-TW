...
2.  在 [加入現有項目-SWIFTSchemas] 對話方塊中，瀏覽至  **\<*磁碟機*>: \Program Files\Microsoft BizTalk Accelerator for SWIFT\<版本 > MessagePack\SWIFTMessages\A4SWIFT SRG\<版本 > \Base 結構描述 * *。  
  
3.  選取**SWIFT 基底 Types.xsd**和**SWIFT 常見資料 Types.xsd**結構描述，然後再按一下**新增**。  
  
    > [!NOTE]
    >  SWIFT 基底 Types.xsd 和 SWIFT 常見資料 Types.xsd 結構描述會出現在 方案總管 SWIFTSchemas 專案底下。  
...
6.  選取**MT103.xsd**結構描述，然後再按一下**新增**。  
    ...
    > [!NOTE]
    >  新的結構描述 MT103.xsd，會出現在方案總管 中，SWIFTSchemas 專案底下。  
  
7.  在 方案總管 中，按兩下**MT103.xsd** BizTalk 編輯器 中檢視結構描述。  
  
    > [!NOTE]
    >  結構描述樹狀結構 （左窗格） 和 XSD 檢視 （右窗格） 會出現在 [BizTalk 編輯器] 中。  
  
8.  在**檔案**功能表上，按一下 **全部儲存**以儲存變更。  
  
 若要繼續[第 4 課： 建立和部署組件](../../adapters-and-accelerators/accelerator-swift/lesson-4-building-and-deploying-the-assembly.md)。