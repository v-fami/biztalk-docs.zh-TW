...
---
# <a name="reuse-adapter-bindings-with-oracle-e-business-suite"></a>重複使用與 Oracle E-business Suite 配接器繫結
繫結會建立邏輯端點 （例如協調流程連接埠或角色連結） 與實體端點之間的對應 (例如傳送和接收埠)。 這可讓不同元件之間的通訊[!INCLUDE[btsBizTalkServerNoVersion](../../includes/btsbiztalkservernoversion-md.md)]商務解決方案。 您可以使用，以建立繫結[!INCLUDE[btsBizTalkServerNoVersion](../../includes/btsbiztalkservernoversion-md.md)]管理主控台。  
...
- 每個設定傳送埠、 接收埠、 接收位置，以及已設定的合作對象。  
  ...
  BizTalk 組件、 應用程式或群組不會自動產生繫結檔案。 不過，您可以產生繫結檔案匯出繫結。 同樣地，您可以接著將繫結檔案匯入的應用程式或群組。 本節提供有關如何匯入和匯出繫結的指示。  
...
指定使用者名稱和密碼的相關資訊，請參閱[for Oracle E-business Suite 中設定的登入認證](../../adapters-and-accelerators/adapter-oracle-ebs/configure-the-sign-in-credentials-for-the-oracle-e-business-suite.md)。
