...
---
# <a name="reuse-adapter-bindings-with-oracle-e-business-suite"></a>Oracle E-business Suite 與重複使用配接器繫結
繫結之間建立對應的邏輯端點 （例如協調流程連接埠或角色連結） 與實體端點 (例如傳送和接收埠)。 這可讓不同元件之間的通訊[!INCLUDE[btsBizTalkServerNoVersion](../../includes/btsbiztalkservernoversion-md.md)]商務解決方案。 您可以使用，以建立繫結[!INCLUDE[btsBizTalkServerNoVersion](../../includes/btsbiztalkservernoversion-md.md)]管理主控台。  
