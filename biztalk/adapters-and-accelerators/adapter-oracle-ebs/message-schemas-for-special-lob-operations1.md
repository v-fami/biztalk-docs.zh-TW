...
Read_\<LOBColName > 和 Update_\<LOBColName > 作業便會顯示資料表和檢視表包含 LOB 資料行，其中\<LOBColName > 是資料表或檢視表中的 LOB 資料行。 這些作業可讓您讀取或寫入 base64Binary 編碼資料的資料流的形式的 LOB 資料。 它們是在單一資料列的 LOB 資料的單一資料行上運作。  
  
 如需 Read_ 的概觀\<LOBColName > 和 Update_\<LOBColName > 作業和 Oracle LOB 資料類型的支援，請參閱[介面資料表、 介面檢視、 資料表和檢視表，包含 LOB 作業資料](../../adapters-and-accelerators/adapter-oracle-ebs/read-and-update-on-interface-tables-and-views-with-large-object-data-types.md)。  
...
> [!IMPORTANT]
>  Read_ 的訊息結構\<LOBColName > 和 Update_\<LOBColName > 檢視表上的作業的相同資料表上會有不同之處在於此作業的命名空間指定的檢視，而不是資料表： `<ReadLOB xmlns ="[VERSION]/Views/[SCHEMA]/[VIEW_NAME]">`。  
