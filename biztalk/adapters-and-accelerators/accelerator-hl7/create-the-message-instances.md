...
若要建立 ADT^A03.txt 訊息檔案，並建立訊息執行個體，您必須使用當您執行批次處理教學課程，請使用下列程序。  
  
> [!NOTE]
>  在 [記事本] 中建立這些訊息時，刪除傳回下列的最後一行的歸位字元。  
  
### <a name="to-create-the-fragmented-batch-message-instance-text-file"></a>若要建立的片段化批次訊息執行個體的文字檔案  
...
3.  將檔案儲存為**FragmentedInboundBatch.txt**中\<*磁碟機*:\>\Batching Tutorial\Instances 資料夾，然後再關閉 [記事本]。  
  
### <a name="to-create-the-batch-inbatch-out-message-instance-text-file"></a>若要建立中的批次 / 批次訊息執行個體的文字檔案出  
...
3.  將檔案儲存為**BatchInBatchOut.txt**中\<*磁碟機*:\>\Batching Tutorial\Instances 資料夾，然後再關閉 [記事本]。  
  
### <a name="to-create-the-create-batch-message-instance-text-files"></a>若要建立文字檔案的建立批次訊息執行個體  
...
3. 將檔案儲存為**CreateBatchMessage1.txt**中\<*磁碟機*:\>\Batching Tutorial\Instances 資料夾，然後再關閉 [記事本]。  
  
4. 將下列文字複製到 [記事本] 的新執行個體中：  
...
5. 將檔案儲存為**CreateBatchMessage2.txt**中\<*磁碟機*:\>\Batching Tutorial\Instances 資料夾，然後再關閉 [記事本]。  
  
   請繼續進行[第 1 部分： 片段化的輸入批次案例](../../adapters-and-accelerators/accelerator-hl7/part-1-fragmented-inbound-batch-scenario.md)。