---
title: HL7 2.X 通用結構描述檔案 |Microsoft Docs
ms.custom: ''
ms.date: 06/08/2017
ms.prod: biztalk-server
ms.reviewer: ''
ms.suite: ''
ms.tgt_pltfrm: ''
ms.topic: article
helpviewer_keywords:
- HL7, schemas
- 2.X schemas, common files
- HL7, 2.X schemas
- common schemas
- schemas, 2.X schemas
ms.assetid: df909f3f-b151-44c8-b3a5-3346a6a44ac1
caps.latest.revision: 3
author: MandiOhlinger
ms.author: mandia
manager: anneta
ms.openlocfilehash: dd026b30a1a13f92019d3f275811fab80ac687e1
ms.sourcegitcommit: 266308ec5c6a9d8d80ff298ee6051b4843c5d626
ms.translationtype: MT
ms.contentlocale: zh-TW
ms.lasthandoff: 06/27/2018
ms.locfileid: "36994679"
---
# <a name="hl7-2x-common-schema-files"></a>HL7 2.X 通用結構描述檔案
每個 HL7 訊息結構描述需要提供資料類型、 區段和資料表的結構描述定義的通用結構描述檔案。 這些通用的結構描述檔案會定義每個版本的 HL7 2.X。 在 Microsoft 中部署的專案[!INCLUDE[btsVStudioNoVersion](../../includes/btsvstudionoversion-md.md)]包含訊息結構描述必須參考包含三個常見的結構描述的通用專案的部署執行個體。  
  
## <a name="in-this-section"></a>本節內容  
  
-   [資料類型的通用結構描述](../../adapters-and-accelerators/accelerator-hl7/data-types-common-schemas.md)  
  
-   [區段的通用結構描述](../../adapters-and-accelerators/accelerator-hl7/segments-common-schemas.md)  
  
-   [資料表值的通用結構描述](../../adapters-and-accelerators/accelerator-hl7/table-values-common-schemas.md)