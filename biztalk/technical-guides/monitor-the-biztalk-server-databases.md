...
> [!IMPORTANT]  
>  這項工作的失敗不一定是構成嚴重的問題，但而不是問題，應調查和解決的定期維護 BizTalk Server 資料庫的一部分。 此作業失敗時設計事件會探索其中一個上面所列的問題。 如上面所列問題的詳細資訊和存取公用程式常用的 Microsoft 產品支援服務疑難排解這些問題，請參閱[使用 BizTalk 結束字元來解決 BizTalk 所識別的問題MsgBoxViewer](http://go.microsoft.com/fwlink/?LinkId=210367) (http://go.microsoft.com/fwlink/?LinkId=210367)。