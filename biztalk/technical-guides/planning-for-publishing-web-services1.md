...
## <a name="overview"></a>概觀
 ...
 [!INCLUDE[btsBizTalkServerNoVersion](../includes/btsbiztalkservernoversion-md.md)]實作支援透過 SOAP 配接器使用 Web 服務中的原生配接器。 原生配接器支援提供 Web 服務的擴充性、容錯和追蹤功能，且無須撰寫任何程式碼。 如需有關 SOAP 配接器的詳細資訊，請參閱[SOAP 配接器](../core/soap-adapter.md)。  
...
### <a name="internet-information-services"></a>Internet Information Services
 您可以發佈 Web 服務，Windows 系統上有 IIS 與 ASP.NET 設定。 在 IIS 中，所有的 Web 服務會執行 ASP.NET 工作者處理序內。  
  
 IIS 用來處理 Web 服務要求的應用程式集區。 IIS 支援多個應用程式集區和每個應用程式集區處理序可以在不同的使用者內容下執行。  
...
### <a name="biztalk-isolated-hosts"></a>BizTalk 外掛式主控件  
 ...
 BizTalk Server 組態管理員會建立 BizTalkServerIsolatedHost[!INCLUDE[btsBizTalkServerNoVersion](../includes/btsbiztalkservernoversion-md.md)]使用做為預設外掛式主控件。 BizTalk 外掛式主控件使用者群組是與這個主控件相關的 Windows 群組預設名稱。 如需主控件和主控件執行個體的詳細資訊，請參閱[管理 BizTalk 主控件和主控件執行個體](../core/managing-biztalk-hosts-and-host-instances.md)。  
...
-   您必須將應用程式集區的使用者帳戶加入適當的本機或網域群組的外掛式主控件。 請參閱[Windows 群組和 BizTalk Server 中的使用者帳戶](../core/windows-groups-and-user-accounts-in-biztalk-server.md)。  
...
請參閱[如何啟用 ASP.NET 4.0 的已發佈 Web 服務](../core/how-to-enable-asp-net-4-0-for-published-web-services.md)。  
  
## <a name="using-the-biztalk-web-services-publishing-wizard"></a>使用 BizTalk Web 服務發佈精靈  
 如需協調流程發佈為 Web 服務的詳細資訊，請參閱[協調流程發佈為 Web 服務](../core/publishing-an-orchestration-as-a-web-service.md)。  
  
 為 Web 服務發佈結構描述的相關資訊，請參閱[發佈為 Web 服務的結構描述](../core/publishing-schemas-as-a-web-service.md)。  
...
## <a name="planning-for-publishing-wcf-services"></a>規劃發佈 WCF 服務  
 ...
 如需有關規劃中的 WCF 服務[!INCLUDE[btsBizTalkServerNoVersion](../includes/btsbiztalkservernoversion-md.md)]，請參閱[發佈 WCF 服務](../core/publishing-wcf-services.md)。  
