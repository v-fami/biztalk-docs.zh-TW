 ...
 **轉換，並將訊息路由傳送至多個端點**  
 ...
 動態解析 」 範例隨附[!INCLUDE[esbToolkit](../includes/esbtoolkit-md.md)]示範這個使用案例。 它會顯示如何使用 ESB 管線元件，特別是，ESB 分派解譯器元件，以動態解析端點位置、 設定路由的屬性，並解析以及執行[!INCLUDE[prague](../includes/prague-md.md)]對應在訊息層級，而不使用協調流程。 它也示範單向和雙向的訊息模式。  
