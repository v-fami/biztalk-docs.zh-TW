 ...
 **定義自訂協調流程服務執行，使用行程**  
 ...
 隨附的行程上手範例[!INCLUDE[esbToolkit](../includes/esbtoolkit-md.md)]示範這個使用案例。 它會顯示如何建立包含解析，路由的行程和服務驗證為一系列的路線步驟定義的引動過程指示如何 ESB 和[!INCLUDE[prague](../includes/prague-md.md)]會處理輸入的訊息。 單向和要求-回應 」 範例隨附。  
