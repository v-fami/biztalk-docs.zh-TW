...
-   移除 BizTalk 組件時，不會自動移除位於全域組件快取 (GAC) 或本機檔案系統中的組件檔案。 您必須手動移除它。 如需相關指示，請參閱 <<c0> [ 如何從 GAC 解除安裝組件](http://msdn.microsoft.com/library/464706a8-f902-4d05-a724-19169facd2b4)並[如何移除其他檔案和設定 BizTalk 應用程式](../core/how-to-remove-other-files-and-settings-for-a-biztalk-application.md)。  
...
3. 按一下 **資源**資料夾中，以滑鼠右鍵按一下 BizTalk 組件中，然後**移除**。  
...
2. 輸入下列命令，並以適當的值替代，如下表所述：  
   ...
   |**/ Luid**|BizTalk 組件的本機唯一識別碼 (LUID)。 您可以使用來取得 LUID **ListApp**命令，如中所述[ListApp 命令](../core/listapp-command.md)。|  
