...
- 執行主應用程式的使用者必須屬於適當 BAM 活動事件寫入器 (bam_\<活動\>_EventWriter) 讓應用程式來讀取攔截器組態資訊和寫入的 SQL Server 角色BAM 活動。  
...
1. 按一下 **開始**，然後按一下**執行**開啟 Windows 命令提示字元。  

2. 型別**cmd**中**開放**欄位，，然後按一下**確定**。  
...
4. 使用 bm.exe 部署觀察模型：  
    ...
    請確定您取代\< *definitionfile.xml* \>您想要部署的觀察模型檔案的名稱。 如需其他選項請參閱[攔截器管理命令](../core/interceptor-management-commands.md)。  
...
4. 使用 bm.exe 部署攔截器組態檔：  
   ...
   > [!NOTE]
   >  您可以使用 **-Force: True**旗標，以覆寫現有的事件來源的攔截器組態檔中的相同名稱。 如果您這樣做，請務必先備份使用現有的組態**取得攔截器**命令。 使用 -Force:True 旗標可能會刪除任何參考要覆寫之事件來源的攔截器組態。  
