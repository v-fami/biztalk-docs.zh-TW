...
本主題描述如何使用 BizTalk Server 管理主控台來設定接收埠的輸入對應。 您會使用輸入對應，將輸入訊息從一個結構描述轉換至其他結構描述；例如，將接收自夥伴的訊息轉換成可供公司使用的結構描述。  
 ...
 您可以使用對應，將 XSL 轉換套用至接收埠傳送的訊息，而無需透過協調流程處理訊息。 您可以新增輸入對應、移除對應，或將現有對應變更為不同的對應。 您可以新增一個以上的對應到接收埠，但每一個對應都必須有唯一的來源結構描述。 如需地圖的背景資訊，請參閱 <<c0> [ 對應](../core/maps.md)。  
...
3. 按一下 **接收連接埠**，以滑鼠右鍵按一下接收埠，按一下**屬性**，然後按一下**輸入對應**。  
  
4. 下表中所述，設定輸入的對應，然後按一下**確定**。  
