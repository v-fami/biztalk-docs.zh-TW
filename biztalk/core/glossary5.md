...
|      BizTalk Management Pack alert (BizTalk 管理封包警示)      |                                                                                                                                                                     系統管理員可以在 BizTalk Server 管理組件中訂閱的通知。 訂閱某個警示之後，只要符合特定條件，系統管理員就會收到通知。 例如，若進行節流的主控件執行個體達到特定數目，即可能引發警示。                                                                                                                                                                      |
...
|EDIFACT UNOA syntax (EDIFACT UNOA 語法)|僅 EDIFACT 語法，允許下列字元： 大寫字母、 所有數字，保留空白，驚嘆號 （！）、 引號 （"）、 百分比符號 （%）、 連字號 (&)、 左和右括弧 ("("and")")、 星號 （*）、 逗號、 虛線 （-），小點 （.）、 斜線 （/），分號 （;），較不-符號 (\<)，和更新版本-符號 (\>)。|  
|EDIFACT UNOB syntax (EDIFACT UNOB 語法)|僅 EDIFACT 語法，允許下列字元： 小寫及大寫字母、 所有數字，保留空白，驚嘆號 （！）、 引號 （"）、 百分比符號 （%）、 連字號 (&)、 單一引號 （'）、 左和右括弧 （"（"和")")、 星號 （*）、 加號 （+）、 逗號、 虛線 （-）、 小數點 （.）、 較不將斜線 （/）、 冒號 （:）、 分號 （;） 的轉送-符號 (\<)，等號 （=）、 大於-符號 (\>)，和問號 （？）。|  
...
|mixed content (混合內容)|特定 XML 項目的內容，同時包含子項目以及不在子項目內的資料。 HTML 的範例是： &lt;P&gt;moons &lt;EM&gt;一律&lt;/e m&gt;提高，是在日落，增加或減少。&lt;/ P&gt;|  
...
|pass-through (通過)|BizTalk Server 連接埠的組態，在傳送埠上僅使用一個篩選條件運算式，將接收埠直接連接到傳送埠。 篩選條件運算式的格式應該為 BTS.ReceivePortName == "名稱或接收埠"。 在這種組態中，接收埠接收的任何訊息會直接傳送到傳送埠。 BizTalk 2004 中通過執行的意義與 BizTalk Server 2000 或 2002年中通過執行的意義不同。 在 BizTalk Server 2004 中通過執行訊息仍會在管線中處理，並可能在連接埠數目也會進行轉換。|  
...
|property promotion (屬性升級)|透過此特定訊息屬性都會寫入至訊息內容與訊息相關聯，做為屬性欄位機制。 寫入訊息內容屬性欄位會被視為升級的屬性的屬性 ( **IsPromoted**欄位的屬性設定為 **，則為 True**)，而且需要相關聯的屬性結構描述。  BizTalk Server 傳訊引擎進行文件追蹤的訊息路由，並進行協調流程中的評估會使用屬性欄位。 訊息的訊息內容會儲存在 MessageBox 資料庫的多工緩衝處理表格中，該訊息所在資料列的 imgContext 一欄。|  
