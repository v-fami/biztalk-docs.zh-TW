...
許多運算質都很易於使用。 這些稱為這裡以便區別中的運算質的基本運算質**進階**類別目錄。 基本運算質包含其餘的類別，例如轉換、 累計、 資料庫、 日期和時間、 邏輯、 數學、 科學，以及字串運算質。  
...
1. 將記錄或欄位節點從來源結構描述拖曳至顯示的格線頁中的基本運算質。  
    ...
    **-或者-**  
    ...
    在顯示的格線頁中，拖曳另一個運算質，位於往左邊，您要建立輸入的連結的基本運算質。  
...
2. 依需要重複步驟 1 來建立完整的基本運算質輸入連結集 (可能不是整個輸入參數集)。  
   ...
   > [!NOTE]
   >  少數運算質不需要任何輸入連結。 例如，**日期**，**時間**，和**日期和時間**中的運算質**日期和時間**運算質類別提供目前的日期，時間或日期和時間，分別在其執行個體訊息處理。 因此，它們不需要來源結構描述的任何輸入參數。  
   > 
   > [!NOTE]
   >  許多運算質的輸入參數的順序很重要，，對應的運算質參考主題中所示 (請參閱**運算質參考** [!INCLUDE[ui-guidance-developers-reference](../includes/ui-guidance-developers-reference.md)])。 您建立連結的順序將會設定運算質的輸入參數順序。 如需運算質屬性和指定的運算質輸入參數順序的詳細資訊，請參閱[編輯運算質屬性和輸入參數](../core/editing-functoid-properties-and-input-parameters.md)。 如需如何設定運算質的輸入的參數的詳細資訊，請參閱[如何設定運算質輸入參數](../core/how-to-configure-functoid-input-parameters.md)。  
   > 
   > [!NOTE]
