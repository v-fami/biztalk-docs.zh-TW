---
title: "BizTalk Server 如何接收 AS2 訊息 |Microsoft 文件"
ms.custom: 
ms.date: 06/08/2017
ms.prod: biztalk-server
ms.reviewer: 
ms.suite: 
ms.tgt_pltfrm: 
ms.topic: article
ms.assetid: ccfb4f6c-1fec-4af5-99e5-12634071dd3a
caps.latest.revision: "10"
author: MandiOhlinger
ms.author: mandia
manager: anneta
ms.openlocfilehash: 51f4b28397aa636b36ec3d063e441d9479e16cd2
ms.sourcegitcommit: cb908c540d8f1a692d01dc8f313e16cb4b4e696d
ms.translationtype: MT
ms.contentlocale: zh-TW
ms.lasthandoff: 09/20/2017
---
# <a name="how-biztalk-server-receives-as2-messages"></a>BizTalk Server 如何接收 AS2 訊息
本節說明 [!INCLUDE[btsBizTalkServerNoVersion](../includes/btsbiztalkservernoversion-md.md)] 中的 AS2 元件如何處理外寄 AS2 訊息。  
  
## <a name="in-this-section"></a>本節內容  
  
-   [AS2 接收元件](../core/as2-receive-components.md)  
  
-   [內送 AS2 訊息的協議解析](../core/agreement-resolution-for-incoming-as2-messages.md)  
  
-   [透過 AS2 內送 EDI 訊息的接收端處理](../core/receive-side-processing-of-an-incoming-edi-message-over-as2.md)  
  
-   [透過 AS2 內送的非 EDI 訊息的接收端處理](../core/receive-side-processing-of-an-incoming-non-edi-message-over-as2.md)  
  
-   [處理內送 AS2 訊息](../core/processing-an-incoming-as2-message.md)  
  
-   [產生外寄 MDN](../core/generating-an-outgoing-mdn.md)  
  
-   [處理內送 MDN](../core/processing-an-incoming-mdn.md)  
  
## <a name="see-also"></a>另請參閱  
 [AS2 方案架構](../core/as2-solution-architecture.md)