...
ms.locfileid: "36985767"
---
# <a name="setting-jd-edwards-enterpriseone-transport-properties"></a>設定 JD Edwards EnterpriseOne 傳輸屬性
JD Edwards EnterpriseOne 傳輸屬性是用於設計與執行階段。 在 [**傳輸屬性**] 對話方塊中，您設定的連接和認證參數特定為伺服器系統與您嘗試存取的物件。  
...
## <a name="enter-transport-properties"></a>輸入 傳輸屬性  

1. 在 BizTalk Server 管理主控台中，展開**BizTalk Server 管理**，展開**BizTalk 群組**，展開**應用程式**，然後展開 所需應用程式。  

2. 以滑鼠右鍵按一下**傳送埠**，指向**新增**，然後按一下**靜態單向傳送埠**。  

3. 中**傳送埠屬性** 對話方塊中，按一下內部**名稱**方塊，然後輸入此連接埠的名稱 (例如`JDEEnterpriseOneSend`)。  
