...
此逐步解說為您顯示如何根據下列範例訂單，使用 BizTalk 一般檔案結構描述精靈從文件執行個體建立一般檔案結構描述。 如需 BizTalk 一般檔案結構描述精靈的簡介，請參閱[如何使用 BizTalk 一般檔案結構描述精靈](../core/how-to-use-biztalk-flat-file-schema-wizard.md)。  
 ...
 訂單的每一行都會以標示為綠色的歸位換行字元 (CRLF) 結尾。 訂單會以顯示為紅色的 PO 標記開頭，後面加上日期。 兩個重複的固定位置欄位包含客戶資訊，並顯示為紫色。 註解 欄位中之後, 就沒有重複的欄位包含多個以逗號 （，） 分隔的記錄和資料值以管道 (& #124;) 分隔的項目標記以開始它會以藍色顯示。  
...
4.  當 BizTalk 一般檔案結構描述精靈開啟時，會出現歡迎頁面。   
    若要在未來執行精靈時，請跳過這個畫面，請選取 **不要再顯示此簡介頁面** 方塊。 按 **[下一步]** ，繼續進行。  
  
## <a name="selecting-purchase-order-instance"></a>選取訂單執行個體  
  
-   在 **一般檔案結構描述資訊** 畫面上，選取您的執行個體並輸入下列資訊。 當您完成之後時，按一下  **下一步** 以繼續。  
  
    -   **執行個體檔案︰** 按一下 **瀏覽** 按鈕找出一般檔案結構描述產生的。 瀏覽至包含您在逐步解說必要條件一節中建立的文字檔案的資料夾︰ 建立一般檔案結構描述的文件執行個體。  
    ...
    -   **計算以位元組為單位的位置︰** 核取此方塊，如果您想要位元組來計算位置資料欄位。 依預設值，位置資料欄位會以字元為單位計算。 在此逐步解說中，保留 **計算以位元組為單位的位置** 未核取的核取方塊。  
  
## <a name="select-purchase-order-data"></a>選取訂單資料  
  
-   在 **選取文件資料** 顯示畫面中，一般檔案的內容。 選取建立結構描述中，所需的資料，然後按一下 **下一步**。  
  
    > [!NOTE]
    >  如果您想要使用的整份文件執行個體，您可以按 **CTRL-A** 全部選取。  
  
    > [!NOTE]
    >  檢查 **長行換行** 方塊，如果您想要檢視整份文件執行個體內容包裝到精靈的 [編輯] 方塊。  
...
-   因為訂單的每一行的結尾是歸位換行字元 (CRLF) 中，選取 **依分隔符號**, ，然後按一下  **下一步** 上 **選取記錄格式** 畫面。  
...
-   在 **分隔記錄** 畫面上，輸入下列定義的結構描述的第一個層級，以及當您完成之後，按一下  **下一步**。  
    ...
    -   **逸出字元︰** 逸出字元是隱藏在它後面之字元的任何特殊意義的單一字元。 請參閱[逸出字元](../core/escape-characters.md)如需詳細資訊。 在此逐步解說中，將它留白。  
  
        > [!NOTE]
        >  當使用 **\\**, ，**{，** 和 **}** 的 **子分隔符號** 或 **逸出字元**, ，必須使用反斜線。 例如， **\\\\,，** 和 **\\{**。  
  
    -   檢查 **記錄具有標記識別項** 方塊，然後輸入 **PO** 中 **標記**。 在多個記錄檔中， **PO** 也用來識別每個個別的記錄。 按 **[下一步]** ，繼續進行。  
...
2.  對下列項目重複步驟 a 到 c。 當您完成之後時，按一下  **下一步**。  
    ...
    > [!NOTE]
    >  按一下之後 **下一步** 上 **子項目**  畫面中，將無法按一下 **回** 重新定義，或對子項目進行任何變更。 您必須關閉精靈再加以重新啟動，才能定義一般檔案結構描述。  
...
1.  因為我們已經定義 **客戶** 項目做為 **重複記錄** 型別和 **項目** 項目做為 **記錄** 型別，BizTalk 一般檔案結構描述精靈現在會繼續進一步定義這兩個元素。 在 **結構描述檢視** 畫面上，選取 **客戶**, ，然後按一下  **下一步** 以繼續。  
  
2.  若要使用客戶記錄，您必須選取表示該項目的資料。 因為它是重複記錄，所以任一行都可以用來定義記錄。 選取客戶資料的第一行，然後按一下 **下一步** 以繼續。  
...
5.  在 **子項目**  頁面上，指定子元素的屬性。 選取第一個資料列，然後輸入 **國家/地區** 為 **項目名稱**。 保留其他欄位的預設值。 輸入下列其他屬性的值 **項目名稱**:  
...
2.  在 **選取文件資料** 頁面上，選取整行開頭的項目，然後按一下  **下一步** 定義其子項目。  
  
3.  在 **選取記錄格式** 頁面上，選取 **依分隔符號**, ，然後按一下  **下一步**。  
  
4.  在 [項目] 記錄中，會使用逗號來分隔個別的項目。 因此，在 **分隔記錄** 頁面上，輸入下列命令以定義項目記錄。 當您完成之後時，按一下  **下一步**。  
...
5.  使用來自值 **分隔記錄** 頁面上，精靈會識別兩個子元素。 因為其中一個是重複記錄時，選取第一個項目，並輸入 **項目** for 項目名稱，然後選取 **重複記錄** 從下拉式清單選取項目清單 **項目型別**。 請保留欄位中的其他預設值。 選取第二個資料列，然後選取 **忽略** 從 **項目型別** 清單。 當您按一下 **下一步**, ，結構描述中建立項目記錄的下一個層級。 您可以繼續定義訂單結構描述的最後一個部分。  
  
6.  選取 **項目** 然後按一下  **下一步** 繼續 **結構描述檢視** 頁面。  
  
7.  在 **選取文件資料** 頁面上，選取 **ITEM872 AA & #124;割草機 #124; 1 & #124; 148.95 & #124;確認這是電動**。 按 **[下一步]** ，繼續進行。  
  
8.  在 **選取記錄格式** 頁面上，選取 **依分隔符號** 選項，因為個別的項目以直立線符號 (& #124;) 分隔。  
  
9. 在 **分隔記錄** 頁面上，輸入下列命令以定義項目記錄。 當您完成之後時，按一下  **下一步**。  
  
    -   選取 **& #124;** 從 **子分隔符號** 下拉式選取清單。  
  
    -   保留 **逸出字元** 文字方塊留白。  
  
10. 在 **子項目** 頁面上，精靈已識別五個子項目。 選取第一個資料列，並輸入 **productCode** 的 **項目名稱**。 請保留其他欄位中的預設值。 其餘的 **項目名稱屬性**, ，輸入下列命令︰  
...
2.  在 **屬性頁**, ，請確定 **輸入執行個體檔案名稱** 欄位指向您在逐步解說必要條件一節中建立的 PO 執行個體的位置︰ 建立一般檔案結構描述的文件執行個體。 按一下  **確定** 以關閉對話方塊。  
