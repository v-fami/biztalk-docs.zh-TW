...
1.  按一下**啟動**，指向**所有程式**，指向 **系統管理工具**，然後按一下  **SharePoint 管理中心。**  
  
2.  在下**虛擬伺服器設定**，按一下 **建立頂層網站**。  
  
3.  在下**虛擬伺服器清單**，選取您在安裝 Windows SharePoint Services 配接器的網站。 例如， `Default Web Site`。  
...
8.  在**成功建立頂層網站**頁面上，按一下您剛建立新最上層的網站。 例如， `http://<server_name>/sites/WSSAdapterWalkthrough`。  
  
9. 選取**小組網站**範本，然後再按一下清單中的範本**確定**。 這樣會開啟「小組網站」首頁。  
...
5.  在**文件範本**區段的**文件範本**下拉式清單中，選取`None`。  
...
3.  在**名稱和描述**區段的**名稱 欄位**，型別`Destination`。  
...
3.  在**名稱**和 描述 部分中**名稱 欄位**，型別`Archive`。  
...
4.  在**選取使用者、 電腦或群組對話方塊**下**輸入物件名稱來選取**，輸入您設定 BizTalk Server 主控件執行個體來執行，然後按一下帳戶名稱**確定**。  
  
5.  在主控台樹狀目錄中，依序展開**服務和應用程式**，然後按一下 **服務**。  
  
6.  以滑鼠右鍵按一下**BizTalk Service BizTalk Group: < BizTalk_Host_Name >**，然後按一下 **重新啟動**。  
...
5.  在**步驟 1： 選擇使用者**，輸入帳戶的名稱，[!INCLUDE[btsBizTalkServerNoVersion](../includes/btsbiztalkservernoversion-md.md)]下執行主控件執行個體。  
  
6.  在**步驟 2： 選擇網站群組**，選取**讀取器**和**參與者**核取方塊。  
  
7.  按一下 **[下一步]**。  
  
8.  清除**傳送下列電子郵件，讓這些使用者現在已被新增**核取方塊，然後**完成**。  
...
1.  按一下**啟動**，**所有程式**， [!INCLUDE[btsBizTalkServerStartMenuItemui](../includes/btsbiztalkserverstartmenuitemui-md.md)]，然後按一下 [ **BizTalk Server 管理]。**  
  
2.  展開[!INCLUDE[btsBizTalkServerAdminConsoleui](../includes/btsbiztalkserveradminconsoleui-md.md)]，依序展開**BizTalk 群組**，依序展開**應用程式**，依序展開**BizTalk Application 1**，以滑鼠右鍵按一下**接收埠**，按一下 **新增**，然後按一下 **單向接收埠...**  
...
1.  在**BizTalk 管理主控台**，以滑鼠右鍵按一下**接收位置** 節點，按一下 **新增**，然後按一下 **單向接收位置**.  
...
6.  在**配接器 Web 服務連接埠**屬性中，輸入已安裝 Windows SharePoint Services 配接器 Web 服務的虛擬伺服器的通訊埠編號。 預設號碼是連接埠 80。  
...
10. 型別`Source`如**來源文件庫**屬性。  
  
11. 按一下 **[確定]**。  
  
12. 在**接收位置屬性**對話方塊中，選取`BizTalkServerApplication`為**接收處理常式**。  
...
1.  在**BizTalk 管理主控台**，以滑鼠右鍵按一下**傳送埠** 節點，按一下 **新增**，然後按一下 **靜態單向傳送埠**.  
...
5.  在**配接器 Web 服務連接埠**屬性中，輸入已安裝 Windows SharePoint Services 配接器 Web 服務的虛擬伺服器的通訊埠編號。 預設號碼是連接埠 80。  
  
6.  在中輸入`Destination`如**目的地資料夾**屬性。  
  
7.  在中輸入`PurchaseOrder1-%MessageID%.xml`如**Filename**屬性。  
  
8.  設定**覆寫**屬性`Yes`。  
  
9. 在 SharePoint 網站 URL 中的型別**SharePoint 網站 Url**屬性。 例如， `http://<server_name>/sites/WSSAdapterWalkthrough`。  
  
10. 設定**Microsoft Office 整合**屬性`No`。  
  
11. 按一下 **[確定]**。  
  
12. 在**傳送埠屬性 對話方塊**，請在**傳送處理常式**下拉式清單中，選取`BizTalkServerApplication`。  
...
## <a name="sending-a-message-through-the-system"></a>透過系統傳送訊息  
 您會在此程序建立 XML 文件並上載至 Windows SharePoint Services 網站。 Windows SharePoint Services 配接器會取得該訊息、封存至「封存」文件庫，然後傳送至「目的地」文件庫。 此程序示範如何在文件流經從 Sharepoint 網站， [!INCLUDE[btsBizTalkServerNoVersion](../includes/btsbiztalkservernoversion-md.md)]，以及 Sharepoint Services 網站使用 Windows Sharepoint Services 配接器。  
...
1.  按一下**啟動**，指向 **所有程式**，指向 **附屬應用程式**，然後按一下  **記事本。**  
...
## <a name="summary"></a>摘要  
 在本逐步解說中，您已經瞭解如何設定 Windows SharePoint Services 和[!INCLUDE[btsBizTalkServerNoVersion](../includes/btsbiztalkservernoversion-md.md)]讓您可以傳送和接收訊息，使用 Windows SharePoint Services 配接器和以內容為基礎的路由 (CBR)。  
