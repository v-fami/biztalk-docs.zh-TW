...
這個逐步解說會將延續[逐步解說： 模組 2-整合 Office 與 Windows SharePoint Services 配接器](../core/walkthrough-module-2--integrate-office-with-the-sharepoint-adapter-in-biztalk.md)，並說明如何存取內送訊息在 Windows SharePoint Services 內容屬性執行階段，然後決定該屬性，使用協調流程中的動態連接埠為基礎的訊息的目的地。 如需 Windows SharePoint Services 配接器的簡介，請參閱[什麼是 Windows SharePoint Services 配接器？](../core/what-is-the-windows-sharepoint-services-adapter.md)。  
  
## <a name="prerequisites"></a>先決條件  
 下列是執行本主題所述程序的必要條件：  
  
- 您必須擁有完整安裝上執行的 BizTalk Server 的單一伺服器部署[!INCLUDE[btsWinSvr2k8](../includes/btswinsvr2k8-md.md)]或[!INCLUDE[btsWinSvr2k8R2](../includes/btswinsvr2k8r2-md.md)]。  
  
- 您必須完成下列逐步解說：[逐步解說： 模組 1-傳送和接收 Windows SharePoint Services 配接器的訊息](../core/walkthrough-module-1--send-and-receive-messages-with-the-sharepoint-adapter.md)和[逐步解說： 模組 2-整合 Office 與 WindowsSharePoint Services 配接器](../core/walkthrough-module-2--integrate-office-with-the-sharepoint-adapter-in-biztalk.md)  
...
## <a name="modify-the-biztalk-project"></a>修改 BizTalk 專案  
 您可以在此程序修改 PurchaseOrder 結構描述，從[逐步解說： 模組 2-整合 Office 與 Windows SharePoint Services 配接器](../core/walkthrough-module-2--integrate-office-with-the-sharepoint-adapter-in-biztalk.md)。 本程序說明如何升級結構描述屬性，以便能在 BizTalk 協調流程中輕鬆存取。  
  
#### <a name="modify-the-purchaseorderxsd-schema"></a>修改 PurchaseOrder.xsd 結構描述  
  
1. 開始**Microsoft Visual Studio**。  
  
2. 按一下 **檔案**，按一下**開放**，然後按一下**專案/方案**。  
  
3. 瀏覽至`OrderProcess.sln`檔案，然後再按一下**開啟**。  
  
4. 在 **方案總管**，以滑鼠右鍵按一下`OrderProcessSchema.xsd`檔案，然後再按一下**開啟**。  
  
5. 在  **BizTalk 編輯器**，展開  `PurchaseOrder`。  
...
1.  中**方案總管**，以滑鼠右鍵按一下`OrderProcess`專案中，按一下 **新增**，然後按一下**新項目**。  
  
2.  底下**分類**，選取**協調流程檔案**。  
  
3.  底下**範本**，選取**BizTalk 協調流程**。  
  
4.  型別`MyCompanyOrderProcessing`中**名稱**欄位，，然後按一下**新增**。  
  
## <a name="create-receive-information"></a>建立接收資訊  
 在此程序中，您要為協調流程建立新訊息、接收埠以及接收圖形。 此程序說明如何設定協調流程接收的訊息[!INCLUDE[btsBizTalkServerNoVersion](../includes/btsbiztalkservernoversion-md.md)]。  
...
4.  在 **訊息類型**屬性，展開**結構描述**，然後選取 `OrderProcess.OrderProcessSchema`結構描述。  
  
#### <a name="add-a-receive-port-to-the-orchestration"></a>新增接收埠至協調流程  
  
1.  底下**BizTalk 協調流程**在 工具箱 拖曳**連接埠**至連接埠介面 圖形。 將會啟動 [連接埠組態精靈]。  
...
6.  在 **連接埠繫結螢幕**，保留預設值，然後按一下 **下一步**。  
  
7.  按一下 **[完成]**。  
  
8.  在 **協調流程檢視**下方**連接埠類型**，展開 `PurchaseOrderPT`連接埠類型。  
...
1.  底下**BizTalk 協調流程**在 [工具箱] 拖曳**接收**圖形至協調流程。  
  
2.  接收 圖形中，以滑鼠右鍵按一下，然後按一下**屬性 視窗**。  
  
3.  設定**Activate**屬性設`True`。  
...
6.  選取  `ReceivePurchaseOrder.PurchaseOrderOperation.Request` for**作業**屬性。 這會將繫結協調流程設計師中的 「 接收 」 圖形的連接埠。  
...
5.  底下**可用的連接埠類型**，選取`OrderProcess.PurchaseOrderPT`，然後按一下**下一步**。  
  
6.  上**連接埠繫結螢幕**下方**連接埠通訊方向**，選取`I'll always be sending messages on this port`，然後按一下**下一步**。  
  
7.  按一下 **[完成]**。  
  
#### <a name="add-a-send-shape-to-the-orchestration"></a>新增 [傳送] 圖形至協調流程  
  
1.  底下**BizTalk 協調流程**在 [工具箱] 拖曳**傳送**圖形至協調流程設計工具。 將它放在 `Receive_PO` 接收圖形下方。  
  
2.  以滑鼠右鍵按一下 傳送 圖形，然後按一下**屬性 視窗**。  
  
3.  型別`Send_PO`中**名稱**欄位。  
  
4.  選取  `Message_PO` for**訊息**屬性。  
  
5.  選取  `SendPurchaseOrder.PurchaseOrderOperation.Request` for**作業**屬性。 這會結合連接埠至 [協調流程設計師] 中的 [傳送] 圖形。  
  
#### <a name="add-a-decide-shape-to-the-orchestration"></a>新增 [決定] 圖形至協調流程  
  
1.  底下**BizTalk 協調流程**在 [工具箱] 拖曳**決定**圖形至協調流程設計工具。 將它放在 `Send_PO` 傳送圖形下方。  
  
2.  [決定] 圖形中，以滑鼠右鍵按一下，然後按一下 [**屬性] 視窗。**  
  
3.  型別`NeedsApproval`中**名稱**欄位。  
  
4.  在 [協調流程設計師 」 中按一下**Rule_1**決定] 圖形上。  
  
5.  在 屬性 Windows 中，鍵入`ApprovalRequired`for**名稱**屬性。  
  
6.  按一下 **運算式**屬性欄位，，然後按一下 省略符號 (**...**) 按鈕。  
...
6.  在上**連接埠繫結螢幕**下方**連接埠通訊方向**，選取`I'll always be sending messages on this port`。  
  
7.  底下**連接埠繫結**，選取`Dynamic`，然後按一下**下一步**。  
  
8.  按一下 **[完成]**。  
  
#### <a name="add-a-send-shape-to-the-decide-shape"></a>新增 [傳送] 圖形至 [決定] 圖形  
  
1.  底下**BizTalk 協調流程**在 [工具箱] 拖曳**傳送**圖形至協調流程設計工具。 將圖形放置在 `ApprovalRequired` 圖形的下方。  
  
2.  以滑鼠右鍵按一下 傳送 圖形，然後按一下**屬性 視窗**  
  
3.  型別`CreateApprovalTask`中**名稱**欄位。  
  
4.  選取  `Message_Task` for**訊息**屬性。  
  
5.  選取  `SendToTasksList.PurchaseOrderOperation.Request` for**作業**屬性。 這會結合連接埠至 [協調流程設計師] 中的 [傳送] 圖形。  
...
1.  底下**BizTalk 協調流程**在 [工具箱] 拖曳**運算式**圖形之前`CreateApprovalTask`傳送圖形。  
  
2.  以滑鼠右鍵按一下 運算式 圖形，然後按一下**屬性 視窗。**  
  
3.  型別`SetPortDestination`中**名稱**欄位。  
  
4.  按一下 **運算式**屬性欄位，，然後按一下 省略符號 (**...**) 按鈕。  
...
1.  底下**BizTalk 協調流程**在 [工具箱] 拖曳**建構訊息**圖形之前`SetPortDestination`運算式 」 圖形。  
  
2.  [建構訊息] 圖形中，以滑鼠右鍵按一下，然後按一下 [**屬性] 視窗。**  
  
3.  型別`ConstructTaskMessage`中**名稱**欄位。  
  
4.  選取  `Message_Task` for**建構的訊息**屬性。  
  
5.  底下**BizTalk 協調流程**在 [工具箱] 拖曳**訊息指派**塑造成`ConstructTaskMessage`**建構訊息**圖形。  
  
6.  在 [**屬性] 視窗**，型別`InitTaskMessage`中**名稱**欄位。  
  
7.  按一下 **運算式**屬性欄位，，然後按一下 省略符號 (**...**) 按鈕。  
...
## <a name="build-the-biztalk-project"></a>建置 BizTalk 專案  
 在此程序中，您要建置和部署 BizTalk 專案。 此步驟，才能建立和部署組件，[!INCLUDE[btsBizTalkServerNoVersion](../includes/btsbiztalkservernoversion-md.md)]在執行階段使用。  
...
2. 依序展開**Microsoft** [!INCLUDE[btsBizTalkServer2006r3ui](../includes/btsbiztalkserver2006r3ui-md.md)] **管理嵌入式管理單元**，依序展開**BizTalk 群組**，依序展開**應用程式**，依序展開  **BizTalk Application 1**，然後按一下**接收位置**節點。  
...
4. 底下**主機**，選取`BizTalkServerApplication`中**主機**欄位。  
  
5. 底下**繫結**，選取`FromSource`如`ReceivePurchaseOrder`輸入邏輯連接埠。  
  
6. 底下**繫結**，選取`SendToDestination`如`SendPurchaseOrder`輸出邏輯連接埠。  
...
3.  按一下 `PurchaseOrder`檔案，以顯示**檔案下載**對話方塊，然後再按一下**開啟**。 InfoPath 將載入此表單。  
  
4.  在 **採購單識別碼**欄位中，輸入`1003`。  
  
5.  在 **付款人**欄位中，輸入`John Doe`。  
...
9. 在 [**另存新檔**] 對話方塊中，輸入`http://<server_name>/sites/WSSAdapterWalkthrough/Source`中**檔案名稱**欄位，然後再按 ENTER 鍵。  
  
10. 型別`PurchaseOrder3.xml`中**檔案名稱**欄位，，然後按一下**儲存**。  
  
11. 關閉 InfoPath。  
  
12. 在網頁瀏覽器中，按一下**文件，並列出**。  
