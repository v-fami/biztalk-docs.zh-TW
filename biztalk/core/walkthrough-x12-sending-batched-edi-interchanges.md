...
9. 傳送管線其他屬性套用至批次交換的信封，然後將交換傳送至目的合作對象， **Party B**。  

    > [!NOTE]
    >  如需詳細資訊，請參閱 <<c0> [ 組合批次 EDI 交換](../core/assembling-a-batched-edi-interchange.md)。  
...
- 更新中的 SQL 配接器的輪詢間隔**BatchControlMessageReccvLoc**接收位置，以便當您按一下 立即啟動批次處理協調流程**開始**按鈕傳送會啟動批次處理協調流程執行個體的 「 控制 」 訊息。  
...
1. 在 [!INCLUDE[btsBizTalkServerNoVersion](../includes/btsbiztalkservernoversion-md.md)]管理主控台中，開啟**BizTalk 群組**，**應用程式**， **BizTalk EDI 應用程式**節點，和**接收位置**節點。 底下**接收位置**節點，以滑鼠右鍵按一下**BatchControlMessageRecvLoc**，然後按一下**屬性**。  

2. 針對 SQL 傳輸類型，按一下**設定**。  

3. 在  **SQL 傳輸屬性**對話方塊方塊中，以較小的值設定的輪詢間隔。 例如，您可以變更**輪詢的度量單位**要**秒**，而非分鐘，若要變更的輪詢間隔為 5 秒。  
...
6. 輸入所需的資料夾**接收資料夾**，和遮罩**檔案遮罩**，這類 **\*.txt**。  
...
7. 在上執行下列工作**party a-> partyb**  索引標籤。  
   ...
   4. 上**批次組態**下方的頁面上**交換設定**區段中，執行下列動作：  

      1.  按一下 **新的批次**。  

      2.  底下**識別**區段中，如**批次名稱**文字、 輸入`Batch1`。  

      3.  底下**篩選條件**區段中，按一下**篩選** 按鈕，然後在**批次篩選條件**對話方塊方塊中，執行下列動作：  
      ...
      4.  底下**發行**區段中，選取**設定中的交易數目上限**選項，從下拉式清單中選取**交換**，並在文字方塊中輸入的數目將批次處理的交換。 在此解決方案中，您將會批次處理兩個交換，因此在文字方塊中，輸入`2`。  

      5.  底下**Activation**區段中，選取**立即啟動**。  

   5. 在上**信封**下方的頁面上**交易集設定**區段中，輸入在格線的第一行中的所有資料行的值。  
...
5. 輸入所需的資料夾**目的地資料夾**，以及**檔名**，這類 **%MessageID%.txt**。  

6. 按一下 [確定] 。  

7. 在 **傳送管線**，選取**EdiSend**。  

8. 在主控台樹狀目錄中，選取**篩選器**。 在 **篩選器**頁面上，執行下列動作：  

   1.  在方格的第一行中，選取**EDI。DestinationPartyName** for**屬性**， **==** 如**運算子**，您選取的合作對象傳送來的批次的名稱**值**，並**並**如**分組**。  

   2.  在第二行中，選取**EDI。ToBeBatched**針對**屬性**， **==** 如**運算子**，並**False**的**值**，並**並**如**分組**。  

   3.  在第三行中，選取  **EDI。BatchName** for**屬性**， **==** 如**運算子**，和批次名稱**值**。  
...
2.  在 **協議屬性**對話方塊的  **party a-> Party**索引標籤上，按一下 **傳送埠**的左窗格中。  

3.  上**傳送埠**下方的頁面上**交換設定**區段中，將您稍早建立的傳送埠產生關聯。 在 **傳送埠**格線下方**名稱** 欄中，按一下空白儲存格，並從下拉式清單中，選取 傳送埠。  
