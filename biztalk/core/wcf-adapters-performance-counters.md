...
>  WCF 傳送埠無法使用 WCF 效能計數器。  
 ...
 對於內含式 WCF 配接器，您可以透過 BTSNTSvc.exe.config 檔案啟用效能計數器。 若為外掛式 WCF 配接器，則可以修改 Web.config 檔案來啟用效能計數器。 如需 WCF 效能計數器的詳細資訊，請參閱 < WCF 效能計數器 」 在[http://go.microsoft.com/fwlink/?LinkID=87245](http://go.microsoft.com/fwlink/?LinkID=87245)。  
...
-   **ServiceOnly**： 只有**ServiceModelService**啟用類別的計數器。  
...
```  
 ...
 會為每個接收位置建立一個效能計數器執行個體。 在前述的模式中，WCF 服務合約的名稱會代表 WCF 配接器選擇透過接收位置接收訊息的服務合約。 如需有關 WCF 配接器從可用的 WCF 所選擇的服務合約的服務合約，請參閱**WCF 配接器服務合約參考** [!INCLUDE[ui-guidance-developers-reference](../includes/ui-guidance-developers-reference.md)]。  
...
```  
 ...
 在前述的模式中，WCF 服務合約的名稱會代表 WCF 配接器選擇透過接收位置接收訊息的服務合約。 **biztalksubmit**是服務合約中宣告的作業名稱，並造成執行階段在中繼資料建立 WSDL 作業。  
...
```  
 ...
 此效能計數器執行個體代表以非同步的方式處理透過接收位置內送之訊息的作業。 這個執行個體的作業名稱部分可以是**twowaymethod**或**onewaymethod**根據 WCF 配接器接收位置中使用的類型。 如果您使用 Wcf-netmsmq 配接器執行個體具有**onewaymethod**建立作業名稱。 為其他配接器， **twowaymethod**用於作業名稱部分。