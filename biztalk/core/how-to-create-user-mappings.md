...
```  
 ...
 如果使用者帳戶已變更，您必須使用此命令為新的使用者帳戶建立對應。 您也必須移除舊的使用者對應。 如需移除對應的詳細資訊，請參閱[如何刪除使用者對應](../core/how-to-delete-user-mappings.md)。  
  
 建立使用者對應之後，您必須先啟用，才能在 SSO 系統中使用這個對應。 如需詳細資訊，請參閱[如何啟用使用者對應](../core/how-to-enable-a-user-mapping.md)。  
