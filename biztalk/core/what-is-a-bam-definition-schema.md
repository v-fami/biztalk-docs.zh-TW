---
title: 何謂 BAM 定義結構描述？ | Microsoft Docs
ms.custom: ''
ms.date: 06/08/2017
...
ms.locfileid: "25975796"
---
# <a name="what-is-a-bam-definition-schema"></a>何謂 BAM 定義結構描述？
BAM 定義結構描述定義了商務分析師所建立之觀察模型的結構。  
  
 結構描述則定義了 BAM 定義 XML 文件 (包含文件的項目和子項目) 的結構。 例如，根項目是 BAM 定義，而 BAM 定義中包含下列項目：  
...
> [!NOTE]
>  如果 BAM 定義 XML 包含兩個檢視，且使用者僅擁有的權限的其中一個檢視，在呼叫**getviewdefinition 後**，使用者將取得這兩個檢視的定義。  
  
 結構描述也定義了文件條件約束。 例如，條件約束規定某些項目只能參考其他特定項目。  
...
> [!NOTE]
>  當您匯出 BAM 定義為 XML 時，您無法匯出 BAM 定義為 localhost。 例如，嘗試匯出 BAM 定義，以`\\localhost\C$\definition1.xml`會導致錯誤。  
>   
>  同樣地，您無法從 localhost 匯入 BAM 定義。  
  
> [!NOTE]
>  如果變更了活動或檢視，您就必須移除並再重新部署 BAM 定義。  
