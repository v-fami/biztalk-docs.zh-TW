...
|元件|0|  
|符號名稱|0|  
|訊息文字|無法建立繫結，因為未指定繫結類型。 指定繫結型別，例如"basicHttpBinding"、"wsHttpBinding，"或"customBinding|  
  
## <a name="explanation"></a>說明  
 未設定 BindingType 屬性程式碼中設定 Wcf-custom 或 Wcf-customisolated 傳輸之後。 或者，則問題可能是其他程式碼路徑中。 繫結設定的使用者介面中，您必須有值。 檢閱您的組態並確定已從接收位置屬性 區域中的下拉式清單中選擇繫結型別。  
...
9. 請確認指定的值是在**繫結的型別**清單。  
  
 如需有關如何設定繫結的詳細資訊，請參閱下列資源[!INCLUDE[btsBizTalkServerNoVersion](../includes/btsbiztalkservernoversion-md.md)]協助：  
