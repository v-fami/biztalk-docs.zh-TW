...
若要將網站或 Web 服務的虛擬目錄加入至 BizTalk 應用程式，您使用**AddResource**命令，並指定**system.biztalk: webdirectory**型別參數。 執行此命令會將虛擬目錄新增至 BizTalk 管理資料庫。 虛擬目錄也會顯示於 BizTalk Server 管理主控台，列在加入該虛擬目錄的應用程式的 [資源] 資料夾中。 此外，虛擬目錄會列出當您使用[ListApp 命令](../core/listapp-command.md)。  
...
|**/ 輸入**(或 **/T**，請參閱 < 備註 >)|是|**System.biztalk: webdirectory** （此值不區分大小寫）。|  
|**/ 覆寫**(或 **/O**，請參閱 < 備註 >)|否|更新現有虛擬目錄的選項。 若未指定此選項，且應用程式中現有的虛擬目錄與所加入的虛擬目錄同名，AddResource 作業將會失敗。|  
|**/ 來源**(或 **/S**，請參閱 < 備註 >)|是|來源虛擬目錄的完整 URI。<br /><br /> 範例:<br /><br /> http://MyHost:80/MyPath/MyVirtualDirectory|  
