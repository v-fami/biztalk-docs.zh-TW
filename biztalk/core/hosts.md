...
- 您在中建立第一個 「 內含式主控件[!INCLUDE[btsBizTalkServerNoVersion](../includes/btsbiztalkservernoversion-md.md)]部署**預設主控件**和就無法予以刪除。 「BizTalk 訊息佇列」配接器為靜態處理常式組態使用預設主控件。 加入配接器可以為預設主控件自動建立接收埠和傳送埠。  
...
-   外掛式主控件的狀態總是**狀態無法使用**。 BizTalk Server 不會存取外部程序的狀態資訊。  
