...
1.  按一下**啟動**，指向 **所有程式**，按一下  [!INCLUDE[btsBizTalkServerStartMenuItemui](../includes/btsbiztalkserverstartmenuitemui-md.md)]，然後按一下 **追蹤設定檔編輯器**。  
...
3.  在**工具**功能表上，按一下 **套用追蹤設定檔**.btt 檔套用到您已經將設定管理資料庫**設定管理資料庫**上的功能表項目**工具**功能表。 透過整合測試驗證結果。  
...
> [!IMPORTANT]
>  從命令列套用追蹤設定檔時，bttdeploy 會將設定檔套用到您在執行組態精靈時所指定的 BizTalk 管理資料庫。 如果您在追蹤設定檔編輯器選項「設定管理資料庫」中指定不同的資料庫設定，則便會使用該設定。 如果您指定管理伺服器和資料庫做為 bttdeploy 命令列選項的一部分，則該命令列選項會覆寫所有其他部分。 如需使用 bttdeploy 的詳細資訊，請參閱[追蹤設定檔部署公用程式](../core/tracking-profile-deployment-utility.md)。  
...
3.  在**工具**功能表上，按一下 **移除追蹤設定檔**從 BizTalk 管理資料庫中的.btt 檔為基礎的追蹤設定檔。  
