...
## <a name="batch-status-reporting-data-may-not-be-updated-if-the-batch-orchestration-is-stopped-outside-of-the-partner-agreement-manager"></a>如果在夥伴協議管理員外部停止了批次協調流程，批次狀態報告資料可能不會更新。  
 批次協調流程執行個體可以停用批次 頁面的 EDI 屬性 對話方塊中透過合作對象。 如果您以這種方式停用批次協調流程執行個體，BizTalk Server 將會更新該批次的狀態報告資料。 不過，如果您在另一種方法停止批次協調流程，例如，停止從查詢頁面之一的協調流程，在 [群組概觀] 頁面上的[!INCLUDE[btsBizTalkServerNoVersion](../includes/btsbiztalkservernoversion-md.md)]管理主控台中，狀態報告資料可能不會更新，以及您可能會得到使用已過時的批次狀態報告。 例如，即使批次協調流程已經停用了，狀態報告可能還是會指出批次仍在作用中。  
 ...
 **解決方式**  
 ...
 重新啟動 BizTalk 服務 (在 [電腦管理] 對話方塊中)。 如果在您的解決方案中使用 AS2EdiReceive 管線或 AS2EdiSend 管線，重新啟動 IIS Admin service (使用*iisreset*命令)，以及。  
...
## <a name="only-one-edi-interchange-will-be-correlated-to-an-as2-message-in-the-status-report-ui"></a>狀態報告 UI 中只有一個 EDI 交換與 AS2 訊息相互關聯  
 如果 AS2 訊息包含多個 EDI 交換，而您嘗試顯示 AS2 訊息中多個 EDI 交換的狀態，則只有最後一個 EDI 交換會在 [交換/通知狀態] 報告中顯示。 颾魤 ㄛ **EDI 交換控制編號**AS2/MDN 狀態 報告中的欄位只會顯示最後一個交換控制編號。 (交換控制編號可使 AS2 訊息與其 EDI 交換內容相互關聯)。  
...
## <a name="status-reporting-will-not-work-after-an-upgrade-if-the-bam-tools-are-not-configured"></a>如果沒有設定 BAM 工具，則在升級之後，狀態報告將無法運作  
 ...
 如果您想要使用狀態報告在您升級至 BizTalk Server 之後，，請確定您在執行升級之前，已設定 BAM 工具。  
  
 如果狀態報告在您執行升級之後無法運作，請在升級記錄中判斷 BAM 工具是否在升級之前設定。 如果沒有，您可以在 設定 BAM 工具，並接著部署 EdiStatusReportingActivityDefs.xml 檔案中包含的 BusinessMessageJournal BAM 活動*\<磁碟機\>*: \Program Files\MicrosoftBizTalk Server。  
...
## <a name="disabling-transaction-set-storage-affects-an-activated-batch-but-enabling-storage-does-not"></a>停用交易集儲存區會影響已啟動的批次，但是啟用儲存區不會  
 如果您在啟動批次處理協調流程的執行個體時停用了交易集的儲存區，這項變更將會立即生效。 BizTalk Server 會在啟用儲存區時儲存批次的交易集，但是在停用儲存區之後就不會儲存交易集。 您清除停用交易集的儲存的體 」 儲存交易集/內容報告 屬性中的 EDI 屬性 對話方塊中的 一般 窗格。  
...
## <a name="enabling-as2-status-reporting-and-send-port-body-tracking-simultaneously-may-cause-an-error"></a>同時啟用 AS2 狀態報告和傳送埠內文追蹤可能會造成錯誤  
 如果您啟用 AS2 狀態報告和傳送埠內文追蹤，同時下列錯誤可能會顯示在事件檢視器中: 「 傳訊引擎會發生錯誤時刪除一或多個訊息 」。 當傳送埠是具有 AS2Send 和 AS2Receive 管線的靜態請求-回應 AS2 傳送埠時，就會發生這種情況。 發生的時機則包括啟用下列屬性時：  
  
- 在 [AS2 屬性] 對話方塊中的 [一般] 窗格中 [啟動 AS2 報告] 屬性。  
...
## <a name="edi-and-as2-message-context-properties-are-not-available-after-upgrading-to-biztalk-2009"></a>EDI 和 AS2 訊息內容屬性都未升級至 BizTalk 2009 之後  
 升級到 BizTalk Server 之後，任何內容屬性會不顯示在狀態報告任何 EDI 或 AS2 接收的訊息升級發生之前。  升級將會正確顯示的內容屬性後收到的訊息。  
  
 EDI 和 AS2 內容屬性集合已不會儲存為舊版 BizTalk Server 中訊息的一部分，並在升級後就無法使用。 在之後升級至 BizTalk Server 中，AS2 內容屬性會儲存為訊息的一部分，不過 EDI 內容屬性不是。  
  
## <a name="interchange-date-for-received-documents-may-display-the-wrong-year-in-status-reports"></a>接收文件的交換日期可能會顯示在狀態報告中錯誤的年度  
 如果接收的文件以 YYMMDD 格式指定日期，則 BizTalk Server 會使用下列邏輯來判斷的年份值：  
  
- 如果大於或等於 75 YY，年將會顯示為 19YY。  
  
- 如果 YY 小於 75，年將會顯示為 20YY。  
