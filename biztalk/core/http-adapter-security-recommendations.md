...
您可以使用 HTTP 配接器透過「超文字傳輸通訊協定」(HTTP) 在 [!INCLUDE[btsBizTalkServerNoVersion](../includes/btsbiztalkservernoversion-md.md)] 與應用程式之間交換資訊。 應用程式可以藉由傳送 HTTP POST 或 HTTP GET 要求到指定的 HTTP URL 來傳送訊息到伺服器。 如需有關 HTTP 配接器的詳細資訊，請參閱 < [HTTP 配接器](../core/http-adapter.md)。 建議您使用下列指導方針在環境中部署 HTTP 配接器並保護其安全：  
  
- 請確定您為 HTTP 配接器設定 Internet Information Services (IIS) 設定。 如需詳細資訊，請參閱 <<c0> [ 針對 HTTP 接收位置設定 IIS 如何](../core/how-to-configure-iis-for-an-http-receive-location.md)。  
  
- 若您使用 7.0，請確定您遵循 IIS 7.0 中對於設定應用程式隔離的建議。  
  
- 當您使用基本驗證，或您未使用訊息層級加密，建議使用安全通訊端層 (SSL) 來接收和傳送訊息，以確保未經授權的人員無法而讓其竊取使用者認證。  
