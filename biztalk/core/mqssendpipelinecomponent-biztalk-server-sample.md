...
1.  建立一個名為 **temp** 在 C:\ 磁碟機如果不存在。  
  
2.  建立資料夾之下的 **C:\temp** 名為 **Pickup3**。  
...
4.  以滑鼠右鍵按一下 方案總管中，按一下  **建置**。 這會將專案編譯成 dll 位於 **SetMQSeriesHeaderPropertyComponent\SetMQSeriesHeaderPropertyComponent\bin\Debug\\** 目錄。  
...
2.  將 MQHeader 屬性檔複製 **SetMQSMQMDHdrProps.xml** 至 **C:\temp** 目錄。  
...
3.  在 **方案總管 中**, ，以滑鼠右鍵按一下專案，然後按一下 **屬性** 來啟動專案設計工具 （在中間視窗中） 專案。  
...
4.  您稍早建立的管線元件已加入至 **預先組合** 這個管線專案的階段。 如果這個元件尚未加入，您必須完成下列步驟將它加入：  
  
    1.  在[!INCLUDE[btsVStudioNoVersion](../includes/btsvstudionoversion-md.md)]IDE] 中，按一下 [**工具箱**左邊的索引標籤。  
  
    2.  以滑鼠右鍵按一下 **工具箱**, ，然後按一下 **選擇項目**。  
  
    3.  在 **選擇工具箱項目** 對話方塊中，按一下  **BizTalk 管線元件** 索引標籤上，選取 **自訂元件以設定 MQseries 標頭屬性**元件，然後再按一下 **確定**。  
  
    4.  拖放到 **自訂元件以設定 MQseries 標頭屬性**元件至 **預先組合** 此管線的階段。  
...
1.  在 方案總管 視窗中，以滑鼠右鍵按一下方案，以及  **部署方案**。 這樣會建置解決方案並將組件部署至 BizTalk 管理資料庫。  
...
10. 按一下 **確定**, ，然後按一下  **確定** 結束 **接收位置屬性** 對話方塊。  
...
4.  在 **建立本機佇列** 對話方塊中，於 **佇列名稱**, ，型別 **SETHEADER**, ，然後按一下  **確定**。  
...
5.  在 **篩選**, ，加入新項目具有下列名稱/值組︰  
  
    -   設定 **屬性** 至 「 BTS。ReceivePortName 」。  
...
12. 在 **匯出** 對話方塊中，按一下  **建立佇列**, ，然後按一下  **確定** 或 **完成** 直到結束所有對話方塊。  
...
3.  在詳細資料窗格中，以滑鼠右鍵按一下 **setmqheader** 傳送埠，然後按一下  **啟動**。  
...
1.  Put 將檔案 **C:\Temp\Pickup3** 資料夾。  
  
2.  啟動 WebSphere MQ Explorer，然後按兩下 [SETHEADER] 佇列，檢查 SETHEADER 佇列中的訊息。  
  
     若要查看 SETHEADER 佇列中訊息的所有內容屬性，請完成下列步驟：  
  
    1.  按兩下 **SETHEADER** 佇列以顯示 **訊息瀏覽器** 對話方塊。  
  
    2.  在 **訊息瀏覽器** 對話方塊中，按一下  **資料行** 顯示 **訊息的顯示/隱藏資料行** 對話方塊。  
  
    3.  在 **可用的資料行**, ，按兩下每個項目，以讓它顯示在 **訊息瀏覽器**  對話方塊中，然後按一下 **確定**。  
  
3.  每個訊息的訊息內容屬性中應該會看到 **訊息瀏覽器** 對話方塊。  
