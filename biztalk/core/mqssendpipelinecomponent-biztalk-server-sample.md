...
1.  建立名為的資料夾**temp**您 C:\ 磁碟機，如果不存在。  
  
2.  底下建立資料夾**C:\temp**名為目錄**Pickup3**。  
...
3.  找出變數**samplesDir**，確認這個變數設位置**C:\temp**。  
...
3.  在**方案總管 中**，以滑鼠右鍵按一下專案，然後按一下**屬性**以啟動專案設計工具 （在中間視窗中） 專案。  
...
4.  已新增您稍早建立的管線元件**預先組合**這個管線專案的階段。 如果這個元件尚未加入，您必須完成下列步驟將它加入：  
    ...
    4.  拖曳**自訂元件以設定 MQseries 標頭屬性**元件至**預先組合**此管線的階段。  
...
1.  在方案總管 視窗中，以滑鼠右鍵按一下方案，然後**部署方案**。 這樣會建置解決方案並將組件部署至 BizTalk 管理資料庫。  
...
12. 中**匯出**對話方塊中，按一下**Create Queue**，然後按一下 **確定**或**完成**直到結束所有對話方塊為止。  
...
3.  在詳細資料窗格中，以滑鼠右鍵按一下**[setmqheader]**傳送連接埠，然後按一下**啟動**。  
...
2.  啟動 WebSphere MQ Explorer，然後按兩下 [SETHEADER] 佇列，檢查 SETHEADER 佇列中的訊息。  
    ...
    3.  在下**可用的資料行**，按兩下每個項目，以讓它顯示在**訊息瀏覽器**對話方塊，然後按一下**確定**。  
