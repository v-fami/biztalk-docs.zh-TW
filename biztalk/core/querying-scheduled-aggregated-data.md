...
透過 BAM 分析資料庫中動態建立的 OLAP Cube 可以查詢排程的彙總資料。  
 ...
 這個 Cube 的名稱與 BAM 定義 XML 中 View 項目的 Name 屬性相同 (與 Excel 精靈中輸入的檢視名稱相同)。 當您執行 Data Transformation Services (DTS) 封裝 BAM_AN_&LT; BAM 重新整理此 cube\<*ViewName*>，其中\< *ViewName*> 是所描述之檢視的名稱您使用 Excel 精靈中。  
