...
**指令碼處理**運算質可讓您在執行階段使用自訂指令碼或程式碼來執行無法使用的功能。 例如，您可以在執行階段呼叫 COM 物件使用**指令碼處理**運算質和寫入您自己的自訂指令碼。  
  
 如需概念資訊**指令碼處理**運算質，請參閱[指令碼處理運算質](../core/scripting-functoid.md)。  
...
2.  拖曳**指令碼處理**運算質![](../core/media/bts-tls-scripting.gif "bts_tls_scripting")從工具箱拖曳至格線頁上的適當位置。  
...
3.  選取**指令碼處理**您剛剛加入顯示的格線頁的運算質。  
  
4.  在[!INCLUDE[btsVStudioNoVersion](../includes/btsvstudionoversion-md.md)]屬性 視窗中，按一下省略符號 (**...**) 相關聯的按鈕**指令碼**屬性。  
  
    > [!NOTE]
    >  或者，您可以以滑鼠右鍵按一下運算質，然後再按一下**設定運算質指令碼**內容功能表中。 **設定指令碼處理運算質** 對話方塊隨即出現，並**指令碼運算質組態**選取的索引標籤。  
  
5.  在**設定指令碼處理運算質**對話方塊中，於**選取指令碼類型**下拉式清單中，選取您的指令碼的類型。  
  
    > [!NOTE]
    >  視您所選的指令碼類型而定，將會啟用和停用其餘對話方塊欄位的不同子集。  
  
6.  如果您選取**外部組件**做為指令碼類型，使用**指令碼組件**，**指令碼類別**，和**指令碼方法**下拉式清單清單，以該順序，與這分別選取組件、 類別和方法，**指令碼處理**運算質。  
    ...
    >  **內嵌指令碼**文字方塊中已停用，當您選取**外部組件**的指令碼類型。  
     ...
     如果您選取的項目以外**外部組件**（一個內嵌選擇） 的指令碼類型，使用**內嵌指令碼**文字方塊中，輸入您的指令碼中您選取的語言。  
  
    > [!NOTE]
    >  內嵌語言選擇**指令碼處理**運算質包括 C#.NET、 jscript.net、 Visual Basic.NET、 XSLT 以及 XSLT 呼叫範本。  
    ...
    > [!NOTE]
    >  您可以建立自訂指令碼中直接**內嵌指令碼**文字方塊中，或您可以建立您的指令碼在其他地方，並將它貼入**內嵌指令碼**文字方塊。  
  
    > [!NOTE]
    >  **指令碼組件**，**指令碼類別**，和**指令碼方法**下拉式清單會停用，當您選取其中一個內嵌選擇 (項目以外**外部組件**) 的指令碼類型。  
...
8.  在大部分情況下，您**指令碼處理**運算質將產生輸出值用於填入目的結構描述中的欄位，或做為另一個運算質的輸入，在許多不同方式基本運算質的執行。  
