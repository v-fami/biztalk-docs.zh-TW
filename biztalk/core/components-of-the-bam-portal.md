---
title: "BAM 入口網站元件 |Microsoft 文件"
ms.custom: 
ms.date: 06/08/2017
ms.prod: biztalk-server
ms.reviewer: 
ms.suite: 
ms.tgt_pltfrm: 
ms.topic: article
helpviewer_keywords: BAM portal, features
ms.assetid: 5349e75e-4946-4ce4-a756-ac7914ddb7f2
caps.latest.revision: "11"
author: MandiOhlinger
ms.author: mandia
manager: anneta
ms.openlocfilehash: 7722b8605254c43e145edd87c7dfa5c3a38b0184
ms.sourcegitcommit: cb908c540d8f1a692d01dc8f313e16cb4b4e696d
ms.translationtype: MT
ms.contentlocale: zh-TW
ms.lasthandoff: 09/20/2017
---
# <a name="components-of-the-bam-portal"></a>BAM 入口網站的元件
BAM 入口網站是劃分為三個功能區域 (對應至入口網站的三個頁面) 的 ASP.NET 網頁。 本節將描述這些頁面： 活動搜尋 頁面、 彙總 頁面上及警示管理員 頁面。 您可以使用這些頁面來建立活動、搜尋現有活動或修改現有活動。  
  
## <a name="in-this-section"></a>本節內容  
  
-   [BAM 入口網站頁面版面配置](../core/bam-portal-page-layout.md)  
  
-   [BAM 入口網站上的活動搜尋頁面](../core/activity-search-on-the-bam-portal-page.md)  
  
-   [BAM 入口網站上的彙總頁面](../core/aggregations-on-the-bam-portal-page.md)  
  
-   [BAM 入口網站上的警示管理員頁面](../core/alert-manager-on-the-bam-portal-page.md)