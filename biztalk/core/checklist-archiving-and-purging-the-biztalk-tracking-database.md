...
|雖然您可使用登入認證來執行 DTA 清除和封存工作，但是為了提高安全性，您應該以必要的 SQL Server 認證設定 BTS_BACKUP_USERS 角色，以執行 DTA 清除和封存工作。 如此可協助避免權限提升的情形發生。|[如何設定 BTS_BACKUP_USERS 角色以封存和清除 BizTalk 追蹤資料庫中的資料](../core/configure-bts_backup_users-role-to-archive-and-purge-from-tracking-database.md)|  
