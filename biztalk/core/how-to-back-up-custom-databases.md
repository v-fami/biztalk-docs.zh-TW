...
2. 若要備份自訂資料庫，您必須透過有權存取待備份之各個資料庫的使用者帳戶進行登入。  
   ...
   -   BizTalk Server 的備份作業可以設定不同於所使用的 SQL Server Agent 服務的使用者帳戶下執行。  
...
1. 在新資料庫中建置物件：  
   ...
   - 瀏覽至[!INCLUDE[btsBiztalkServerPath](../includes/btsbiztalkserverpath-md.md)]結構描述的目錄，並針對所有您想要備份您自訂資料庫執行 Backup_Setup_All_Procs.sql 和 Backup_Setup_All_Tables.sql。 這會建立必要的程序、資料表和角色，並將權限指派給預存程序。  
...
3. 使用 SQL Server Enterprise Manager 或 SQL Server Management Studio，在 BizTalk 管理 (BizTalkMgmtDb) 資料庫中，修改**adm_OtherBackupDatabases**自訂資料庫的每個包含資料列的資料表。  
