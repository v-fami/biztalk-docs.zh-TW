...
1.  在**傳輸屬性**，依序展開**系統定義**，然後輸入 TIBCO EMS 伺服器連線的所有必要的資訊。  
  
     您必須設定組態參數，Microsoft BizTalk Adapter for TIBCO Enterprise Message Service 才能連線至 TIBCO EMS。 **此資料是區分大小寫。**  
  
2.  展開**訊息處理**，然後輸入所有必要的資訊。  
...
6.  展開**使用者認證**輸入**使用者名**和**密碼**來存取 TIBCO EMS 伺服器。  
