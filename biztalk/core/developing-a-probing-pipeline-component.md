...
可實作任何管線元件 （一般、 組合或解譯）`IProbeMessage`介面必須支援訊息探查功能。 探查元件會在管線階段具有**FirstMatch**執行模式。 在這種階段中，BizTalk 傳訊引擎會提供訊息的開頭部分給元件，以確定該元件是否可識別訊息的格式。 若元件可識別格式，則會傳送完整的訊息給此元件進行處理。  
...
3. **探查**叫用方法。 如果傳回的值是 **，則為 True**，表示元件已執行。 接著便完成階段處理，並將訊息傳送到下一個階段。  
