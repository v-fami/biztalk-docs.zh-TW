...
2. 在  **X12 後援設定**對話方塊中，於**X12 協議頁面**索引標籤之下**交易集設定**區段中，按一下**本機主機設定**.  
  
3. 選取  **Convert 隱含的小數格式 Nn 10 進制數值**来轉換成 BizTalk Server 中的中繼 XML 中的基底 10 數值格式 Nn 指定的 EDI 數字。  
...
5. 針對**目標命名空間**、 輸入 （或從下拉式清單中選取） 目標命名空間，[!INCLUDE[btsBizTalkServerNoVersion](../includes/btsbiztalkservernoversion-md.md)]用以決定處理所接收的訊息之結構描述。  
  
6. 按一下 **套用**以接受變更，或按一下**確定**輸入並驗證變更，然後關閉對話方塊。  
