...
6.  在**目的地**，輸入從.msi 檔案，包括檔案名稱安裝應用程式時要複製的組件檔案所在的位置的完整路徑。 如果不提供，安裝期間就不會將組件檔案複製到本機檔案系統。  
  
7.  在**選項**，指定安裝至 GAC，BizTalk 組件的選項，然後按一下**確定**。  
...
2.  輸入下列命令，並以適當的值替代，如下表所述：  
    ...
    |**/ 選項**|-   **為 GacOnAdd**： 在 AddResource 作業期間指定的組件安裝至全域組件快取 (GAC) 在本機電腦上。<br />-   **GacOnInstall**： 指定從.msi 檔案安裝應用程式時，將組件安裝到 GAC。<br />-   **GacOnImport**： 指定匯入應用程式.msi 檔案時，將組件安裝到 GAC。<br /><br /> 您必須以逗點分隔多個選項。|  
