...
ms.locfileid: "37000343"
---
# <a name="managing-parties"></a>管理合作對象
使用**合作對象**節點，您可以設定商務夥伴 （合作對象） 或內部部門 （商務設定檔） 與[!INCLUDE[btsBizTalkServerNoVersion](../includes/btsbiztalkservernoversion-md.md)]解決方案互動。 如需詳細資訊，請參閱 <<c0> [ 交易夥伴](../core/trading-partners-and-business-profiles.md)。  
...
1. 開啟 [BizTalk Server 管理] 。  依序展開[!INCLUDE[btsBizTalkServerAdminConsoleui](../includes/btsbiztalkserveradminconsoleui-md.md)]、 BizTalk 群組、 以滑鼠右鍵按一下**合作對象**，選取**新增**，然後選取**合作對象**。  
...
2. 在 **一般**頁面上，執行下列動作：  
   ...
   |                                **其他屬性 – 名稱/值**                                 |                                                                                                                                                請輸入名稱-值組來儲存合作對象的任何資訊。 您可以視需要新增任意數目的名稱-值組。 **請注意**： 名稱 / 值組不會使用 BizTalk Server 進行任何處理; 此資料僅供參考之用。                                                                                                                                                 |
   |                                               **刪除**                                                |                                                                                                                                                                                                                                                    選取即可刪除選取的名稱 / 值組。                                                                                                                                                                                                                                                     |
...
3. 在 **傳送埠**頁面上，執行下列動作。  
   ...
   > [!NOTE]
   >  在 BizTalk Server 傳送埠關聯是在合約層級。 **傳送埠**可用為合作對象屬性的一部分是純粹是為了回溯相容性 頁面。 當您將傳送埠與協議相關聯後，傳送埠設定也會傳播到合作對象設定，而您就可以在此頁面看到該傳送埠關聯。 但是，並非反之亦然。 在您將傳送埠與合作夥伴相關聯後，該傳送埠無法自動出現在協議設定中。  
...
4. 在 **憑證**頁面上，執行下列動作：  
   ...
   |        使用        |                                                                                                                                                 以進行此動作                                                                                                                                                 |
   |------------------------|------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
   |       **瀏覽**       |                                                選取即可顯示**選取憑證**對話方塊，其中要套用至訊息傳送至合作對象在本機電腦 」 或 「 其他人憑證存放區選取簽章憑證。                                                 |
   |    **一般名稱**     |                                                                                                                            顯示選取憑證的描述。                                                                                                                             |
...
5. 選取**套用**以接受這些屬性，或選取**確定**完成組態設定。 任何一項動作都會驗證設定。  
...
4. 選取**套用**以接受這些屬性，或選取**確定**完成組態設定。 任何一項動作會驗證設定。  
...
1. 在 [ **BizTalk Server 管理]**，選取**合作對象**，以滑鼠右鍵按一下您想要刪除此項目，然後選取的合作對象**刪除**。  

2.  在 **確認刪除合作對象**對話方塊中，選取**是**刪除合作對象。  

## <a name="search-for-a-party"></a>搜尋合作對象
如果您有大量建立合作對象時，您可以使用**搜尋**選項，以顯示您想要查看的合作對象。  

1. 在 [ **BizTalk Server 管理]**，選取**合作對象**。

2. 在 **合作對象與商務設定檔**窗格中的，於右上角，輸入搜尋字串中的**搜尋合作對象**文字方塊，然後選取搜尋圖示。 您也可以按 ENTER 開始搜尋。  
