...
7.  在**新增連結的伺服器**對話方塊**一般**頁面上，於**連結的伺服器**，輸入您想要連結至伺服器的名稱。  
  
     例如，裝載 BizTalk MessageBox (BizTalkMsgBoxDb) 資料庫、BizTalk 追蹤 (BizTalkDTADb) 資料庫或驗證伺服器的伺服器。  
  
8.  下**伺服器類型**，按一下  **SQL Server**，然後按一下 **確定**。  
  
9. 在**Microsoft SQL Server Management Studio**，連按兩下**SQL Server Agent**，然後按一下 **作業**。  
