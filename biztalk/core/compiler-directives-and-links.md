---
title: "編譯器指示詞和連結 |Microsoft 文件"
ms.custom: 
ms.date: 06/08/2017
ms.prod: biztalk-server
ms.reviewer: 
ms.suite: 
ms.tgt_pltfrm: 
ms.topic: article
helpviewer_keywords:
- compilier directives
- maps, compiling
- BizTalk Mapper, compiler directives
- links [maps], compiling
ms.assetid: 1655e10c-af98-4100-849d-b8214f93cfe9
caps.latest.revision: "7"
author: MandiOhlinger
ms.author: mandia
manager: anneta
ms.openlocfilehash: b598638072d59e635fd75c8e00836829d9459078
ms.sourcegitcommit: cb908c540d8f1a692d01dc8f313e16cb4b4e696d
ms.translationtype: MT
ms.contentlocale: zh-TW
ms.lasthandoff: 09/20/2017
---
# <a name="compiler-directives-and-links"></a>編譯器指示詞和連結
您可以設定以下兩個以逐一連結為基礎的編譯器指示詞：  
  
-   從輸入執行個體訊息中擷取何種資料，並複製為輸出執行個體訊息中相關的項目或屬性值。  
  
    > [!NOTE]
    >  此處的資料包含複製項目名稱或屬性本身的選項，而不是任何與項目或屬性相關聯的值。 項目和屬性名稱一般不會被視為 XML 執行個體訊息中附帶資料的一部分。  
  
-   如何執行來源與目的結構描述之間的節點比對。  
  
 本節提供這些指示詞可用選項的詳細說明。  
  
## <a name="in-this-section"></a>本節內容  
  
-   [資料轉換組態](../core/data-transformation-configuration.md)  
  
-   [節點階層層級比對](../core/node-hierarchy-level-matching.md)