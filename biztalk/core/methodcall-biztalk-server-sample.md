...
3.  可能是適當的數學程式庫方法，**新增**或**Subtract**，執行要求的計算，以及封裝以 XML 文件的結果。  
...
-   使用**決定**」 圖形來表達協調流程中的"if-then-else"邏輯。 協調流程將辨別欄位的值指派給內部變數之後，即會進入**決定**圖形以檢查是否應該執行的加法或減法運算。 如果沒有需要執行的作業，協調流程就會終止。  
