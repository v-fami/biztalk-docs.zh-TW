...
## <a name="about-tibco-rendezvous"></a>關於 TIBCO Rendezvous  
 TIBCO Rendezvous 是程式的訊息匯流排提供企業應用程式整合 (EAI)。 TIBCO 可提供 C、C++、Java、Visual Basic 與 Microsoft .NET Framework 的訊息 API 以在 Microsoft Office Excel 工作表與其他自選的應用程式上接收資料摘要。 如需詳細資訊，請參閱[TIBCO Rendezvous 概念](../core/tibco-rendezvous-concepts.md)。  
