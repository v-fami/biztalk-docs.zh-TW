...
---
# <a name="configuring-x12-fallback-agreement-properties"></a>設定 X12 後援協議屬性
只要交換的是 X12 編碼交換 (包括 HIPAA)，同時 [!INCLUDE[btsBizTalkServerNoVersion](../includes/btsbiztalkservernoversion-md.md)] 無法判斷負責 (輸入或輸出) 交換之協議的情況下，就可適用本節所說明的屬性。 本節也說明了 X12 和 EDIFACT 編碼訊息都通用的一般後援設定。  
