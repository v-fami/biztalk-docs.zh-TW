...
1. 更新 web.config 檔案來設定 Web 服務**追蹤**編譯器指示詞 **，則為 true** ，並新增**TraceSwitch**值：  
   ...
   > [!NOTE]
   >  如果**追蹤**編譯器指示詞未設定為 **，則為 true**則不會產生任何追蹤輸出。 **TraceSwitch**值也可以設定在呼叫的類別中，但仍會設定以下便利的 web.config 檔案中。 設定**TraceSwitch**值適當的層級，適用於開發用途，並在開發完成以減少或禁止追蹤輸出後變更的值。  
...
2. 建立的執行個體**TraceSwitch**並**TextWriterTraceListener**類別，並使用**try...catch**截獲並將錯誤寫入的 Web 服務 [WebMethod] 呼叫中封鎖追蹤輸出檔。 例如，在嘗試將整數變數 s2 設定為物件變數 o2 之 null 執行個體時所產生的錯誤，將由下列程式碼所截獲：  
   ...
   > [!NOTE]
   >  此程式碼是預設會產生當您建立新的 HelloWorld Web 服務的修改的版本**ASP.Net Web 服務**專案，在 Microsoft [!INCLUDE[btsVStudioNoVersion](../includes/btsvstudionoversion-md.md)]。  
   > 
   > [!NOTE]
...
##### <a name="resolution"></a>解決方案  
 確認已安裝及啟用安裝 ASP.NET。 安裝[!INCLUDE[btsDotNetFramework](../includes/btsdotnetframework-md.md)]如果它是未安裝和/或執行**aspnet_regiis.exe**程式位於 %WinDir%\Microsoft.NET\Framework\\*vXXX.XXX*\ IIS 伺服器的資料夾。  
...
##### <a name="resolution"></a>解決方案  
 此錯誤已記錄在 Microsoft 知識庫文件 823196 [PRB： 您會收到"System.IO.FileNotFoundException"錯誤時用戶端應用程式呼叫 Web 服務](http://go.microsoft.com/fwlink/?LinkID=84694)"。 請依照此知識庫文件中＜解決方法＞一節的步驟執行，解決這項錯誤。  
...
##### <a name="problem"></a>問題  
 當您處理具有與 BizTalk Server Web 服務發佈精靈，與欄位中包含的任何資料產生 web 服務的文件**資料型別**的**xs: date**和**Nillable**的屬性 **，則為 True**從文件中移除。  
  
##### <a name="cause"></a>原因  
 這個問題發生的原因，是因為 Microsoft .NET Framework 類別庫命名空間 System.Xml.Serialization 所提供的 XmlSerializer 類別發生已知問題。  
  
##### <a name="resolution"></a>解決方案  
 若要解決此問題，請安裝.NET Framework 2.0 hotfix，Microsoft 知識庫文件 925272 」[修正： XML 序列化可能會遺失某些選用的項目，在.NET Framework 2.0 中的 XSD 結構描述](http://go.microsoft.com/fwlink/?LinkId=84696)"。  
