...
EDI 接收管線和 EDI 傳送管線，可以對交易集資料元素執行 EDI 驗證。 這項驗證針對特定合作對象，透過該合作對象的協議屬性的所有訊息上設定**驗證**網頁 (底下**交易集設定**的 [X12] 區段或 EDIFACT 協議）。 如果**EDI 類型驗證**屬性中未選取**驗證**頁面上，將不會執行驗證，本主題中所述的資料。  
  
 EDI 類型驗證啟用內送與外寄訊息選取**EDI 類型驗證**中的屬性**驗證**頁面之雙向協議中的索引標籤**協議屬性** 對話方塊。 對於內送和外寄訊息，這個屬性預設會選取，因此預設會啟用 EDI 驗證。  
...
- 對於 EDIFACT，資料元素中建立字元集**UNB1**的**字元集和分隔符號**頁面的單向協議索引標籤**協議屬性**對話方塊或**EDIFACT 後援設定**（若尚未建立任何協議） 對話方塊。  
  
- 對於 KEDIFACT，資料元素中建立字元集**UNB1**的**字元集和分隔符號**頁面的單向協議索引標籤**協議屬性**對話方塊 方塊中，像 EDIFACT 一樣。 值**UNB1.1**項目必須設定為`KECA`。  
  
- 對於 X12，要在管線屬性中建立訊息的字元集。  
  
  > [!NOTE]
  >  當 BizTalk 執行階段對訊息執行 EDI 驗證時，它會使用已在管線屬性中選取的 X12 字元集。 不過，屬性的驗證輸入的頁面**協議屬性** 對話方塊中會執行使用中選取的字元集**字元集和分隔符號**該對話方塊的頁面。 在執行階段，管線會忽略此值的 x12 字元集屬性**字元集和分隔符號**頁**協議屬性** 對話方塊。 如果這兩項設定不相同 (亦即，X12 字元集屬性**協議屬性** 對話方塊中設定為**擴充**而管線屬性中的字元屬性設定為 X12**基本**)，可能會造成驗證錯誤的訊息。  
 ...
 **空白資料元素和尾端分隔符號驗證**  
 ...
 對於 X12，標示為必要的資料元素不可為空白執行個體中的區段是否存在。 若是複合資料元素，至少必須要有一個元件資料元素有設定值。  
...
|未預期的區段/記錄|接收管線會產生適當的前置詞的標記： \<UnexpectedSegment_ SegmentID %\>。<br /><br /> 傳送管線會將 XML 標記名稱的前三個字元做為區段名稱。|  
