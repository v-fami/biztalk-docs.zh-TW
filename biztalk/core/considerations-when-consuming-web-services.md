...
## <a name="required-xlangs-support-for-parameter-types"></a>參數類型必要的 XLANG/s 支援  
 使用非 XLANG/s 支援的 Web 方法參數類型將會造成編譯錯誤。 例如，BizTalk Server 不支援包含結構描述類型之單一維度陣列的參數。 此外，BizTalk Server 不支援多維度陣列。 如需在 BizTalk Server 的 XLANG/s 語言保留字的清單，請參閱[XLANG s 保留字](../core/xlang-s-reserved-words.md)。  
...
> [!NOTE]
>  BizTalk Server 不支援使用的陣列**資料集**Web 服務所公開的物件。 XLANG/s 子服務原本即支援 .NET DataSet 類別，但是如果您建立了 BizTalk 專案，其中包含了將 .NET DataSet 物件陣列公開的 Web 服務之 Web 參考，則當您嘗試編譯專案時，會產生錯誤。  
...
## <a name="consuming-messaging-only-web-services"></a>使用僅供傳訊的 Web 服務  
 ...
 如需詳細資訊，請參閱[傳訊只在情節中取用 Web 服務如何](../core/how-to-consume-web-services-in-a-messaging-only-scenario.md)。  
...
> [!NOTE]
>  若要設定**MethodName**屬性靜態 soap 傳送埠以程式設計的方式，您需要設定**方法名稱**至 **[稍後指定]** 中**Web服務** 索引標籤**SOAP 傳輸屬性**在 BizTalk Server 管理主控台 對話方塊。  
>   
>  如需有關**MethodName**屬性，請參閱[如何動態設定取用 Web 服務的 URI](../core/how-to-dynamically-set-the-uri-of-a-consumed-web-service.md)。  
>   
>  如需有關**SOAP 傳輸屬性**對話方塊中，請參閱**SOAP 傳輸屬性對話方塊、 Web 服務** 索引標籤[!INCLUDE[ui-guidance-developers-reference](../includes/ui-guidance-developers-reference.md)]。
...
-   如需自訂管線元件的詳細資訊，請參閱[開發自訂管線元件](../core/developing-custom-pipeline-components.md)。  
  
-   如需 SOAP 傳送配接器的組態屬性的詳細資訊，請參閱[如何動態設定取用 Web 服務的 URI](../core/how-to-dynamically-set-the-uri-of-a-consumed-web-service.md)。  
...
4.  在 BizTalk Server 管理主控台中，在**Web 服務**索引標籤中**SOAP 傳輸屬性**對話方塊中的 soap 傳送埠時，指定您想要使用您建立該 proxy。 您同時需要指定組件、類型與方法。 如需詳細資訊，請參閱**SOAP 傳輸屬性對話方塊、 Web 服務** 索引標籤[!INCLUDE[ui-guidance-developers-reference](../includes/ui-guidance-developers-reference.md)]。
...
## <a name="biztalk-schema-generation-using-the-add-service-reference"></a>使用加入服務參考來產生 BizTalk 結構描述  
 當您使用**加入服務參考**若要加入服務參考加入 BizTalk 專案，BizTalk Server 將會呼叫每個 Web 方法至結構描述所需的結構描述類型。 BizTalk Server 會將這些結構描述加入到 Reference.xsd 中。 若要確保**加入服務參考**產生 BizTalk 結構描述正確，Web 服務必須符合下列指導方針：  
