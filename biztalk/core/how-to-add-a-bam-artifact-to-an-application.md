...
本主題描述如何使用 [BizTalk Server 管理] 主控台或命令列，將 BAM 成品新增至 BizTalk 應用程式。 新增 BAM 定義檔案時，不會一併部署 BAM 定義。 當匯入應用程式 .msi 檔案時，會部署此 BAM 定義。  
 ...
 如果您想要覆寫應用程式中現有的 BAM 成品，請指定覆寫選項。 只有在現有的 BAM 成品具有相同的檔案名稱，與您想要新增時，才需要覆寫選項。 如果未指定，BAM 成品已經存在於相同的檔案名稱，為所新增的應用程式，新增作業將會失敗。  
...
1. 按一下 **開始**，按一下**Al 程式**，按一下  [!INCLUDE[btsBizTalkServerStartMenuItemui](../includes/btsbiztalkserverstartmenuitemui-md.md)]，然後按一下**BizTalk Server 管理**。  
...
6. 在 **目的地**，輸入要在安裝應用程式從.msi 檔案，包括檔案名稱複製成品檔案之位置的完整路徑。 如果未提供此路徑，安裝期間就不會將檔案複製到本機檔案系統，以致安裝期間無法將元件寫入 Windows 登錄。  
