...
6. 傳送管線將每個交易集建置成 EDI 交換，然後將交換傳送至目的地。  
   ...
   > [!NOTE]
   >  如需有關如何[!INCLUDE[btsBizTalkServerNoVersion](../includes/btsbiztalkservernoversion-md.md)]分割交易集在批次，請參閱[分割批次 EDI 交換](../core/splitting-a-batched-edi-interchange.md)。  
...
3. 接收管線升級相同的屬性，如同交換不是批次的情況一樣，唯一的不同是它會在產生的 XML 上套用保留標記。 此標記現在\<X12InterchangeXml\>針對 X12 編碼的 EDI 交換或\<EdifactInterchangeXml\>針對 EDIFACT 編碼的 EDI 交換。 EDI 接收管線也會套用內容屬性 `ReuseEnvelope`，以識別保留的交換。  

   > [!NOTE]
   >  EDI 傳送管線會使用\<X12InterchangeXml\>或是\<EdifactInterchangeXml\>標記來識別為保留的批次訊息。 `ReuseEnvelope` 內容屬性可讓您建立傳送埠，以訂閱保留的所有批次交換。  

4. 接收管線會置放在 MessageBox 中的訊息 XML 檔案。  
...
6. 傳送管線將 XML 檔案中的兩個交易集建置成批次 EDI 交換，然後將交換傳送到目的地。  
   ...
   > [!NOTE]
   >  如需有關如何[!INCLUDE[btsBizTalkServerNoVersion](../includes/btsbiztalkservernoversion-md.md)]處理保留批次，請參閱 <<c2> [ 保留收到批次 EDI 交換](../core/preserving-a-received-batched-edi-interchange.md)。  
...
-   將不會傳回技術或功能通知，以回應原本收到的交換或任何傳送的批次交換。  
    ...
    > [!NOTE]
    >  如需產生 EDI 通知的資訊，請參閱[逐步解說 (X12)： 接收 EDI 交換和傳送，將通知傳回](../core/walkthrough-x12--receive-edi-interchanges-and-send-back-an-acknowledgement.md)。  
...
- 將一個批次 EDI 交換放入與接收位置關聯的本機資料夾。 接著您可以確認 [!INCLUDE[btsBizTalkServerNoVersion](../includes/btsbiztalkservernoversion-md.md)] 是否已將接收之批次中的個別交換 (如果分割批次的話) 或單一保留的批次交換 (如果保留批次的話) 放入與傳送埠關聯的資料夾中。  
  ...
  > [!NOTE]
  >  如果您已經執行傳送批次交換逐步解說中的步驟，可以使用來自該解決方案的輸出做為此解決方案的輸入。 該解決方案的輸出是兩則 850 訊息的批次。 如需詳細資訊，請參閱 <<c0> [ 逐步解說 (X12)： 傳送批次 EDI 交換](../core/walkthrough-x12-sending-batched-edi-interchanges.md)。  
...
2. 以滑鼠右鍵按一下您的專案，指向**新增**，然後按一下**現有項目**。 移至**\<磁碟機\>: \Program Files\Microsoft BizTalk Server 2009\XSD_Schema\EDI\X12\00401**，然後按兩下測試訊息的對應結構描述。  
   ...
   > 
   > [!NOTE]
   >  針對測試訊息，您可以使用傳送批次交換逐步解說中解決方案所產生的批次交換。 該解決方案的輸出是兩個用於「EDI 介面開發人員」教學課程之 850 範例訊息執行個體的批次。 如果您這樣做，您必須使用位於結構描述 x12_00401_850.xsd [!INCLUDE[btsBiztalkServerPath](../includes/btsbiztalkserverpath-md.md)]SDKEDI 介面開發人員 TutorialInbound_EDI。  
...
4. 接收位置命名，選取**檔案**for**型別**，然後按一下**設定**。  

5. 輸入所需的資料夾**接收資料夾**，和遮罩**檔案遮罩**，這類 **\*.txt**。  

6. 按一下 [確定] 。  

7. 針對**接收管線**，選取**EdiReceive**。  
...
2. 在 [**傳送埠屬性**] 對話方塊中，傳送埠的名稱。  

3. 在 **傳輸**區段中，選取**檔案**的**型別**，然後按一下**設定**。  

4. 輸入所需的資料夾**目的地資料夾**，和遮罩**檔案遮罩**，這類 **\*.txt**。  

5. 按一下 [確定] 。  

6. 在 **傳送管線**，選取**EdiSend**。  

7. 在主控台樹狀目錄中，選取**篩選器**。 在 **篩選器**頁面上，輸入將訂閱批次中的訊息篩選條件運算式。 例如，選取**BTS。ReceivePortName** for**屬性**， **==** 如**運算子**，和您剛才為建立的接收埠名稱**值**。  

8. 按一下 [確定] 。  

9. 在主控台樹狀目錄中，按一下**傳送埠**。 在 **傳送埠**窗格中，以滑鼠右鍵按一下您的傳送埠，然後**開始**。  
...
4. 在 **設定檔屬性**對話方塊的 **一般**頁面上，輸入**PartyA_Profile**中**名稱**文字方塊。  
...
4. 在 **設定檔屬性**對話方塊的 **一般**頁面上，輸入**PartyB_Profile**中**名稱**文字方塊。  
...
7. 在上執行下列工作**party a-> partyb**  索引標籤。  
   ...
   3. 在上**字元集和分隔符號**下方的頁面上**交換設定**區段中，選取**CR LF**選項。  

   4. 上**本機主機設定**下方的頁面上**交換設定**區段的**輸入訊息處理選項**方塊中，選取**分割交換為交易集-發生錯誤時暫停交易集**選項。  

      > [!NOTE]
      >  我們在此解決方案中，一開始會選取此選項，來分割交換。 更新版本的一部分[若要測試逐步解說](../core/walkthrough-x12-receiving-batched-edi-interchanges.md#BKMK_Proc)以下的程序，我們將設定為 「 保留交換的解決方案。  

   5. 上**傳送埠**下方的頁面上**交換設定**區段中，將您稍早建立的傳送埠產生關聯。 在 **傳送埠**格線下方**名稱** 欄中，按一下空白儲存格，並從下拉式清單中，選取 傳送埠。  

   6. 在上**信封**下方的頁面上**交易集設定**區段中，輸入在格線的第一行中的所有資料行的值。  
...
3. 在 [[!INCLUDE[btsBizTalkServerNoVersion](../includes/btsbiztalkservernoversion-md.md)]管理主控台中，按一下**合作對象**] 節點，按一下任何一個是稍早建立的協議的一部分的兩個商務設定檔。 在 **協議**區段中，以滑鼠右鍵按一下 協議，然後按一下**屬性**。  

4. 在 **協議屬性**對話方塊的 **本機主機設定**頁面**交換設定**區段的**輸入訊息處理選項**方塊中，選取**保留交換-發生錯誤時暫停交換**或是**保留交換-發生錯誤時暫停交易集**按一下**確定**。  

5. 按一下 **主控件執行個體**下方**平台設定** 節點，以滑鼠右鍵按一下**biztalkserverapplication**，然後按一下 **重新啟動**。  
...
## <a name="see-also"></a>另請參閱  
 ...
 [保留收到的批次處理 EDI 交換](../core/preserving-a-received-batched-edi-interchange.md)   
 [逐步解說 (X12)： 傳送批次的 EDI 交換](../core/walkthrough-x12-sending-batched-edi-interchanges.md)   
 [逐步解說 (X12)： 接收 EDI 交換，並傳回通知](../core/walkthrough-x12--receive-edi-interchanges-and-send-back-an-acknowledgement.md)   
