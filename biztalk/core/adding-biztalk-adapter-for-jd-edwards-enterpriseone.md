...
1.  開啟**BizTalk Server 管理**，依序展開**BizTalk Server 管理**，依序展開**BizTalk 群組**，然後展開**平台設定**.  
  
2.  以滑鼠右鍵按一下**配接器**，選取**新增**，然後選取**配接器**。  
  
3.  輸入配接器的名稱。 例如，輸入`JDEEnterpriseOne`。  
  
4.  選取**JDEEnterpriseOne**從**配接器**清單，並選取**確定**。  

## <a name="create-the-send-port"></a>建立傳送埠  
  
1.  在**BizTalk Server 管理**，依序展開**應用程式**，然後展開您想要主控之成品的應用程式
...
3.  在 **傳送埠屬性** 對話方塊方塊中，執行下列動作︰  
    ...
    -   在**類型**下拉式清單中，選取**JDEdwards**。  
  
    -   在**傳送處理常式**下拉式清單中，選取傳送處理常式位址。  
...
### <a name="enter-the-properties"></a>輸入的內容  
...
6.  在**JDE EnterpriseOne 傳輸屬性**屬性，依序展開**配接器必要屬性**，並輸入 JD Edwards EnterpriseOne 伺服器連線的所有必要的資訊。  使用下列指導方針設定傳輸屬性：  
    ...
    |JDEdwards JAR 檔案|輸入每個 JAR 檔案的完整路徑與檔案名稱：<br /><br /> -C:\JDEOWJars\Connector.jar<br />-C:\JDEOWJars\Kernel.jar<br />-Program Files\Microsoft BizTalk Adapters for Enterprise Applications\J.D。 Edwards EnterpriseOne(r)\Classes\JDEDynAccess.jar<br /><br /> 每個 jar 檔案都必須以不加空格的分號 (;) 分隔 (例如，<br /><br /> `<c:>\Connector.jar;<c:>\Kernel.jar;`)|  
    ...
    |**並行存取控制**||  
    |同時呼叫數目上限|輸入的數值 **同時呼叫數目上限**。 這個數字代表同時呼叫，最大數目，例如 `10`。<br /><br /> 此欄位的預設值是 5。|  
    |**重新整理代理程式**||  
    |重新整理代理程式|選取 **是** 的 **重新整理代理程式** 強制 runtimeagent.exe 和 browsingagent.exe 程序在必要時自動重新啟動。<br /><br /> 例如，您希望處理程序在與伺服器中斷連線時自動重新啟動，或是您在伺服器加上其他項目，但是該項目並未出現在 Microsoft 配接器精靈的選項中。|  
    |**安全性伺服器**||  
...
#### <a name="refresh-agent"></a>重新整理代理程式  
 當您選取 **是** 的 **重新整理代理程式**, ，您可以強制 runtimeagent.exe 和 browsingagent.exe 程序在必要時自動重新啟動。  
...
> [!NOTE]
>  browsingagent.exe 要等到您終止目前的瀏覽工作階段才會重新整理。 例如，您必須結束 「 新增產生的項目...」 瀏覽工作階段並重新輸入重新整理 browsingagent.exe。  
...
此配接器會要求您選取**XMLTransmit**和**XMLReceive**傳送和接收管線分別。  
...
3.  在 **傳送埠屬性** 對話方塊方塊中，執行下列動作︰  
    ...
    1.  輸入傳送埠的名稱，例如 `SendToJDEEnterpriseOne`。  
