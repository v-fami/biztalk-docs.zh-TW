---
title: "管理彙總 |Microsoft 文件"
ms.custom: 
ms.date: 06/08/2017
ms.prod: biztalk-server
ms.reviewer: 
ms.suite: 
ms.tgt_pltfrm: 
ms.topic: article
helpviewer_keywords:
- managing [BAM], aggregations
- aggregations [BAM], managing
ms.assetid: 54b11cec-1bc4-4c40-a8f8-9f3a40f3161f
caps.latest.revision: "6"
author: MandiOhlinger
ms.author: mandia
manager: anneta
ms.openlocfilehash: f1d645db5b109383ace2a7870dbb01000689b1d1
ms.sourcegitcommit: cb908c540d8f1a692d01dc8f313e16cb4b4e696d
ms.translationtype: MT
ms.contentlocale: zh-TW
ms.lasthandoff: 09/20/2017
---
# <a name="managing-aggregations"></a>管理彙總
彙總是資料集衍生一般代表特性或資料集本身描述的累計作業。 本節說明如何管理不同的彙總項目，例如索引、活動與即時彙總 (RTA)。  
  
 如需彙總的一般資訊，請參閱[BAM 入口網站中的彙總](../core/aggregations-in-the-bam-portal.md)和[BAM 入口網站頁面上的彙總](../core/aggregations-on-the-bam-portal-page.md)。  
  
## <a name="in-this-section"></a>本節內容  
  
-   [如何建立索引](../core/how-to-create-an-index.md)  
  
-   [如何取得彙總的索引清單](../core/how-to-get-a-list-of-indexes-on-an-aggregation.md)  
  
-   [如何刪除索引](../core/how-to-delete-an-index.md)  
  
-   [如何設定活動視窗的持續時間](../core/how-to-set-the-duration-on-an-activity-window.md)  
  
-   [如何取得活動視窗的持續時間](../core/how-to-get-the-duration-on-an-activity-window.md)  
  
-   [如何設定 RTA 視窗的持續時間](../core/how-to-set-the-duration-on-an-rta-window.md)  
  
-   [如何取得 RTA 視窗的持續時間](../core/how-to-get-the-duration-on-an-rta-window.md)  
  
-   [如何定義 BAM 彙總](../core/how-to-define-bam-aggregations.md)