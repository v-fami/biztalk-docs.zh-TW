...
## <a name="what-this-sample-does"></a>此範例的用途  
 這個範例示範如何使用**ExplorerOM**系統管理類別來建立及設定接收埠和接收位置。 本主題也包含 Windows PowerShell 範例指令碼。 此範例會執行下列作業：  
  
-   建立新接收埠命名為 「 我的接收埠 」。  
  
-   建立新接收位置與新的連接埠相關聯，而且設定為使用 HTTP 傳輸通訊協定。  
  
-   此範例也包含範例程序來刪除，並列舉接收埠和位置。  
...
|檔案|Description|  
|---------------|-----------------|  
|ReceiveLocations.cs|[!INCLUDE[btsVCSharp](../includes/btsvcsharp-md.md)]這個範例中示範的作業的來源檔案。|  
...
2.  若要變更\<*範例*> \Admin\ExplorerOM\ReceiveLocations\bin\debug 目錄。  
  
3.  執行 ReceiveLocations.exe。  
  
4.  檢視新的接收埠和接收位置與[!INCLUDE[btsBizTalkServerNoVersion](../includes/btsbiztalkservernoversion-md.md)]管理主控台。  
  
## <a name="windows-powershell-script-example"></a>Windows PowerShell 指令碼範例  
 下列 PowerShell 範例指令碼示範如何為相同的作業[!INCLUDE[btsVCSharp](../includes/btsvcsharp-md.md)]版本。 請確定指令碼執行原則已正確設定本主題中的需求 > 一節中所述。  
...
```  
 ...
 以下是範例輸出顯示接收埠和位置正在建立並刪除 PowerShell 指令碼執行：  
