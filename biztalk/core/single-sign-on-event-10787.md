---
title: 單一登入： 事件 10787 |Microsoft 文件
ms.custom: ''
ms.date: 06/08/2017
ms.prod: biztalk-server
ms.reviewer: ''
ms.suite: ''
ms.tgt_pltfrm: ''
ms.topic: article
ms.assetid: 9f50d8f2-01b1-40cc-ab1e-7d601e4793c8
caps.latest.revision: 6
author: MandiOhlinger
ms.author: mandia
manager: anneta
ms.openlocfilehash: 284c18dc6763f48e2a810553b46159e244e1a7dd
ms.sourcegitcommit: cb908c540d8f1a692d01dc8f313e16cb4b4e696d
ms.translationtype: MT
ms.contentlocale: zh-TW
ms.lasthandoff: 09/20/2017
ms.locfileid: "22276958"
---
# <a name="single-sign-on-event-10787"></a>單一登入： 事件 10787
## <a name="details"></a>詳細資料  
  
|||  
|-|-|  
|產品名稱|企業單一登入|  
|產品版本|[!INCLUDE[btsSSOVersion](../includes/btsssoversion-md.md)]|  
|事件識別碼|10787|  
|事件來源|ENTSSO|  
|元件|不適用|  
|符號名稱|ENTSSO_E_FLAGS_NOT_VALID|  
|訊息文字|指定的旗標無效或互不相容。 請參閱事件記錄檔 （在電腦 '%1'），如需詳細資訊。|  
  
## <a name="explanation"></a>說明  
 指定的旗標無效或互不相容。  
  
## <a name="user-action"></a>使用者動作  
 請檢查事件記錄檔以查看此應用程式的有效和無效的旗標，指定的電腦上和相關的錯誤。