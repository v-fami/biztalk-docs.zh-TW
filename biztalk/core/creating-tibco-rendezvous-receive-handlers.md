...
2.  以滑鼠右鍵按一下**接收連接埠**，指向**新增**，然後按一下**單向接收埠**。  
...
4.  在 **接收位置**頁面上，執行下列動作：  
    ...
    3.  從**型別**下拉式清單中，選取**BizTalkServerIsolatedHost**，以及從**接收處理常式**下拉式清單中，選取傳輸位址。  
...
2.  在 **保證接聽程式設定**，提供可重複使用的名稱與分類帳檔案名稱，如果您想要經認證的訊息。  
...
3.  在 **分散式佇列設定**、 分散式的佇列不是必要的如果不會變更項目。  
...
4.  依序展開**一般設定**並輸入 TIBCO Rendezvous 伺服器連線的所有必要的資訊。  
    ...
    |**不支援的型別對應至字串**|不支援的類型應產生錯誤或對應至字串。 如果使用配接器搭配已新增類型的較新版 TIBCO Rendezvous 時，就可使此項目。|  
...
5.  依序展開**Rendezvous 傳輸**，然後輸入 TIBCO Rendezvous 精靈和程式之間的通訊的所有必要的資訊。  
...
6.  提供使用單一登入 (SSO) 的認證。  
    ...
    1.  選取  **是**中**使用 SSO**使用單一登入。  
        ...
        > [!NOTE]
        >  請參閱[安全性](../core/security-in-biztalk-adapter-for-tibco-rendezvous.md)如需如何設定 SSO。  
    ...
    2.  從清單中選取一個分支機構應用程式。  
        ...
        > [!NOTE]
        >  如需有關如何建立分支機構應用程式的資訊，請參閱 <<c0> [ 建立分支機構應用程式](../core/creating-affiliate-applications1.md)。  
  
7.  提供所有必要的資訊後，按一下**套用**，然後按一下**確定**。  
