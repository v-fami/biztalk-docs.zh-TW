...
本章節提供一個集中的位置來保存使用 BizTalk Server 管理主控台時經常遇到之問題的相關資訊。  
 ...
 有下列已知的問題，除了[常見的問題和解決方式與 BizTalk Server 管理主控台](http://social.technet.microsoft.com/wiki/contents/articles/common-issues-and-resolutions-with-the-biztalk-server-administration-console.aspx)提供其他資訊。  
...
##### <a name="problem"></a>問題  
 將電腦重新開機與未設定為自動啟動 DTC 可能防止 BizTalk Server 服務啟動。  
 ...
 **交易 (處理序識別碼 95) 另一個處理序的鎖定資源上發生死結，並已被選為死結的犧牲者。請重新執行交易。**  
  
##### <a name="cause"></a>原因  
 這是非常罕見的情況下，執行另一位使用者遭到鎖定而無法管理資料庫中的一位使用者結果的系統管理作業中。  
  
##### <a name="solution"></a>方案  
 問題應該解決本身很快。 再試一次在幾分鐘的時間。  
  
#### <a name="sql-database-may-become-locked"></a>SQL database 可能會變成鎖定  
  
##### <a name="problem"></a>問題  
 使用者可能會被鎖定的 SQL database。 可能傳回不同的錯誤訊息數目。  
...
##### <a name="problem"></a>問題  
 若要從 BizTalk Server 管理主控台的多個服務執行個體的嘗試失敗，且會顯示類似下面的錯誤：  
  
 SQL Server 封鎖存取程序的元件 'Agent XPs' 的' sys.xp_sqlagent_enum_jobs'，因為此元件已關閉這部伺服器的安全性組態的一部分。  
  
> [!NOTE]
>  在多個 messagebox 環境中，就會發生這個問題。  
  
##### <a name="cause"></a>原因  
 在多個 messagebox 環境中會發生此問題，如果 SQL agent 作業 'Operations_OperateOnInstances_OnMaster_\<*dbName*\>' 次要 messagebox 資料庫上未執行。 必須執行此作業，才能傳播至主要 messagebox 資料庫從次要 messagebox 資料庫資訊。 這項作業將無法執行，如果未啟用，或登入失敗，就會發生。  
  
##### <a name="solution"></a>方案  
 如果您使用 BizTalk 管理主控台中同時執行多個服務執行個體上的作業，而且 BizTalk Server 環境設定具有多個 messagebox 資料庫，請確認 SQL Server Agent 作業名稱為 ' Operations_OperateOnInstances_OnMaster_\<*dbName*\>' 在所有的次要 (非 master) messagebox 資料庫上啟用。 此外，在裝載次要 messagebox 資料庫的 SQL Server 電腦上的 SQL Server Agent 服務必須執行包含 BTS_SQLAGENT_USER 資料庫角色的次要 messagebox 資料庫中的帳戶。  
...
2.  出現提示時選擇**伺服器類型**的**Database Engine**然後輸入或選取**伺服器名稱**裝載次要 messagebox 資料庫。  
  
3.  按一下以展開**資料庫**、 按一下以展開次要 messagebox 資料庫中，按一下以展開**安全性**，按一下以展開**角色**，按一下以展開**資料庫角色**，然後按兩下 BTS_SQLAGENT_USER 資料庫角色。  
...
> [!NOTE]
>  如果 SQL Server Agent 服務帳戶不是指定群組的成員則您必須將它新增至群組。  
...
##### <a name="resolution"></a>解決方案  
 如果您在 BizTalk 管理主控台中收到並行違規錯誤，請按一下更新 BizTalk 管理主控台的執行個體的快取**重新整理**BizTalk 管理主控台上的按鈕工具列或按下**F5**索引鍵。  
...
##### <a name="cause"></a>原因  
 BizTalk 管理主控台就會維持連接[!INCLUDE[btsBizTalkServerNoVersion](../includes/btsbiztalkservernoversion-md.md)]管理資料庫。 當連線[!INCLUDE[btsBizTalkServerNoVersion](../includes/btsbiztalkservernoversion-md.md)]管理資料庫容錯移轉期間已中斷，部分管理工作可能會傳回 「 無法連線 」 或 「 無法執行 」 錯誤，直到已關閉並重新開啟 BizTalk 管理主控台。  
  
##### <a name="resolution"></a>解決方案  
 關閉 BizTalk 管理主控台，並將它重新開啟。 重新開啟 BizTalk 管理主控台時它會建立新的連接，以指定[!INCLUDE[btsBizTalkServerNoVersion](../includes/btsbiztalkservernoversion-md.md)]管理資料庫。  
...
##### <a name="problem"></a>問題  
 在 [!INCLUDE[btsBizTalkServerNoVersion](../includes/btsbiztalkservernoversion-md.md)]管理主控台中，使用 32 位元唯一選取的選項 （預設值） 的電腦可能無法在 X64 上建立的 BizTalk 主控件執行個體。  
...
- MessageBox 資料庫包含未完成的執行個體。  
  ...
  刪除失敗時，如果快取重新整理間隔尚未過期，會出現下列錯誤訊息: 「 由於可能會那里剩餘 MessageBox 中的工作，無法刪除 MessageBox。 請確定 MessageBox 沒有未完成的執行個體，然後再試一次。"  
...
##### <a name="resolution"></a>解決方案  
 ...
 如果 MessageBox 資料庫包含未完成的服務執行個體，會出現下列錯誤訊息: 「 因為它可能仍包含未完成的執行個體，無法刪除 MessageBox。 請確定 MessageBox 沒有未完成的執行個體，然後再試一次。"  
  
 您可以使用 BizTalk Server 管理主控台中的 [群組中樞] 頁面，檢視 MessageBox 資料庫中未完成的服務執行個體。 如需檢視 [群組中樞] 頁面中的 [服務執行個體的狀態資訊，請參閱[搜尋追蹤的服務執行個體如何](../core/how-to-search-for-tracked-service-instances.md)。  
