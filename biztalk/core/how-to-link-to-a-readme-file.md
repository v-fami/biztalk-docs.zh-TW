...
本主題說明如何使用 BizTalk Server 管理主控台或命令列加入 Readme.htm 檔案，以便使用者按一下 [控制台] 中 [新增或移除程式] 的連結時會出現這個檔案。  
 ...
 安裝 BizTalk 應用程式時，會在 [控制台] 的 [新增或移除程式] 中自動建立名為 Readme.htm 檔案的連結。 如果 Readme.htm 檔案存在於應用程式的安裝資料夾，使用者可以開啟您的應用程式的名稱下按一下出現的"按一下這裡取得支援資訊"連結，然後按一下 "%BTAD_InstallDir%\Readme.htm"。 （%btad_installdir%是代表應用程式安裝資料夾的環境變數）。  
...
3.  展開**應用程式**和您要在其中新增 Readme.htm 檔案的應用程式。  
  
4.  以滑鼠右鍵按一下**資源**資料夾，指向**新增**，然後按一下 **資源**。  
  
5.  按一下**新增**，選取 [Readme.htm] 檔案，然後按**開啟**。  
  
6.  在**目的地位置**，，如下所示指定根據預設，應用程式安裝資料夾的完整路徑: %BTAD_InstallDir%\Readme.htm。 您不應該變更此路徑。 如果未提供應用程式安裝資料夾的正確路徑，安裝期間就不會將檔案複製到此位置，而且連結也不會作用。  
