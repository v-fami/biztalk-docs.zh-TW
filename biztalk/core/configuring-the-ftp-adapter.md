...
- FTP 配接器支援從唯讀的檔案位置下載檔案。 
- FTP 配接器支援不可部分完成檔案傳輸 ASCII 模式也。

請參閱[最佳作法和 FTP 配接器的建議](../core/best-practices-and-recommendations-for-the-ftp-adapter.md)。


## <a name="configure-the-receive-location"></a>設定接收位置

您可以設定 FTP 接收位置配接器屬性，BizTalk Server 管理主控台中。 如果屬性未設定接收位置中，則會使用 BizTalk Server 管理主控台中的預設接收處理常式值。  
  
> [!NOTE]
>  前先完成下列程序，您必須已經新增接收埠。 請參閱[如何建立接收埠](../core/how-to-create-a-receive-port.md)。  
...
4.  在**接收位置屬性**對話方塊中，於**傳輸**區段旁邊**類型**，選取**FTP**從下拉式清單和然後按一下 **設定**。  
...
5.  在**FTP 傳輸屬性**，執行下列動作：  
    ...
    **批次**
    ...
    |使用|動作|  
    |--------------|----------------|  
    |**檔案上限**|指定每個 BizTalk Server 批次的檔案數目上限。<br /><br /> 零 (0) 表示無限制。<br /><br /> **預設值：** 0|  
    ...
    **防火牆**
    ...
    |**模式**|指定配接器與 FTP 伺服器之間的連接模式。<br /><br /> **有效值：** Passive 和 Active<br /><br /> 在主動模式下，FTP 伺服器會連線到 FTP 配接器所開啟的連接埠。 在被動模式下，FTP 配接器會連線到 FTP 伺服器所開啟的連接埠。<br /><br /> **預設值：** Active|  
    |**密碼**|指定防火牆的密碼。|  
    |**[通訊埠]**|指定防火牆的連接埠。<br /><br /> **有效值：** 1 到 65535 之間 （含）<br /><br /> **預設值：** 21|  
    |**型別**|指定部署的防火牆類型。<br /><br /> **有效值：** 無、 Socks 4 和 Socks 5<br /><br /> **預設值：** 無|  
    |**使用者**|指定防火牆的使用者名稱。|  
    ...
    **FTP**
    ...
    |**FTP 伺服器類型** | 新開頭[!INCLUDE[bts2016_md](../includes/bts2016-md.md)]。 <br/><br/>若要選擇 FTP 伺服器不需要 SYST 命令中使用這個屬性。 選項為 None、 AIX、 偵測、 GXS、 MVS、 OS400，和其他。 <br/><br/>如果設定為**無**，使用系統命令。 **其他**的 OS 類型不符合任何指定的類別時，會使用。 <br /><br /> **預設值：** 無|    
    |**Log**|指定要用來儲存記錄檔複本的位置。 您可以使用此檔案來診斷透過 FTP 傳送或接收檔案時所發生的錯誤情況。|  
    |**檔案大小上限**|指定可下載的檔案大小上限，以 MB 為單位。<br /><br /> 零 (0) 表示檔案大小無限制。<br /><br /> **預設值：** 100|  
    ...
    **輪詢**
    ...
    |**啟用時間戳記比較**|指定配接器是否要根據修改的時間戳記，再次下載檔案。 如果配接器不具有 FTP 伺服器的刪除權限，MDTM (修改時間) 命令可讓配接器知道檔案自從上次下載後是否曾被修改。  根據此屬性的值，再次下載檔案。<br /><br /> **預設值：** 否**注意**： 如果 FTP 伺服器不支援 MDTM，設定**重新下載間隔**屬性。 **注意：** ，這個值時才適用**下載後刪除**設為 [否]。|  
    |**間隔**|指定輪詢此位置的時間間隔數。 若要連續輪詢，請將此值設為零 (0)。<br /><br /> **預設值：** 60|  
    |**重新下載間隔**|指定配接器再次下載檔案的間隔。 這個屬性是時才適用兩者**下載後刪除**和**啟用時間戳記比較**會設定為 [否]。<br /><br /> **預設值：** -1<br /><br /> -1 表示配接器將不會再次下載檔案。<br /><br /> 0 表示配接器會在每個輪詢循環下載檔案一次。|  
    |**單位**|指定的單位類型**間隔**和**重新下載間隔**屬性。<br /><br /> **有效值：** 秒、 分鐘、 小時及天<br /><br /> **預設值：** 秒|  
...
7.  在**接收位置屬性**對話方塊方塊中，輸入適當的值，以完成接收位置的組態，然後按一下 **確定**儲存設定。 如需有關資訊**接收位置屬性**對話方塊中，請參閱[如何建立接收位置](../core/how-to-create-a-receive-location.md)。  
...
>   
>  若要提供高可用性，為 ftp 接收配接器，您應該先設定 FTP 接收配接器以便在叢集 BizTalk 主控件執行個體中執行。 請參閱[執行叢集主控件中的配接器處理常式的考量](../core/considerations-for-running-adapter-handlers-within-a-clustered-host1.md)。  

## <a name="configure-the-send-port"></a>設定傳送埠

您可以在 BizTalk Server 管理主控台中設定 FTP 傳送埠配接器屬性。 如果未設定傳送埠的屬性的預設傳送處理常式會使用 BizTalk Server 管理主控台中的值。  
  
1.  在 BizTalk Server 管理主控台中，建立新的傳送埠，或按兩下現有的傳送埠進行修改。 請參閱[如何建立傳送埠](../core/how-to-create-a-send-port2.md)。 設定所有的傳送埠選項，然後在**傳輸**區段**一般**頁面上，指定**FTP**如**類型**選項。  
  
2.  在**一般**頁面上，於**傳輸**區段中，按一下**設定**旁邊**類型**。  
...
3.  在**FTP 傳輸屬性**，執行下列動作：  
    ...
    **防火牆**
    ...
    |--------------|----------------|    
    |**位址**|指定防火牆的位址，可指定 DNS 名稱或 IP 位址。|  
    |**模式**|選取配接器與 FTP 伺服器間的連接模式。<br /><br /> **有效值：** Passive 和 Active<br /><br /> 在主動模式下，FTP 伺服器會連線到 FTP 配接器所開啟的連接埠。 在被動模式下，FTP 配接器會連線到 FTP 伺服器所開啟的連接埠。<br /><br /> **預設值：** Active|  
    |**密碼**|指定防火牆的密碼。|  
    |**[通訊埠]**|指定防火牆的連接埠。<br /><br /> **有效值：** 1 到 65535 之間 （含） 之間<br /><br /> **預設值：** 21|  
    |**型別**|選取部署之防火牆的類型。<br /><br /> **有效值：** Socks 4、 Socks 5、 無<br /><br /> **預設值：** 無|  
    |**使用者**|指定防火牆的使用者名稱。|  
    ...
    **FTP**
    ...
    |**配置儲存區**|指定是否配置儲存空間給舊版主控件系統。 為回溯相容性提供此選項。<br /><br /> **有效值：** 否和是<br /><br /> **預設值：** 否|  
    ...
    **調整參數**
    ...
    |使用|動作|  
    |--------------|----------------| 
    |**連線限制**|指定伺服器最多可以開啟的並行 FTP 連線數目。 值為 0 時表示沒有限制。<br /><br /> **預設值：** 0**附註：** 這個屬性會取代舊版的 BizTalk Server 中使用來控管連接限制的登錄項目。 [!INCLUDE[btsBizTalkServerNoVersion](../includes/btsbiztalkservernoversion-md.md)]會忽略用來控制連接限制的登錄項目。|  
    |**暫存資料夾**|指定 FTP 伺服器上的暫存資料夾位置。 檔案會先上傳到此處，然後移至目的地 FTP 資料夾。 如果傳輸失敗，配接器會以 ASCII 傳輸模式重新開始上傳檔案，並繼續以二進位傳輸模式下載檔案。 **注意：** 如果檔案傳輸是不可部分完成的暫存位置與 FTP 伺服器上的相關位置之間，則檔案上傳也是不可部分完成。|  
...
FTP 配接器做為 FTP 用戶端運作，且可能需要下列的命令位於 FTP 伺服器，才能正確運作： 
 
|Command  |需要由接收  |所需的傳送  |
|---------|---------|---------|
|SYST | ✔ <br/><br/>選擇性開頭[!INCLUDE[bts2016_md](../includes/bts2016-md.md)] | ✔<br/><br/>選擇性開頭[!INCLUDE[bts2016_md](../includes/bts2016-md.md)] |
| 存放區 | |✔|
| RETR | ✔ | |
| 使用者 |✔ | ✔ |
| PASS | ✔ | ✔ |
...
-   [RFC 3659-ftp 的延伸模組](http://go.microsoft.com/fwlink/p/?LinkId=183155)(http://go.microsoft.com/fwlink/p/?LinkId=183155)  
  
## <a name="configuring-an-ftp-adapter-to-work-with-legacy-hosts"></a>設定 FTP 配接器與傳統主控件工作

本節說明您需要知道要促進 FTP 配接器和大型主機電腦之間的通訊。   
> [!NOTE]
...
-   **Quote 命令**： 使用 Quote 命令在遠端電腦上執行指令碼。 輸入 Quote 命令**之前取得**， **PUT 之前**，**之後收到**，和**After PUT**欄位上的任何端點。 請以下列格式輸入 Quote 命令：  
  
    ```  
    QUOTE RCMD <command to be run on the remote system>.  
    ```  
  
-   **整合檔案系統 (IFS)**: IFS 是 AS400 系統，可讓電腦為基礎的檔案，因此相同的命名慣例，做為電腦的儲存體中的區域。 若要使用 IFS 而不使用預設的「程式庫檔案系統」(Library File System)，要輸入的第一個命令是 `quote site namefmt 1`。 此命令告知 AS400 系統要使用 IFS 命名慣例。    
