---
title: "管理主要密碼 |Microsoft 文件"
ms.custom: 
ms.date: 06/08/2017
ms.prod: biztalk-server
ms.reviewer: 
ms.suite: 
ms.tgt_pltfrm: 
ms.topic: article
helpviewer_keywords:
- Master Secret server, managing
- managing [Master Secret server]
- SSO, Master Secret server
ms.assetid: f79e8f3f-c740-4ea1-9589-b42552fcd52d
caps.latest.revision: "10"
author: MandiOhlinger
ms.author: mandia
manager: anneta
ms.openlocfilehash: c2505fa6f5ec1abc04ded45e7701fd4e5212f889
ms.sourcegitcommit: cb908c540d8f1a692d01dc8f313e16cb4b4e696d
ms.translationtype: MT
ms.contentlocale: zh-TW
ms.lasthandoff: 09/20/2017
---
# <a name="managing-the-master-secret"></a>管理主要密碼
主要密碼是用以加密所有儲存在 SSO 資料庫中之資訊的金鑰。 若主要密碼伺服器失敗且密碼遺失，您將無法擷取儲存在 SSO 資料庫中的資訊。 因此，請務必在產生主要密碼之後立即備份。  
  
## <a name="in-this-section"></a>本節內容  
  
-   [如何產生主要密碼](../core/how-to-generate-the-master-secret.md)  
  
-   [如何備份主要密碼](../core/how-to-back-up-the-master-secret.md)  
  
-   [如何還原主要密碼](../core/how-to-restore-the-master-secret.md)  
  
-   [如何移動主要密碼伺服器](../core/how-to-move-the-master-secret-server.md)