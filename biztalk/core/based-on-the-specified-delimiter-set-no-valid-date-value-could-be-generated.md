...
|元件|EDI 引擎|  
|符號名稱|-|  
|訊息文字|根據指定的分隔符號集，無法產生沒有有效的日期值。 請使用另一個的分隔符號集。|  
  
## <a name="explanation"></a>說明  
 這個錯誤/警告/資訊事件表示，EDI 傳送管線無法產生有效的日期值，因為日期欄位的外寄交換中使用的字元是分隔符號字元相同。  
  
## <a name="user-action"></a>使用者動作  
 若要解決這個錯誤，變更分隔符號值用於 edi 傳送管線，建立一則訊息，如此分隔符號的字元將日期欄位的外寄交換中使用字元相同。 執行下列其中之一：  
