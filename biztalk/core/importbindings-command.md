...
|參數|Required|值|  
|---------------|--------------|-----------|  
|**-來源**(或**-因此**，請參閱 < 備註 >)|Required|要匯入的繫結檔案完整路徑 (包含檔案名稱)。 路徑如果包含空格，必須括在引號 (") 中。|  
...
| **-ImportTrackingSettings** | 選擇性 | 新開頭[!INCLUDE[bts2016_md](../includes/bts2016-md.md)]。 <br /><br />這會覆寫全域追蹤設定匯入選項。 "True"的值可讓追蹤設定匯入。 False 不允許追蹤設定匯入。 |
| **-ExcludeParties** | 選擇性 | 新開頭[!INCLUDE[bts2016_md](../includes/bts2016-md.md)]。 <br /><br />如果指定，它會排除合作對象資訊從繫結檔案。 |
