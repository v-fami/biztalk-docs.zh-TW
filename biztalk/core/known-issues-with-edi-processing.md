---
title: "EDI 處理的已知問題 |Microsoft 文件"
ms.custom: 
ms.date: 06/08/2017
ms.prod: biztalk-server
ms.reviewer: 
ms.suite: 
ms.tgt_pltfrm: 
ms.topic: article
ms.assetid: b5195457-4873-473f-9f13-6df2483ebe60
caps.latest.revision: "22"
author: MandiOhlinger
ms.author: mandia
manager: anneta
ms.openlocfilehash: 7a2bff7c8367b5471b5ba58e89cd7fd46c1bcada
ms.sourcegitcommit: cb908c540d8f1a692d01dc8f313e16cb4b4e696d
ms.translationtype: MT
ms.contentlocale: zh-TW
ms.lasthandoff: 09/20/2017
---
# <a name="known-issues-with-edi-processing"></a>EDI 處理的已知問題
本節包含的主題將說明 [!INCLUDE[btsBizTalkServerNoVersion](../includes/btsbiztalkservernoversion-md.md)] EDI 方案的已知問題。  
  
## <a name="in-this-section"></a>本節內容  
  
-   [已知的問題的 EDI 接收處理](../core/known-issues-with-edi-receive-processing.md)  
  
-   [已知的問題的 EDI 傳送處理](../core/known-issues-with-edi-send-processing.md)  
  
-   [EDI 通知的已知的問題](../core/known-issues-with-edi-acknowledgments.md)  
  
-   [EDI 批次處理的已知的問題](../core/known-issues-with-edi-batching.md)  
  
-   [EDI 合作對象的已知的問題](../core/known-issues-with-edi-parties.md)  
  
-   [EDI 驗證、 結構描述和訊息的已知的問題](../core/known-issues-with-edi-validation-schemas-and-messages.md)  
  
-   [EDI 解決方案搭配使用的 XML 工具的已知的問題](../core/known-issues-with-xml-tools-used-with-edi-solutions.md)  
  
## <a name="see-also"></a>另請參閱  
 [疑難排解 EDI 和 AS2 解決方案](../core/troubleshooting-edi-and-as2-solutions.md)   
 [EDI 解決方案架構](../core/edi-solution-architecture.md)   
 [開發和設定 BizTalk Server EDI 解決方案](../core/developing-and-configuring-biztalk-server-edi-solutions.md)