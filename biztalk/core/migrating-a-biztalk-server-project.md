...
|------------------------------------------------------------------------------------|---------------------------------------------------------------------------------------|  
|**內嵌追蹤資訊** 輸出組態屬性|**定義 TRACE 常數** 建置選項 **建置** 專案設計工具 索引標籤|  
|**產生偵錯資訊** 輸出組態屬性|**定義 DEBUG 常數** 建置選項 **建置** 專案設計工具 索引標籤|  
|**BPEL 規範** 程式碼產生組態屬性|**BPEL 規範** 程式碼產生專案的 [屬性] 視窗中的屬性|  
  
> [!NOTE]
>  BizTalk 專案現在已有兩個組建類型︰ **版本** 和 **偵錯**, ，取代了 **開發** 和 **部署** 的舊版本。 不過，您將會繼續看見**開發**和**部署**從移轉的專案組態[!INCLUDE[btsbiztalkserver2006r2](../includes/btsbiztalkserver2006r2-md.md)]。  
  
> [!CAUTION]
>  只有.btproj 和 \*。 選擇備份選項，在轉換期間備份.btproj.user 專案檔。 其他檔案必須手動備份。  
 ...
 **轉換專案檔案時發生錯誤。子項目\<BIZTALK\>項目的\<VisualStudioProject\>不正確。**  
...
> [!NOTE]
>  您可以轉換僅專案檔 (.btproj) 使用 Visual Studio 專案。