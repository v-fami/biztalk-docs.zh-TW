...
## <a name="insert-an-any-element-node-or-an-any-attribute-node"></a>插入 Any 項目節點或 Any 屬性 節點  
  
1.  在結構描述樹狀結構檢視中，選取**記錄**想要插入的節點**Any 項目**節點或**Any 屬性**節點。  
  
2.  在**BizTalk**功能表上，指向**插入結構描述節點**，然後按一下  **Any 項目**或**Any 屬性**視情況。  
  
> [!IMPORTANT]
>  設定**Process Contents**屬性**Lax**如**Any 項目**或**Any 屬性**節點可能無法正確運作。 通過驗證**Any 項目**或**Any 屬性**節點，將屬性設定為**略過**。  這個屬性的詳細[!INCLUDE[ui-guidance-developers-reference](../includes/ui-guidance-developers-reference.md)]。
>
>  若要建立包含對應**Any 項目**或**Any 屬性**節點，您必須使用[大量複製](mass-copy-functoid.md)運算質或[指令碼處理](scripting-functoid.md)運算質 （與內嵌 XSLT 或內嵌 XSLT 呼叫範本） 來執行這類節點中，對應或直接不對應這些節點。  
