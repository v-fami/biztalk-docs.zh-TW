---
title: "協調流程中使用訊息 |Microsoft 文件"
ms.custom: 
ms.date: 06/08/2017
ms.prod: biztalk-server
ms.reviewer: 
ms.suite: 
ms.tgt_pltfrm: 
ms.topic: article
helpviewer_keywords:
- orchestrations, messages
- messages, variables
- messages, about messages
ms.assetid: 6902b168-3567-44b7-b0ed-d983da54530f
caps.latest.revision: "16"
author: MandiOhlinger
ms.author: mandia
manager: anneta
ms.openlocfilehash: 5002351f3f73afbd4643744d482d49da49b2ca38
ms.sourcegitcommit: cb908c540d8f1a692d01dc8f313e16cb4b4e696d
ms.translationtype: MT
ms.contentlocale: zh-TW
ms.lasthandoff: 09/20/2017
---
# <a name="using-messages-in-orchestrations"></a>在協調流程中使用訊息
訊息是協調流程中的基本通訊單位。 它包含一或多個部分，以及描述訊息屬性和訊息內容的內容資料。  
  
## <a name="in-this-section"></a>本節內容  
  
-   [如何新增或移除協調流程中的訊息](../core/how-to-add-or-remove-messages-in-orchestrations.md)  
  
-   [建構訊息](../core/constructing-messages.md)  
  
-   [使用辨別的欄位和屬性欄位](../core/using-distinguished-fields-and-property-fields.md)  
  
-   [如何使用多部分訊息類型](../core/how-to-use-multi-part-message-types.md)  
  
-   [如何使用訊息內容屬性](../core/how-to-use-message-context-properties.md)