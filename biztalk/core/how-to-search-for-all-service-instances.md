...
您可以使用**查詢** 索引標籤，在 BizTalk Server 管理主控台中搜尋所有服務執行個體。 若有任何執行中或擱置的執行個體，就無法取消登錄特定服務類型。 例如，取消登錄特定服務類型之前，可以搜尋所有的服務執行個體，以確保沒有執行中或擱置的執行個體。  
...
4.  在**查詢運算式**群組中**值**欄中，選取**所有進行中的服務執行個體**從下拉式清單方塊。  
