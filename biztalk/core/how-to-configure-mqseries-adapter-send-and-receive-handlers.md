...
您可以透過 [BizTalk Server 管理] 主控台，為 MQSeries 配接器的傳送與接收處理常式設定一些屬性。 此程序中所述[如何設定 MQSeries 配接器接收位置和傳送埠](../core/how-to-configure-mqseries-adapter-receive-locations-and-send-ports.md)建立大多數傳送處理常式屬性的值。  
...
2. 在 BizTalk Server 管理主控台中，依序展開**BizTalk Server 管理]**，展開**BizTalk 群組**，按一下以展開**平台設定**，然後按一下 [以依序展開**配接器**。  
