...
---
# <a name="walkthrough-as2-sending-edi-over-as2-with-an-asynchronous-mdn"></a>逐步解說 (AS2)：使用非同步 MDN 透過 AS2 傳送 EDI
本逐步解說提供一組逐步執行的程序，建立使用非同步 MDN 透過 AS2 傳送 EDI 訊息的解決方案。  您可以建立並測試完整的解決方案，在單一電腦上的這個逐步解說  
...
## <a name="how-the-solution-sends-an-edias2-message-and-returns-an-asynchronous-mdn"></a>解決方案傳送 EDI/AS2 訊息並傳回非同步 MDN 的方式  
 解決方案會執行下列作業：  
...
2. 藉由使用通過接收管線，接收埠將測試訊息保留原狀置放在 MessageBox 中。  
  
3. 靜態單向傳送埠取用 EDI 交換，並將其編碼為 AS2 格式。  
  
4. 傳送埠透過 AS2 傳輸，將 EDI 交換傳送到 Fabrikam 合作對象。  
  
5. Fabrikam 的單向接收埠使用 Fabrikam 虛擬目錄接收 AS2 訊息。 接收管線會解碼來自 AS2 的 EDI 交換，然後將 EDI 交換置放在 MessageBox 中。  
  
6. 同一個單向接收埠還會產生 MDN，然後將 MDN 置放在 MessageBox 中。  
  
7. 靜態單向傳送埠取用 EDI 訊息。  
  
8. 靜態單向傳送埠將 EDI 訊息傳送至本機資料夾。  
  
9. 單向動態傳送埠取用非同步 MDN。  
  
10. 單向動態傳送埠將 MDN 傳送至 Contoso。  
  
11. 單向接收埠接收 MDN，然後將 MDN 置放在 MessageBox 中。  
  
12. 具有通過傳送管線的靜態單向傳送埠會拾取該 MDN。  
  
13. 單向傳送埠會將 MDN 傳送至本機資料夾。  
  
    下圖顯示這個解決方案的架構。  
  
    ![使用非同步 MDN 的 AS2 傳送](../core/media/343bbfcc-5387-426c-8700-db9738816218.gif "343bbfcc-5387-426c-8700-db9738816218")  
...
- 指定 Fabrikam 和 Contoso 虛擬目錄不是由 Windows SharePoint Services 管理。  
  
- 建立單向 FILE 接收埠，接收要透過 AS2 傳輸來傳送的 EDI 測試訊息。 建立本機資料夾，接收測試訊息。  
  
- 建立靜態單向 HTTP 傳送埠，讓 [!INCLUDE[btsBizTalkServerNoVersion](../includes/btsbiztalkservernoversion-md.md)] 傳送包含 EDI 商業文件的 AS2 訊息給 Fabrikam。 將傳送管線設定為 AS2Send 管線。  
  
- 建立單向 HTTP 接收埠，讓 [!INCLUDE[btsBizTalkServerNoVersion](../includes/btsbiztalkservernoversion-md.md)] 接收 AS2 訊息。 請將接收管線設定為 AS2Receive 管線。 設定接收位置，以透過 Fabrikam 虛擬目錄接收 AS2 訊息。  
...
- 建立動態單向 HTTP 傳送埠，將 MDN 傳回 Contoso。  
  
  > [!NOTE]
  >  測試解決方案位於單一電腦上，因此，用於傳送 AS2 訊息 (來自 Contoso) 的單向傳送埠和用於傳送 MDN 訊息 (來自 Fabrikam) 的單向傳送埠皆位於相同電腦上。  
  
- 建立單向接收埠，讓 [!INCLUDE[btsBizTalkServerNoVersion](../includes/btsbiztalkservernoversion-md.md)] 接收來自 Fabrikam 的 MDN 回應。  
...
7. 在 [IIS 管理員] 中，開啟**站台**資料夾。 使用滑鼠右鍵按一下 [預設的網站]，然後選取 [新增應用程式]。  
...
13. 在 **驗證**，選取**匿名驗證**，並確認**狀態**是**已啟用**。 如果**狀態**是**停用**，按一下 **啟用**中**動作**窗格。  
  
##### <a name="to-configure-the-contoso-web-page"></a>若要設定 Contoso 網頁  
  
1. 在 [IIS 管理員] 中，開啟**站台**資料夾。 使用滑鼠右鍵按一下 [預設的網站]，然後選取 [新增應用程式]。  
  
2. 在 **新增應用程式**對話方塊方塊中，輸入**Contoso**中**別名**，然後按一下**選取**。  
  
3. 在 **選取應用程式集區** 對話方塊中，選取**BizTalkAppPool** ，按一下  **確定**。  
  
4. 按一下省略符號 （...） 按鈕並瀏覽至[!INCLUDE[btsBiztalkServerPath](../includes/btsbiztalkserverpath-md.md)]針對 HttpReceive**實體路徑**。  
  
5. 按一下 [**測試設定**，並確認沒有顯示在錯誤**測試連接**] 對話方塊。 按一下 [關閉]，然後按一下 [確定]。  
  
6. 在 [IIS 管理員] 中，選取 Contoso 虛擬目錄並在**功能檢視**，按兩下**驗證**。  
  
7. 在 **驗證**，選取**匿名驗證**，並確認**狀態**是**已啟用**。 如果**狀態**是**停用**，按一下 **啟用**中**動作**窗格。  
  
##### <a name="to-specify-that-the-fabrikam-and-contoso-virtual-directories-are-not-managed-by-windows-sharepoint-services"></a>若要指定 Fabrikam 和 Contoso 虛擬目錄不是由 Windows SharePoint Services 管理  
...
7. 在 [檔案遮罩] 中，輸入檔案副檔名。 如果您使用 SamplePO.txt 檔案做為測試訊息，請輸入 **\*.txt**。 按一下 [確定] 。  
  
8. 針對**接收管線**，接受預設值是**PassThruReceive**。  
  
9. 按一下  **確定**，然後按一下**確定**一次。  
  
10. 按一下 **接收位置**節點，以滑鼠右鍵按一下您接收位置，然後按一下**啟用**。  
  
##### <a name="to-create-a-send-port-to-send-the-edi-interchange-over-as2-to-fabrikam"></a>若要建立傳送埠以透過 AS2 將 EDI 交換傳送至 Fabrikam  
...
2. 在 [**傳送埠屬性**] 對話方塊中，將傳送埠命名為**SendISAToFab**。  
...
7. 在主控台樹狀目錄中，選取**篩選器**。 針對**屬性**，輸入**BTS。ReceivePortName**; 對於**運算子**，輸入**==**; 及針對**值**輸入將接收 EDI 接收埠的名稱交換 (`RecvISAFromCont`)。  
  
8. 按一下 [確定] 。  
  
9. 按一下 **傳送埠**節點在管理主控台中，以滑鼠右鍵按一下您的傳送埠，然後**開始**。  
  
##### <a name="to-create-a-receive-port-for-fabrikam-to-receive-the-as2-message"></a>若要為 Fabrikam 建立接收埠以接收 AS2 訊息  
  
1. 在 [!INCLUDE[btsBizTalkServerNoVersion](../includes/btsbiztalkservernoversion-md.md)]管理主控台中的，在 [BizTalk Application 1] 節點中，以滑鼠右鍵按一下**接收埠**，指向**新增**，然後按一下 **單向接收埠**.  
  
2. 接收埠命名為**RecvAS2ForFabrikam**，然後按一下**接收位置**在主控台樹狀目錄中。  
...
6. 選取  **AS2Receive** for**接收管線**。 按一下  **確定**，然後按一下**確定**一次。  
...
3. 在 **傳送埠屬性**對話方塊中，將傳送埠作為**SendEDIToFab**。 選取 **檔案**for**型別**，然後按一下**設定**。  
  
4. 在 [ **FILE 傳輸屬性**] 對話方塊中，如**目的地資料夾**，輸入您建立之 EDI 內容的本機資料夾。  
  
5. 針對**檔案名**，輸入檔案名稱。 如果您使用 SamplePO.txt 檔案做為測試訊息，請輸入 **%MessageID%.txt**。 按一下 [確定] 。  
  
6. 接受預設值是**PassThruTransmit** for**傳送管線**。  
  
7. 按一下 **篩選器**在主控台樹狀結構，並加入挑選 EDI 內容的篩選屬性。 在第一行，如**屬性**，輸入**BTS。ReceivePortName**; 對於**運算子**，輸入**==**; 對於**值**，輸入接收的 AS2 接收埠的名稱訊息 (`RecvAS2ForFabrikam`); 至於**分組**，接受**和**。 在第二行，如**屬性**，輸入**EdiIntAS.IsAS2PayloadMessage**; 對於**運算子**，輸入**==**; 以及**值**，輸入 **，則為 True**。  
  
8. 按一下 [確定] 。  
  
9. 按一下 **傳送埠**節點，以滑鼠右鍵按一下您的傳送埠，然後**開始**。  
  
##### <a name="to-create-a-dynamic-one-way-send-port-to-return-the-mdn"></a>若要建立動態單向傳送埠以傳回 MDN  
  
1. 在 [!INCLUDE[btsBizTalkServerNoVersion](../includes/btsbiztalkservernoversion-md.md)]管理主控台中，以滑鼠右鍵按一下**傳送埠**，指向**新增**，然後按一下 **動態單向傳送埠**。  
  
2. 在 **傳送埠屬性**對話方塊中，將傳送埠作為**Send_MDN**。  
  
3. 針對**傳送管線**，請輸入 AS2Send。  
  
4. 按一下 **篩選器**在主控台樹狀結構，並加入挑選 EDI 內容的篩選屬性。 在第一行，如**屬性**，輸入**BTS。ReceivePortName**; 對於**運算子**，輸入**==**; 對於**值**，輸入接收的 AS2 接收埠的名稱訊息 (`RecvAS2ForFabrikam`); 至於**分組**，接受**和**。 在第二行，如**屬性**，輸入**EdiIntAS.IsAS2AsynchronousMDN**; 對於**運算子**，輸入**==**; 以及**值**，輸入 **，則為 True**。  
  
5. 按一下 [確定] 。  
  
6. 按一下 **傳送埠**節點，以滑鼠右鍵按一下您的傳送埠，然後**開始**。  
  
##### <a name="to-create-a-receive-port-to-receive-the-mdn-from-fabrikam"></a>若要建立接收埠以接收來自 Fabrikam 的 MDN  
  
1. 在 [!INCLUDE[btsBizTalkServerNoVersion](../includes/btsbiztalkservernoversion-md.md)]管理主控台中的，在 [BizTalk Application 1] 節點中，以滑鼠右鍵按一下**接收埠**，指向**新增**，然後按一下 **單向接收埠**.  
  
2. 接收埠命名為**RecvMDNFromFab**，然後按一下**接收位置**在主控台樹狀目錄中。  
  
3. 按一下 **[新增]**。  
  
4. 中**接收位置屬性**對話方塊中，名稱您接收位置中，選取**HTTP**如**型別**，然後按一下**設定**。  
  
5. 在  **HTTP 傳輸屬性**對話方塊方塊中，輸入 **/Contoso/BTSHTTPReceive.dll** for**虛擬目錄加 ISAPI 延伸模組**。 清除**成功時傳回相互關聯控制代碼**，然後選取**擱置失敗的要求**。 按一下 [確定] 。  
...
3. 在 **傳送埠屬性**對話方塊方塊中，將傳送埠命名。 選取 **檔案**for**型別**，然後按一下**設定**。  
...
7. 按一下 **篩選器**在主控台樹狀目錄中。 針對**屬性**，輸入**BTS。ReceivePortName**; 對於**運算子**，輸入**==**; 對於**值**，輸入接收埠接收 MDN (名稱`RecvMDNFromFab`);至於**分組**，接受**和**。 在第二行中，如**屬性**，輸入**EdiIntAS.IsAS2MdnResponseMessage**。 針對**運算子**，輸入**==**。 針對**值**，輸入 **，則為 True**。  
...
6.  在上執行下列工作**Contoso-> Fabrikam**  索引標籤。  
    ...
    2.  在 **通知 (Mdn)** 頁面上，執行下列動作：  
        ...
        5.  在 **回條傳遞選項 (URL)**，輸入**http://localhost/Contoso/BTSHttpReceive.dll**。  
  
        6.  **配置通知來**預設會設定為您指定**回條傳遞選項 (URL)** 屬性。 在處理 AS2 的過程中並不會使用這個欄位的值。  
  
    3.  在 **傳送埠**頁面上，建立將用來傳送 EDI 交換給 Fabrikam 的雙向傳送埠的關聯。 在 **傳送埠**格線底下**名稱**資料行中，按一下空白儲存格，並從下拉式清單中，選取 傳送埠**SendISAToFab**。  
...
2. 貼上**SamplePO.txt**檔案放置於您要建立從 Contoso 接收測試訊息的本機資料夾。  
  
3. 移至您建立用來傳送 EDI 內容的本機資料夾。 確認該資料夾包含 EDI 檔案。 開啟檔案和原始測試訊息，並確認其內容是相同的。  
  
4. 移至您建立用來做為結果 MDN 之傳送目標的本機資料夾。 確認該資料夾包含測試檔案、開啟該檔案並確認是否為 MDN 檔案。  
