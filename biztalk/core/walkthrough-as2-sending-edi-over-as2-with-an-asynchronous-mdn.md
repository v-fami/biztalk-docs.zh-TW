...
本逐步解說提供一組逐步執行的程序，建立使用非同步 MDN 透過 AS2 傳送 EDI 訊息的解決方案。  您可以建立並測試在單一電腦上的本逐步解說的完整方案  
...
2.  在**新增應用程式集區**對話方塊方塊中，輸入**BizTalkAppPool**中**名稱**，然後選取 **.NET Framework v4.0.30210**中 **.NET framework 版本**下拉式清單中。 按一下 **[確定]**。  
...
7.  在 [IIS 管理員] 中，開啟**網站**資料夾。 使用滑鼠右鍵按一下 [預設的網站]，然後選取 [新增應用程式]。  
...
13. 在**驗證**，選取**匿名驗證**並確認**狀態**是**啟用**。 如果**狀態**是**已停用**，按一下 **啟用**中**動作**窗格。  
  
##### <a name="to-configure-the-contoso-web-page"></a>若要設定 Contoso 網頁  
  
1.  在 [IIS 管理員] 中，開啟**網站**資料夾。 使用滑鼠右鍵按一下 [預設的網站]，然後選取 [新增應用程式]。  
  
2.  在**新增應用程式**對話方塊方塊中，輸入**Contoso**中**別名**，然後按一下 **選取**。  
  
3.  中**選取應用程式集區**對話方塊中，選取**BizTalkAppPool**按一下**確定**。  
  
4.  按一下省略符號 （...） 按鈕並瀏覽至[!INCLUDE[btsBiztalkServerPath](../includes/btsbiztalkserverpath-md.md)]HttpReceive 的**實體路徑**。  
...
7.  在 [檔案遮罩] 中，輸入檔案副檔名。 如果您使用 SamplePO.txt 檔案做為測試訊息，輸入 **\*.txt**。 按一下 **[確定]**。  
...
2.  在**傳送埠屬性**對話方塊中，將傳送埠為**SendISAToFab**。  
...
7.  在主控台樹狀目錄中，選取**篩選**。 如**屬性**，輸入**BTS。ReceivePortName**;**運算子**，輸入 **==** ; 以及**值**輸入將接收 EDI 接收埠的名稱交換 (`RecvISAFromCont`)。  
...
6.  選取**AS2Receive**如**接收管線**。 按一下**確定**，然後按一下 **確定**一次。  
...
3.  在**傳送埠屬性**對話方塊中，將傳送埠為**SendEDIToFab**。 選取**檔案**如**類型**，然後按一下 **設定**。  
...
7.  按一下**篩選**在主控台樹狀結構，並加入挑選 EDI 內容的篩選條件屬性。 在第一行，如**屬性**，輸入**BTS。ReceivePortName**;**運算子**，輸入 **==** ;**值**，輸入接收 AS2 的接收埠名稱訊息 (`RecvAS2ForFabrikam`); 以及**分組**，接受**和**。 在第二行，如**屬性**，輸入**EdiIntAS.IsAS2PayloadMessage**;**運算子**，輸入 **==** ; 以及**值**，輸入**True**。  
...
2.  在**傳送埠屬性**對話方塊中，將傳送埠為**Send_MDN**。  
  
3.  如**傳送管線**，請輸入 AS2Send。  
  
4.  按一下**篩選**在主控台樹狀結構，並加入挑選 EDI 內容的篩選條件屬性。 在第一行，如**屬性**，輸入**BTS。ReceivePortName**;**運算子**，輸入 **==** ;**值**，輸入接收 AS2 的接收埠名稱訊息 (`RecvAS2ForFabrikam`); 以及**分組**，接受**和**。 在第二行，如**屬性**，輸入**EdiIntAS.IsAS2AsynchronousMDN**;**運算子**，輸入 **==** ; 以及**值**，輸入**True**。  
  
5.  按一下 **[確定]**。  
  
6.  按一下**傳送埠** 節點，以滑鼠右鍵按一下傳送埠，然後**啟動**。  
...
3.  在**傳送埠屬性**對話方塊方塊中，將傳送埠命名。 選取**檔案**如**類型**，然後按一下 **設定**。  
...
7.  按一下**篩選**在主控台樹狀目錄中。 如**屬性**，輸入**BTS。ReceivePortName**;**運算子**，輸入 **==** ;**值**，輸入接收埠接收 MDN (名稱`RecvMDNFromFab`);和**分組**，接受**和**。 第二行，如**屬性**，輸入**EdiIntAS.IsAS2MdnResponseMessage**。 如**運算子**，輸入 **==** 。 如**值**，輸入**True**。  
...
6.  在上執行下列工作**Contoso-> Fabrikam**  索引標籤。  
    ...
    2.  在**通知 (Mdn)** 頁面上，執行下列動作：  
        ...
        6.  **配置通知至**預設將設定為您指定**回條傳遞選項 (URL)** 屬性。 在處理 AS2 的過程中並不會使用這個欄位的值。  
  
    3.  在**傳送埠**頁面上，將會傳送 EDI 交換給 Fabrikam 的雙向傳送埠產生關聯。 在**傳送埠**方格下方**名稱**資料行，按一下空白儲存格，然後從下拉式清單中，選取 傳送埠**SendISAToFab**。  
