...
## <a name="what-this-sample-does"></a>此範例的用途  
 這個範例會示範使用 SSO 和 BizTalk HTTP 傳送和接收配接器; 來模擬 SSO 如何允許使用者不需提供額外的認證，Windows 驗證它們之後，登入非 Microsoft Windows 系統的端對端案例。  

 此範例還會使用 SSO 管理和對應模組，建立使用 SSO 之 interop 用戶端 DLL 的分支機構應用程式和 SSO 對應。  

 此範例將示範透過實作下列各方面的端對端實例使用 SSO：  

- 設定為使用 Windows 整合式的驗證的 Microsoft Internet Information Services (IIS) 虛擬目錄所代表的使用者介面。  
...
- SQL 範例資料庫 Northwind 所代表的後端資料庫。  
  ...
  有效使用此範例的前提為，您具備 SSO 和 BizTalk HTTP 配接器的足夠知識。 例如，您應該了解什麼是 SSO 內容中的分支機構應用程式。 如需這些主題的資訊，請參閱 <<c0> [ 使用 SSO](../core/using-sso.md)。 另請參閱[HTTP 配接器](../core/http-adapter.md)。  

  在您完成設定之後，此範例的運作方式如下：  

1. 當您按一下 **完成**在包含此範例中的使用者介面的精靈應用程式，Internet Explorer 的執行個體啟動並傳遞 BizTalk HTTP 接收 DLL 的 URL。  
...
|SsoSample.cs|構成此範例大部分的 HTTP SSO 圖形應用程式的最上層 Microsoft Visual C# 原始程式檔。 此檔案包含名為類別中的 SSO 組態程式碼**SsoConfigurator** ，最後是本範例的重點。|  
...
2. 使用 Microsoft SQL Server Enterprise Manager，開啟**員工**Northwind 範例資料庫中資料表，然後再加入對應至您剛剛建立，包括各種範例資料的本機 Windows 帳戶的資料列資料行。 您新增至 [員工] 資料表中 [姓氏] 資料欄的值，必須與您在步驟 1 中新增的本機 Windows 帳戶的使用者名稱相同。  
...
2. 在 歡迎使用 頁面上，接受預設設定來設定 IIS、 SSO 和 BizTalk，然後**下一步**。  

3. 在 IIS 組態 頁面上，接受預設設定的兩個 IIS 虛擬目錄來建立，然後按一下您想**下一步**。  

4. 在 SSO 組態 頁面中，接受預設設定分支機構應用程式，也就是可使用**將應用程式加入** 按鈕。  

5. 在 SSO 組態 頁面上，接受 使用者對應的預設設定的大部分可使用**新增對應** 按鈕。 根據建置和啟始此範例時新增的本機 Windows 帳戶，為下面兩個設定提供值。  
...
6. 在 [SSO 組態] 頁面中，按一下 [**下一步]**。  

7. 在 BizTalk 組態 頁面中，接受預設設定為傳送和接收連接埠，並依此類推，，然後按一下**下一步**。  

   > [!NOTE]
   >  您可以變更預設的傳送埠設定，從**EmployeeData.aspx**要**ValidateUser.aspx**如果您想要查看簡單的使用者驗證，而不是從的 [員工] 資料表提取的資料範例Northwinds SQL 資料庫。 進行這項變更會改變的輸出會顯示在瀏覽器中按一下後**完成**在步驟 9。  
