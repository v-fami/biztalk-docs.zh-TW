...
2.  在 **EDIFACT 後援設定** 對話方塊中，於 **EDIFACT 協議頁面** 索引標籤中，於 **交換設定** 區段中，按一下 **信封**。  
  
3.  如 **處理優先順序代碼 (UNB8)**, ，輸入最少一個字元，最多一個字元的字母值。 這是選擇性欄位。  
...
7.  選取 **套用 UNG 區段 （功能群組標頭）** 外寄訊息中的功能群組標頭中建立群組區段。  
  
8.  按一下  **套用** 接受變更，再繼續進行設定，或按一下 **確定** 來驗證變更，然後關閉對話方塊。  
