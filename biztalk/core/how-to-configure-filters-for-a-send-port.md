...
3. 依序展開**傳送埠**，以滑鼠右鍵按一下傳送埠，按一下**屬性**，然後按一下**篩選**。  
...
4. 下表中所述，設定篩選器，然後按一下**確定**。  
   ...
   |**[運算子]**|輸入或選取運算式的運算子。|  
   |**ReplTest1**|輸入值以驗證屬性。 接受的值類型依屬性類型的不同而不同。 若要查看某個屬性接受的值類型，請將滑鼠停留在該屬性上方。 可接受的值如下：Int: (Integer) 這必須是整數值。 String：字元字串。 dateTime：以 .NET 支援的格式表示的日期和 (或) 時間。 如需有關 .NET 所支援的時間格式詳細資訊，請參閱「.NET Frameworks 說明」中的＜DateTimeFormatInfo 類別＞。|  
   |**群組依據**|選取**並**或是**或**表示這與其他篩選條件運算式之間的關聯性。|  
