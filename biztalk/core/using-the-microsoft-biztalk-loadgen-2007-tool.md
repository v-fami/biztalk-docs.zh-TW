...
> [!NOTE]
>  下載[LoadGen](https://www.microsoft.com/download/details.aspx?id=14925)。 
  
 此工具只能用於測試環境，不應使用於生產環境。 此工具是以「現況」提供，所以並不支援。  
  
> [!NOTE]
>  您可以使用 BizTalk Server 負載產生工具搭配 MSMQ 配接器，但您必須執行一些額外的步驟。 如需指示，請參閱[搭配 MSMQ 使用 LoadGen 2007](../core/using-loadgen-2007-with-msmq.md)。  
  
> [!NOTE]
>  在 64 位元電腦上可能不支援 LoadGen。 您可以使用在 32 位元遠端電腦上執行的 LoadGen，針對 64 位元伺服器產生負載。  
