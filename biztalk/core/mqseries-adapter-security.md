...
MQSeries 配接器安全性是從保護 BizTalk 與 MQSeries 伺服器的安全開始。 如需保護 BizTalk Server 的資訊，請參閱[安全和保護資料](secure-and-protect-your-biztalk-messages.md)。 如需 MQSeries Server 安全性的詳細資訊，請參閱 IBM MQSeries Server 文件。  
...
-   適當地使用**SSO 分支機構應用程式**屬性  
 ...
 如果您的應用程式使用 MQCIH 與 MQIIH 標頭屬性將使用者認證放在輸出訊息，請**SSO 分支機構應用程式**屬性**傳輸屬性**頁面。 如需有關這個屬性的詳細資訊，請參閱[如何設定 MQSeries 配接器接收位置和傳送埠](../core/how-to-configure-mqseries-adapter-receive-locations-and-send-ports.md)。  
