...
## <a name="how-an-activity-search-works"></a>活動搜尋如何運作  
 活動搜尋讓您能夠依據 BAM 檢視所提供的追蹤值和項目指定搜尋準則，針對 BAM 資料執行搜尋以尋找符合條件的活動，並顯示活動讓您進行編輯或依據活動建立警示。  
  
 您可以儲存和重複使用為了搜尋所建立的查詢。 您也可以使用查詢做為警示的基礎，例如，「收到特定客戶的訂單時通知我」。 請使用內容頁面上方的按鈕來儲存、開啟和執行查詢，以及設定警示。  
  
 活動搜尋可以讓您找到所要監控的活動。 一旦建立了活動搜尋，您即可使用該搜尋來建立警示以通知您感興趣的事件。  
