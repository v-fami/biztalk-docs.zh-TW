...
使用 MMC 嵌入式管理單元或命令列，可以啟用整個企業單一登入 (SSO) 系統。  
 ...
 若您要設定 SSO 系統中的分支機構應用程式與對應，則還需建立分支機構應用程式。 在 SSO 分支機構系統管理員建立分支機構應用程式之後，應用程式系統管理員可將它變更，而應用程式使用者 (一般使用者) 可建立他們自己的對應。 如需詳細資訊，請參閱[管理分支機構應用程式](../core/managing-affiliate-applications.md)和[管理使用者對應](../core/managing-user-mappings.md)。  
...
6.  型別**ssomanage createapps *\<應用程式檔案 >*** 建立分支機構應用程式，其中\<應用程式檔案 > 是包含定義的 XML 檔案分支機構應用程式中。  
