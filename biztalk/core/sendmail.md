...
2. [!INCLUDE[btsBizTalkServerNoVersion](../includes/btsbiztalkservernoversion-md.md)]協調流程升級**PONumber**並**電子郵件**日後更容易存取的屬性。  
...
|                   Setup.bat                    | 建置並初始化此範例。 **注意：** 此安裝程式檔案會建立並繫結連接埠，並於 SDK 範例，使用不同的機制，比大多數的安裝程式檔案。 並不需要附贈 .xml 檔案。 |
...
4. 使用記事本之類的程式修改檔案 SendMailInput.xml，讓**電子郵件**項目會指定要接收電子郵件訊息，此範例所產生的合法電子郵件地址。  

5. 按一下 **開始**，指向**程式**，指向[!INCLUDE[btsBizTalkServerStartMenuItemui](../includes/btsbiztalkserverstartmenuitemui-md.md)]，然後按一下**BizTalk Server 管理**。  

6. 在  **BizTalk Server 管理**主控台中，展開 BizTalk 群組樹狀結構。  

7. 依序展開**平台設定**樹狀目錄中的左窗格中。  

8. 依序展開**配接器**資料夾中，按一下**SMTP**節點，然後再按兩下右窗格中的資料列的 SMTP 配接器。  

9. 在 [ **SMTP-配接器處理常式屬性**] 對話方塊中，按一下**屬性**。  

10. 在  **SMTP 傳輸屬性**對話方塊的 **屬性**索引標籤上，提供適當的值，如**SMTP 伺服器名稱**和 **（電子郵件地址）** 屬性，然後再按一下**確定**。  
