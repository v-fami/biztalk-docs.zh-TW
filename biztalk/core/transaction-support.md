...
3. 建立**DataConnection**使用您在步驟 1 和 2 中建立的連接和交易物件的物件。  
...
6. 現在，叫用**認可**或是**Rollback**資料連線物件上根據您自己的邏輯。  
...
-   您也可以使用**OleDbConnection**並**OleDbTransaction**物件，而不要使用**SqlConnection**並**SqlTransaction**若要以交易方式執行資料庫更新的物件。  
  
-   從原則執行的所有修改作業都是在記憶體中完成的。 您必須叫用**更新**方法**DataConnection**更新資料庫的物件。  
  
-   您可以認可，或藉由叫用回復交易**認可**或是**Rollback**方法**DataConnection**分別物件。