---
title: "在協調流程中使用運算式 |Microsoft 文件"
ms.custom: 
ms.date: 06/08/2017
ms.prod: biztalk-server
ms.reviewer: 
ms.suite: 
ms.tgt_pltfrm: 
ms.topic: article
ms.assetid: 1947cd39-6ef2-4b2d-afeb-a0132b19db97
caps.latest.revision: "10"
author: MandiOhlinger
ms.author: mandia
manager: anneta
ms.openlocfilehash: 880e1ee08a232aca3a04763c5d5c1797fd238fbe
ms.sourcegitcommit: cb908c540d8f1a692d01dc8f313e16cb4b4e696d
ms.translationtype: MT
ms.contentlocale: zh-TW
ms.lasthandoff: 09/20/2017
---
# <a name="using-expressions-in-orchestrations"></a>在協調流程中使用運算式
您可以使用「BizTalk 運算式編輯器」輸入 XLANG/s 運算式，以新增邏輯來處理和測試協調流程變數和訊息的值。 但是，這種方式比較不適合用來執行高階協調流程邏輯，比較好的方式是透過協調流程繪圖本身來呈現高階協調流程邏輯。 為了使商務程序透明化且便於重新設定，建議您使用簡單且模組化的運算式。  
  
## <a name="in-this-section"></a>本節內容  
 [運算式的需求與限制](../core/requirements-and-limitations-for-expressions.md)  
  
 [採用運算式的圖形](../core/shapes-that-take-expressions.md)  
  
 [如何建立運算式](../core/how-to-create-expressions.md)  
  
 [在運算式中使用運算子](../core/using-operators-in-expressions.md)  
  
 [如何使用運算式執行訊息指派](../core/how-to-use-expressions-to-perform-message-assignments.md)  
  
 [如何使用運算式動態地轉換訊息](../core/how-to-use-expressions-to-dynamic-transform-messages.md)  
  
 [如何使用運算式執行管線](../core/how-to-use-expressions-to-execute-pipelines.md)  
  
 [如何使用運算式來建立物件和呼叫物件方法](../core/how-to-use-expressions-to-create-objects-and-call-object-methods.md)  
  
 [如何使用運算式來將值指派給動態連接埠](../core/how-to-use-expressions-to-assign-values-to-dynamic-ports.md)  
  
## <a name="see-also"></a>另請參閱  
 [XLANG 的語言](../core/xlang-s-language.md)   
 [協調流程中使用訊息](../core/using-messages-in-orchestrations.md)   
 [在協調流程中使用變數](../core/using-variables-in-orchestrations.md)