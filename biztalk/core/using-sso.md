---
title: "使用 SSO |Microsoft 文件"
ms.custom: 
ms.date: 06/08/2017
ms.prod: biztalk-server
ms.reviewer: 
ms.suite: 
ms.tgt_pltfrm: 
ms.topic: article
helpviewer_keywords:
- managing [SSO]
- SSO, managing
ms.assetid: e7245632-9c71-4b1f-836d-a4ea1dd6e5ee
caps.latest.revision: "12"
author: MandiOhlinger
ms.author: mandia
manager: anneta
ms.openlocfilehash: 850425f140b526baf251568f09ab47db9115e8ad
ms.sourcegitcommit: cb908c540d8f1a692d01dc8f313e16cb4b4e696d
ms.translationtype: MT
ms.contentlocale: zh-TW
ms.lasthandoff: 09/20/2017
---
# <a name="using-sso"></a>使用 SSO
您可以使用 MMC 嵌入式管理單元或是命令列管理公用程式 (ssomanage)，以管理 SSO 系統。 這包括像是更新 SSO 資料庫、新增、刪除和管理應用程式以及管理使用者對應等活動。  
  
> [!NOTE]
>  在系統上可支援使用者帳戶控制 (UAC)，您可能需要以系統管理權限執行 SSO 命令列工具。  
  
 只有「單一登入」系統管理員可以執行這些工作。  
  
## <a name="in-this-section"></a>本節內容  
  
-   [如何設定 SSO 伺服器](../core/how-to-set-the-sso-server.md)  
  
-   [如何啟用 SSO](../core/how-to-enable-sso.md)  
  
-   [如何變更主要密碼伺服器](../core/how-to-change-the-master-secret-server.md)  
  
-   [如何停用 SSO](../core/how-to-disable-sso.md)  
  
-   [如何更新 SSO 資料庫](../core/how-to-update-the-sso-database.md)  
  
-   [如何顯示 SSO 資料庫資訊](../core/how-to-display-the-sso-database-information.md)  
  
-   [如何設定 SSO 票證](../core/how-to-configure-the-sso-tickets.md)  
  
-   [如何稽核 SSO](../core/how-to-audit-sso.md)  
  
-   [如何為 SSO 啟用 SSL](../core/how-to-enable-ssl-for-sso.md)  
  
-   [管理主要密碼](../core/managing-the-master-secret.md)  
  
-   [如何指定 SSO 系統管理員和分支機構系統管理員帳戶](../core/how-to-specify-sso-administrators-and-affiliate-administrators-accounts.md)  
  
-   [管理分支機構應用程式](../core/managing-affiliate-applications.md)  
  
-   [管理使用者對應](../core/managing-user-mappings.md)  
  
-   [主控件初始化的 SSO](../core/host-initiated-sso.md)