...
>  在此範例中，將使用 Microsoft Outlook Express 做為電子郵件用戶端，而 [!INCLUDE[btsWinSvr2k3](../includes/btswinsvr2k3-md.md)] 或 [!INCLUDE[btsWinSvr2k8](../includes/btswinsvr2k8-md.md)] 則做為電子郵件伺服器。 不過，任何 POP3 電子郵件用戶端與 RFC 相容 POP3 伺服器皆可以用於此實例。  
 ...
 此應用程式是一個簡單以內容為基礎的路由應用程式，只使用一個接收位置和傳送埠。 從執行的伺服器上的信箱讀取接收位置[!INCLUDE[btsWinSvr2k3](../includes/btswinsvr2k3-md.md)]或[!INCLUDE[btsWinSvr2k8](../includes/btswinsvr2k8-md.md)](「 Windows 伺服器 」\)。 傳送埠會從接收位置拾取訊息，然後將訊息傳送到 [!INCLUDE[btsBizTalkServerNoVersion](../includes/btsbiztalkservernoversion-md.md)] 之本機檔案系統上的資料夾。  
...
6.  在**密碼**和**確認密碼** 方塊中，輸入密碼，然後再按一下**確定**。  
  
7.  請記下**帳戶名稱**和**郵件伺服器**登入使用純文字驗證中所顯示的資訊**POP3 服務**對話方塊，然後按一下**確定**。 您以 POP3 傳輸類型設定的 [!INCLUDE[btsBizTalkServerNoVersion](../includes/btsbiztalkservernoversion-md.md)] 接收位置將會使用此資訊。  
...
10. 在**郵件伺服器**方塊中，輸入您在其中建立信箱之 Windows Server 型的伺服器的名稱。  
  
11. 在**驗證配置**方塊中，選取**基本**。  
  
12. 在**密碼**，按一下下拉箭號，然後輸入信箱的密碼。  
  
13. 在**使用者名**方塊中，輸入信箱的完整格式的使用者名稱，例如 *username@host.domain.toplevel_domain。*  
  
14. 在**輪詢間隔**方塊中，輸入**1**，按一下**確定**，然後按一下  **確定**一次。  
...
2.  以滑鼠右鍵按一下**SendToFile**傳送埠，然後**啟動**。  
...
1.  按一下**啟動**，指向 **程式**，然後按一下  **Outlook Express**。  
...
10. 在**屬性**對話方塊中，按一下**進階**] 索引標籤中，按一下以選取選項以**保留訊息的複本伺服器上**，然後按一下 [**確定**.  
...
1.  使用 Windows 檔案總管開啟資料夾指定為**目的地資料夾**傳送埠。  
