...
2.  工具功能表上，按一下**BAM**  功能表項目，然後按一下**BAM 活動**。  
...
3. 從**項目類型**下拉式選單，選取此項目的類型。 可能的值包括：  
  
   |項目類型|描述|  
   |---------------|-----------------|  
   |商務里程碑|日期/時間值。 例如，訂單的核准日期。|  
   |商務資料 – TEXT|包含任何英數字元的字串。 例如，送貨地址： 城市、 縣/市和郵遞區號的程式碼。|  
   |商務資料 - INTEGER|整數值。 例如，總訂購數。|  
   |商務資料 – Decimal|十進位值。 例如 PO 的總金額。|  
  
    如果您選取的項目輸入 「 商務資料-Text 中，您必須輸入中的字串的字元數上限**最大長度** 方塊中。  
...
5. 完成 [商務活動監控活動精靈] 之後，[商務活動監控檢視精靈] 便會自動啟動。  
   ...
   如需使用此精靈的詳細資訊，請參閱[定義 BAM 檢視](../core/defining-a-bam-view.md)。  
