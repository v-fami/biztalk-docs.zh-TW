---
title: 單一登入： 事件 10663 |Microsoft Docs
ms.custom: ''
ms.date: 06/08/2017
ms.prod: biztalk-server
ms.reviewer: ''
ms.suite: ''
ms.tgt_pltfrm: ''
ms.topic: article
ms.assetid: 85223ded-1226-4dce-affc-46142157d18d
caps.latest.revision: 10
author: MandiOhlinger
ms.author: mandia
manager: anneta
ms.openlocfilehash: 6ed3a9a722d39a621bd2873dcc441e8b56b94088
ms.sourcegitcommit: 266308ec5c6a9d8d80ff298ee6051b4843c5d626
ms.translationtype: MT
ms.contentlocale: zh-TW
ms.lasthandoff: 06/27/2018
ms.locfileid: "36993967"
---
# <a name="single-sign-on-event-10663"></a>單一登入： 事件 10663
## <a name="details"></a>詳細資料  

|                 |                                                                                                                                                                                                                      |
|-----------------|----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
|  產品名稱   |                                                                                              企業單一登入                                                                                               |
| 產品版本 |                                                                              [!INCLUDE[btsSSOVersion](../includes/btsssoversion-md.md)]                                                                              |
|    事件識別碼     |                                                                                                        10663                                                                                                         |
|  事件來源   |                                                                                                        ENTSSO                                                                                                        |
|    元件    |                                                                                                         N\A                                                                                                          |
|  符號名稱  |                                                                                      SSO_INFO_EXTERNAL_PASSWORD_CHANGE_RECEIVED                                                                                      |
|  訊息文字   | 從配接器收到外部密碼變更。%r<br /><br /> 追蹤識別碼: %1 %r<br /><br /> 配接器: %2 %r<br /><br /> 外部帳戶: %3 %r<br /><br /> 用戶端使用者: %4 %r<br /><br /> 用戶端電腦： %5 |

## <a name="explanation"></a>說明  
 這個資訊事件表示從配接器收到外部密碼變更。  

## <a name="user-action"></a>使用者動作  

-   使用者不必採取任何動作。  

## <a name="more-info"></a>其他資訊

- [如何管理密碼同步處理](../core/how-to-administer-password-synchronization.md)  

- **密碼同步配接器屬性： 選項** [!INCLUDE[ui-guidance-developers-reference](../includes/ui-guidance-developers-reference.md)]
