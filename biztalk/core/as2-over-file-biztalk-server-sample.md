---
title: AS2 over File （BizTalk Server 範例） |Microsoft Docs
ms.custom: ''
ms.date: 06/08/2017
...
6. 在 **加入新項目**對話方塊方塊中，選取**管線檔案**在左側窗格中，選取**接收管線**在右側窗格中的名稱，管線 AS2OverFile_Receive.btp，，然後按一下**新增**。  
...
12. 至 AS2 解碼器元件拖放**解碼**自訂管線的 AS2 Over File 元件之後的階段。  
...
15. 在 [!INCLUDE[btsBizTalkServerNoVersion](../includes/btsbiztalkservernoversion-md.md)]管理主控台中，將自訂管線新增至 [管線] 節點中，按一下 [管線] 節點，然後按一下**重新整理**。  
...
3. 在 [接收位置] 節點中，以滑鼠右鍵按一下 [as2overfile_receive] 接收位置，然後按一下**啟用**。  
...
5. 在 傳送埠 節點中，as2overfile_send 傳送埠中，以滑鼠右鍵按一下，然後按一下**啟動**。  

6. 在 [合作對象] 節點中，建立名稱為 "Partner" 的合作對象。 以 [別名] 清單中，新增與別名**名稱**的**EDIINT-AS2 From 值**，則**限定詞**的**AS2-從**，和**值**的**夥伴**。  
...
3. 在文字編輯器中，開啟輸入的訊息 SampleMessage.txt，並開啟輸出訊息\<GUID\>在文字編輯器中的.xml。 確認 SampleMessage.txt 輸入訊息有 HTTP (和 AS2) 標頭，而輸出訊息則沒有 HTTP 標頭。  
