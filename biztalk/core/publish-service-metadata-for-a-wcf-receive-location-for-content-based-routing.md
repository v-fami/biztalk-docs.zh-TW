...
6.  在**WCF 服務**頁面上，定義發行服務中繼資料的 WCF 服務合約。 使用中的樹狀結構**Web 服務描述**加入、 移除、 重新命名，以及編輯發佈的 WCF 服務的 Web 服務描述節點 對話方塊。 **資訊**對話方塊提供所選節點的相關資訊，並顯示目前的節點或任何子節點中的任何錯誤：  
  
    -   樹狀結構的根節點 (Web 服務描述) 會描述要發佈的 WCF 服務合約。 虛擬目錄名稱會使用根節點做為預設名稱。 您可以修改的選取將發佈的 WCF 服務 Web 服務描述名稱**重新命名 web 服務描述**。  
