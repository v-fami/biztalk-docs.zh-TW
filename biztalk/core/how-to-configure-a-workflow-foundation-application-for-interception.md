...
4. 使用 bm.exe 部署觀察模型：  
    ...
    請確定您取代\< *definitionfile.xml* \>具有您想要部署的觀察檔案名稱。 如需其他選項請參閱[攔截器管理命令](../core/interceptor-management-commands.md)。  
...
2.  將下列組態資訊插入 App.config 檔案以新增 BAM 追蹤服務。 請定位該資訊，讓它成為 `configuration` 元素的子系。  
    ...
    > [!NOTE]
    >  Section 項目應該對應至使用 WorkflowRuntime 類別時，您的應用程式程式碼所使用的名稱。  
