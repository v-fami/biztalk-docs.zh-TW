...
- BizTalk Adapter for TIBCO EMS 會要求您新增 TIBCO EMS C# API，TIBCO。EMS.dll，全域組件快取 (GAC)。 如需有關如何安裝組件的詳細資訊，請參閱 < [TIBCO Enterprise Message Service 需求和限制](../core/tibco-enterprise-message-service-requirements-and-limitations.md)。  
...
## <a name="how-this-sample-is-designed-and-why"></a>此範例的設計方式和原因  
 在 Visual Studio 中設計的這個範例說明如何使用 BizTalk Adapter for TIBCO Enterprise Message Service 與 BizTalk 協調流程的基本功能。  

## <a name="where-to-find-this-sample"></a>可在何處找到此範例  
 此範例的預設位置是  

 C:\Program Files\Microsoft BizTalk Adapters for Enterprise Applications\TIBCO(r) Enterprise 訊息 Service(TM) \Sdk\OneWaySend  
...
1. 啟動 [!INCLUDE[btsBizTalkServerNoVersion](../includes/btsbiztalkservernoversion-md.md)] 管理主控台。 按一下 **開始**，**所有程式**， **Microsoft** [!INCLUDE[btsBizTalkServer2006r3ui](../includes/btsbiztalkserver2006r3ui-md.md)]， **BizTalk Server 管理**。  
...
3. 輸入的值**名稱**欄位，例如**TIBCOEMSOneWaySP**。  

4. 從使用中的配接器清單中選取 TIBCO EMS 配接器**型別**下拉式清單方塊，然後按一下**設定** 按鈕以顯示配接器**傳輸屬性** 對話方塊。  

   > [!NOTE]
   >  這個值是在中建立 TIBCO Enterprise Message System 配接器時所指定的名稱[!INCLUDE[btsBizTalkServerNoVersion](../includes/btsbiztalkservernoversion-md.md)]管理主控台。  
...
6. 輸入的值**使用者認證**:  
   ...
   |[使用者名稱]|TIBCO EMS 伺服器的使用者名稱。|  
    ...
    如需有關屬性的詳細資訊，請參閱[建立 TIBCO Enterprise Message Service 傳送處理常式](../core/creating-tibco-enterprise-message-service-send-handlers.md)。  

7. 按一下 [確定] 。  

8. 選取**XML 傳輸**從清單中可用的管線的管線**傳送管線**下拉式清單中，按一下 **確定**。  
...
3. 輸入的值**名稱**欄位，例如**TIBCOEMSOneWayFileRP**，然後按一下**確定**。  
...
3.  輸入的值**名稱**欄位，例如**TIBCOEMSOneWayFileRL**。  
...
2.  在 [屬性] 視窗中，按一下以選取**輸出執行個體的檔名**選項底下**一般**類別目錄。  
