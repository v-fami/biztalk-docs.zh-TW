...
---
# <a name="restrictions-when-configuring-the-file-adapter"></a>設定 File 配接器的限制
限制和規則使用 file 配接器時。

## <a name="file-mask-and-file-name-gotchas"></a>檔案遮罩和檔案名稱錯誤
...
- Windows 磁碟區使用 8.3 (8.3) 命名慣例，根據預設，會使用簡短和長檔名。 非系統磁碟區停用 8.3 命名慣例，並因此只能使用長檔名。 

  啟用 8.3 時，檔案和其副檔名取得轉換為簡短名稱。 例如，`testabcdefgh.docx`取得轉換成`testab~1.doc`。 請注意，已縮短檔案名稱，並已從縮短檔案延伸模組`.docx`至`doc`。

  此行為會影響 File 配接器接收檔案的方式。 如果檔案遮罩之所以設為`*.xml`，然後檔案 比對兩者`*.xml`和`*.xmln`延伸模組會挑選。 

  若要查看您的磁碟上是否已啟用 8.3 命名慣例，開啟命令提示字元做為系統管理員，然後輸入`fsutil 8dot3name query c:`，或`fsutil 8dot3name query d:`，依此類推。 範例輸出看起來如下所示：
  ...
  File 配接器會使用[FindFirstFile 函式](https://msdn.microsoft.com/library/windows/desktop/aa364418(v=vs.85).aspx)。 此函式會包含具有短期和長期的檔案名稱的搜尋結果。 若要查看資料夾中的短期和長期的檔案名稱，開啟命令提示字元，移至您的資料夾，並輸入`dir /x`。 在命令提示字元中，您也可以輸入`dir c:\foldername /x`。
    
  如果您變更 8dot3name （英文） 上的設定磁碟區時，新的檔案就會使用新的設定。 任何現有的檔案會保留其名稱，直到會跟著移動。 
    
  若要只挑選您預期的檔案，並在高負載期間取得較佳的效能 （較少額外負荷），則可能最好設定 File 配接器使用的磁碟區會在停用 8dot3name （英文）。 
...
|%SourceFileName%|FILE 配接器從中讀取訊息的檔案名稱。 檔案名稱包含副檔名但排除檔案路徑，例如 Sample.xml。 File 配接器時取代這個屬性，從儲存在的絕對檔案路徑擷取檔案名稱**檔案。ReceivedFileName**內容屬性。 如果內容屬性沒有值 — 比方說，如果在 File 配接器以外的配接器收到的訊息-巨集不會替換，而將保留在檔案名稱 (例如 C:\Drop\\SourceFileName %)。 **注意：** 正確實作此巨集需要的輸出訊息和接收的訊息相同的訊息。|  
...
## <a name="receive-folder-and-destination-location-properties-gotchas"></a>接收資料夾和目的地位置屬性錯誤
...
- 您可以藉由使用通用命名慣例 (UNC) 格式指定的檔案路徑 (例如\\ \\ < *server* \> \\ < *共用*\>)。  
  
- 如果檔案路徑為 UNC 格式，伺服器名稱必須包含下列字元: ' ~ ！ @ # $ ^ & * ( ) = + [ ] { } \ &#124; ; : ' " , \< \> / ? ;  
  
- 您無法使用父代 (\\...\\) 和目前 (\\。\\) 資料夾符號的檔案路徑的任何部分。  
...
- 沒有設定為使用含有符號連結的 Microsoft Windows NT 分散式檔案系統資料夾的接收資料夾屬性。 如果您使用的 Windows NT 分散式檔案系統，您可以僅使用資料夾含有直接網路路徑，在 File 配接器接收位置。  
  
- 當文件傳送到 UNC 路徑，而且您有一部以上的伺服器在 FILE 配接器的接收位置上接收文件時，則只有一部伺服器會收取和處理傳送至該 UNC 路徑的大部分文件。 如需檔案重新命名的詳細資訊，請參閱 < File 接收配接器區段[File 配接器](../core/file-adapter.md)。  
