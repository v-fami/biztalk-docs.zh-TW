...
1. 建立新 Visual C# 主控台應用程式中[!INCLUDE[btsVStudioNoVersion](../includes/btsvstudionoversion-md.md)]同名**MQSeriesQueues**。  
...
6. 輸入已編譯主控台應用程式的名稱加上適當的參數，然後按 ENTER。 例如，若要刪除佇列**qm_test**佇列管理員**testq**您會在命令提示字元中輸入下列文字並按 ENTER:  
  
   ```  
   MQSeriesQueues d QM_Test testq  
   ```  
  
7. 若要建立佇列**qm_test**佇列管理員**testq**您會在命令提示字元中輸入下列文字並按 ENTER:  
