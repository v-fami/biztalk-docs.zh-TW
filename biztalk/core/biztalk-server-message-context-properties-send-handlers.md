 ...
 **問題：** 怎麼 BizTalk Adapter for TIBCO Rendezvous 讀取及操作的協調流程內的回覆主旨，以便讓使用的傳送主體做為回應？ 配接器如何找到來自 Rendezvous 之內送訊息的訊息內容？  
  
 **答：** 回覆主旨會填入內容中的內送的訊息，以及協調流程可以讀取它。 如果協調流程最終會產生回覆，即可使用該值來設定回覆訊息的傳送主體。  
...
2. 在協調流程中的接收圖形 (內送 Rendezvous 訊息傳入之處) 與傳送圖形 (傳送回覆之處) 之間的某處，新增訊息建構/指派圖形 (或運算式圖形)，以對您所建構的回覆執行如下列範例的作業：  
   ...
   TIBCO Rendezvous 未提供中繼資料儲存機制，而 Microsoft BizTalk Adapter for TIBCO Rendezvous 管理組件未提供瀏覽功能或結構描述產生功能。 因此，您必須提供描述結構給 BizTalk Server。 如需詳細資訊，請參閱 <<c0> [ 安裝、 結構描述，與限制](../core/installing-biztalk-adapter-for-tibco-rendezvous.md)。
