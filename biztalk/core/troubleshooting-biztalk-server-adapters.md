---
title: "疑難排解 BizTalk Server 配接器 |Microsoft 文件"
ms.custom: 
ms.date: 06/08/2017
ms.prod: biztalk-server
ms.reviewer: 
ms.suite: 
ms.tgt_pltfrm: 
ms.topic: article
ms.assetid: 428b127f-df1e-4c41-9cc8-486519f15fa1
caps.latest.revision: "11"
author: MandiOhlinger
ms.author: mandia
manager: anneta
ms.openlocfilehash: bdaf265f3a0c58f9505f9b4806dac0a5cee05a75
ms.sourcegitcommit: cb908c540d8f1a692d01dc8f313e16cb4b4e696d
ms.translationtype: MT
ms.contentlocale: zh-TW
ms.lasthandoff: 09/20/2017
---
# <a name="troubleshooting-biztalk-server-adapters"></a>BizTalk Server 配接器疑難排解
本主題所敘述的一般疑難排解步驟可用來識別及解決 BizTalk 整合配接器可能發生的問題。  
  
## <a name="in-this-section"></a>本節內容  
  
-   [檔案配接器疑難排解](../core/troubleshooting-the-file-adapter.md)  
  
-   [FTP 配接器疑難排解](../core/troubleshooting-the-ftp-adapter.md)  
  
-   [HTTP 配接器疑難排解](../core/troubleshooting-the-http-adapter.md)  
  
-   [MQSeries 配接器疑難排解](../core/troubleshooting-the-mqseries-adapter.md)  
  
-   [MSMQ 配接器疑難排解](../core/troubleshooting-the-msmq-adapter.md)  
  
-   [POP3 配接器疑難排解](../core/troubleshooting-the-pop3-adapter.md)  
  
-   [SMTP 配接器疑難排解](../core/troubleshooting-the-smtp-adapter.md)  
  
-   [SOAP 配接器疑難排解](../core/troubleshooting-the-soap-adapter.md)  
  
-   [Windows SharePoint Services 配接器疑難排解](../core/troubleshooting-the-windows-sharepoint-services-adapter.md)  
  
-   [WCF 配接器疑難排解](../core/troubleshooting-the-wcf-adapters.md)  
  
-   [使用 BizTalk 配接器追蹤](../core/using-biztalk-adapter-tracing.md)  
  
-   [BizTalk 配接器搭配使用 IPv6 定址](../core/using-ipv6-addressing-with-biztalk-adapters.md)