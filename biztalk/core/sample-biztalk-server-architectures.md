---
title: 範例 BizTalk Server 架構 |Microsoft 文件
ms.custom: ''
ms.date: 06/08/2017
ms.prod: biztalk-server
ms.reviewer: ''
ms.suite: ''
ms.tgt_pltfrm: ''
ms.topic: article
helpviewer_keywords:
- architecture, examples
ms.assetid: 3e371771-55cf-4826-89cd-76b5537dd042
caps.latest.revision: 9
author: MandiOhlinger
ms.author: mandia
manager: anneta
ms.openlocfilehash: 17f512015d5f2c1fd1a3bedc002140628fde4969
ms.sourcegitcommit: cb908c540d8f1a692d01dc8f313e16cb4b4e696d
ms.translationtype: MT
ms.contentlocale: zh-TW
ms.lasthandoff: 09/20/2017
ms.locfileid: "22269014"
---
# <a name="sample-biztalk-server-architectures"></a>BizTalk Server 架構範例
若要儘可能提供 Microsoft® BizTalk® Server 環境最大的安全性，建議您使用根據其中所含資料與服務的重要性以及攻擊者危害伺服器時的暗示，來使用可分散 BizTalk Server 的架構。  
  
 本節提供的資訊可協助您衍生自己的 BizTalk Server 安全部署，以符合企業與環境的特殊需求與挑戰。  
  
## <a name="in-this-section"></a>本節內容  
  
-   [大型分散式的架構](../core/large-distributed-architecture.md)  
  
-   [具有資訊工作者服務的大型分散式的架構](../core/large-distributed-architecture-with-information-worker-services.md)  
  
-   [伺服器命名慣例](../core/server-naming-conventions.md)  
  
-   [縮小架構](../core/scaled-down-architecture.md)  
  
-   [縮小具有資訊工作者服務的架構](../core/scaled-down-architecture-with-information-worker-services.md)  
  
-   [Windows 帳戶用於安全分散式 BizTalk Server 部署](../core/windows-accounts-for-a-secure-distributed-biztalk-server-deployment.md)  
  
-   [必要的連接埠，以便讓 BizTalk Server](../core/required-ports-for-biztalk-server.md)