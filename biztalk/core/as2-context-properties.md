...
## <a name="context-properties-in-the-ediintpropertiesxsd-schema"></a>EdiIntProperties.xsd 結構描述中的內容屬性  
 EDI/INT 全域屬性結構描述中的訊息內容屬性都是公開屬性，因此您可以在例如訊息路由等作業中使用這些內容屬性。 這些內容屬性都是定義在 `Microsoft.BizTalk.Edi.BaseArtifacts` 組件的 EdiIntProperties.xsd 結構描述中。 屬性的命名空間是 `http://schemas.microsoft.com/BizTalk/2006/as2-properties`。 如果升級，這些訊息內容屬性都像`EdiIntAS.<Property Name>`中**篩選**頁面**傳送埠屬性** 對話方塊。  
