...
4.  在協調流程中，您已建立了邏輯連接埠 (**ReceiveSO**) 從服務匯流排佇列接收訊息。 在此步驟中，您會建立實體接收埠以對應至邏輯連接埠。  
    ...
    6.  從 [Sb-messaging 傳輸屬性] 對話方塊上**一般**索引標籤上，針對**佇列或訂閱 URL**，輸入**sb://mynamespace.servicebus.appfabriclabs.com/queueordersedi**. 在這裡， *mynamespace*是服務匯流排命名空間和*queueordersedi*是您在建立服務匯流排佇列[(Azure) 的步驟 3： 建立服務匯流排佇列](../core/step-3-for-azure-create-a-service-bus-queue.md)。  
...
5.  在協調流程中，您已建立了邏輯連接埠 (**SendToSQL**) 傳送訊息至**SalesOrder**資料庫資料表。 在此步驟中，您會建立實體傳送埠以對應至邏輯連接埠。  
    ...
    5.  WCF-SQL 傳輸屬性 上**認證**索引標籤上，選取**不會使用單一登入**，並指定 （區分大小寫） 的認證，以連接到 SQL Server 資料庫中指定連接字串。 如果您想使用 [Windows 驗證] 進行連線，請將認證空白。  
