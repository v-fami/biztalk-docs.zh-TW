...
循序群組可讓多個單一訊息聯結在一起，以達成所需的結果。 循序群組是具有預先定義順序的相關訊息組。 雖然訊息不需要完全相同，但 BizTalk Server 必須以循序方式接收這些訊息。  
 ...
 前述的案例是需要對內送訊息進行循序群組處理的範例。 根據商務需求指出，在特定日下午 2:00 之前接到的所有單一訂單，都應該合併批次處理。 為此，第一個收到的訊息必須啟動新的協調流程執行個體，並初始化相互關聯集合。 此時，如果收到該訊息類型的其他訂單，都會路由至已經執行的協調流程執行個體。 若要達成此目的，您定位**接聽**圖形 (包含**接收**圖形和**延遲**圖形) 內**迴圈**圖形。 在到達延遲之後，迴圈就會結束。 這樣可以在相同的商務程序中接收未知數目的訂單。  
...
- 如果循序群組是由啟動接收所初始化，則其後的接收不能位在巢狀的協調流程內。  
  ...
  如需循序群組實作的範例，請參閱 <<c0> [ 彙總工具 （BizTalk Server 範例）](../core/aggregator-biztalk-server-sample.md)。  
