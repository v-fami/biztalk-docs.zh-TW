...
-   如果其他成品對此協調流程或對包含在也將移除之組件中的成品有相依性，則當您移除協調流程時，這些成品將再也無法正確運作。 相依性的相關背景資訊，請參閱[相依性和應用程式部署](../core/dependencies-and-application-deployment.md)。  
...
2.  在主控台樹狀目錄中，依序展開**BizTalk Server 管理**、 依序展開 BizTalk 群組、 展開**應用程式**，然後展開包含您想要移除的協調流程的應用程式。  
  
3.  按一下**協調流程**，協調流程中，以滑鼠右鍵按一下，然後按一下**取消登錄**。  
  
4.  選取的協調流程，指向**檢視**，然後按一下 **執行個體資訊**。  
  
5.  查詢結果] 窗格中，以滑鼠右鍵按一下協調流程執行個體，然後按一下 [ **Terminate**。  
  
    > [!NOTE]
    >  您可以取消登錄、 終止執行中的執行個體，並停止所有協調流程的應用程式中的一次使用完全停止 選項，則應用程式中所述[如何啟動和停止 BizTalk 應用程式](../core/how-to-start-and-stop-a-biztalk-application.md)。  
  
6.  按一下**協調流程**，協調流程中，以滑鼠右鍵按一下，然後按一下**移除**。  
