...
使用 [!INCLUDE[btsSettingsDashboard](../includes/btssettingsdashboard-md.md)]，您就可以跨 BizTalk 群組修改指定主控件執行個體的組態資訊。 此主題中提供的逐步程序，可讓您修改 [!INCLUDE[btsBizTalkServerNoVersion](../includes/btsbiztalkservernoversion-md.md)] 中的主控件執行個體層級效能設定。  
 ...
 如需匯入[!INCLUDE[btsBizTalkServerNoVersion](../includes/btsbiztalkservernoversion-md.md)]設定，請參閱[匯入或匯出 BizTalk 設定使用設定儀表板](how-to-import-biztalk-settings-using-settings-dashboard.md)並[匯入或匯出 BizTalk 設定使用 BTSTask](how-to-import-biztalk-settings-using-btstask.md)。 
...
2. 在  **BizTalk 設定儀表板**對話方塊的 **主控件執行個體**索引標籤上，執行下列任一項：  
