---
title: EDIFACT CONTRL 訊息作為功能通知 |Microsoft Docs
ms.custom: ''
ms.date: 06/08/2017
...
ms.locfileid: "36985015"
---
# <a name="edifact-contrl-message-as-functional-acknowledgment"></a>EDIFACT CONTRL 訊息做為功能通知
如果您選取要產生功能通知，在商務設定檔設定或交易夥伴協議 （或後援協議，如果未定義協議是兩個商務設定檔之間），或如果設定為"1"，CONTRL 訊息中的 UNB9 欄位做為功能通知 (ACK)，將會產生訊息。 這個通知會報告對交換進行語法檢查的結果。  
 ...
 **SG 迴圈**  
 ...
 通知訊息的 XML 格式將包含封裝每個 UCF 區段 SG3Loop 項目、 SG4Loop 項目會封裝每個 UCM 元素 SG5Loop 項目會封裝每對 UCS 和 UCD 元素。 訊息的原生 EDI 格式中並不包含 SG 迴圈標記。  
...
> [!NOTE]
>  依照預設和業界用法，已接受的交易集不應有 SG1/SG4 迴圈。 不過，若要支援標準的合規性，您可以強制產生 SG1/SG4 藉**針對接受的交易集的產生 SG1/SG4 迴圈**核取方塊**通知**頁面協議兩個商務設定檔之間的協議的 內容 對話方塊 (或**通知**EDI 設定索引標籤，為商務設定檔 頁面)。 如果選取此核取方塊，則接收管線會產生 SG1/SG4 迴圈是否接受或拒絕交易集。 如果不勾選，則只會針對錯誤交易集產生這些迴圈 (UCM5 != 7)。  
...
|UCF7|資料元素識別碼|識別具有 UCF5 資料元素中指出之錯誤狀況的資料元素。 UCF7 的子元件包括：<br /><br /> -錯誤資料元素區段中的位置 （必要項）<br /><br /> -錯誤元件資料元素區段中的位置 （條件式選用性）<br /><br /> -出現的區段 （必要項） 中的錯誤資料元素|  
...
|UCM7|資料元素識別碼|識別具有 UCM5 資料元素中指出之錯誤狀況的資料元素。 UCM7 的子元件包括：<br /><br /> -錯誤資料元素區段中的位置 （必要項）<br /><br /> -錯誤元件資料元素區段中的位置 （條件式選用性）<br /><br /> -出現的區段 （必要項） 中的錯誤資料元素|  
|UCS1|訊息內文中的區段位置|錯誤的區段中，位置計數開始 UNH 為 1。 若要報告區段遺漏，這是處理遺失的區段位置應該要的位置之前的最後一個區段的數值計數位置。 遺失區段群組則表示為遺失群組中的第一個區段。<br /><br /> 這是必要的資料元素。|  
...
|UCD1|語法錯誤代碼|識別群組中的錯誤狀況 (如果有的話)。 如需詳細資訊，請參閱 < [EDIFACT CONTRL 通知錯誤碼](../core/edifact-contrl-acknowledgment-error-codes.md)。<br /><br /> 這個資料元素具有條件式選用性。<br /><br /> **注意：** 則 UCD1 資料元素的 XSD 驗證失敗時，會報告程式碼值為 12，無效的值。|  
|UCD2|資料元素識別碼|識別具有 UCD1 資料元素中指出之錯誤狀況的資料元素。 UCD2 的子元件包括：<br /><br /> -錯誤資料元素區段中的位置 （必要項）<br /><br /> -錯誤元件資料元素區段中的位置 （條件式選用性）<br /><br /> -出現的區段 （必要項） 中的錯誤資料元素|  
|UNT1|區段計數|-|  
|UNT2|訊息參考編號|-|