---
title: "管理分支機構應用程式 |Microsoft 文件"
ms.custom: 
ms.date: 06/08/2017
ms.prod: biztalk-server
ms.reviewer: 
ms.suite: 
ms.tgt_pltfrm: 
ms.topic: article
helpviewer_keywords:
- managing [SSO applications]
- applications [SSO], managing
- SSO, applications
ms.assetid: 9bc6b120-6849-476a-b3af-c18bf2fa0197
caps.latest.revision: "8"
author: MandiOhlinger
ms.author: mandia
manager: anneta
ms.openlocfilehash: dc5a355faed995fb6543b1e8f5810eec14584600
ms.sourcegitcommit: cb908c540d8f1a692d01dc8f313e16cb4b4e696d
ms.translationtype: MT
ms.contentlocale: zh-TW
ms.lasthandoff: 09/20/2017
---
# <a name="managing-affiliate-applications"></a>管理分支機構應用程式
本節提供如何建立與設定分支機構應用程式的相關資訊。  
  
## <a name="in-this-section"></a>本節內容  
  
-   [如何建立分支機構應用程式](../core/how-to-create-an-affiliate-application.md)  
  
-   [如何刪除分支機構應用程式](../core/how-to-delete-an-affiliate-application.md)  
  
-   [如何更新分支機構應用程式的內容](../core/how-to-update-the-properties-of-an-affiliate-application.md)  
  
-   [如何啟用分支機構應用程式](../core/how-to-enable-an-affiliate-application.md)  
  
-   [如何停用分支機構應用程式](../core/how-to-disable-an-affiliate-application.md)  
  
-   [如何列出分支機構應用程式](../core/how-to-list-affiliate-applications.md)  
  
-   [如何列出分支機構應用程式的屬性](../core/how-to-list-the-properties-of-an-affiliate-application.md)  
  
-   [如何清除應用程式快取](../core/how-to-clear-the-application-cache.md)  
  
-   [如何設定 SSO 伺服器使用用戶端公用程式](../core/how-to-set-the-sso-server-using-the-client-utility.md)  
  
-   [如何顯示 SSO 伺服器使用用戶端公用程式](../core/how-to-display-the-sso-server-using-the-client-utility.md)  
  
-   [如何使用用戶端公用程式之分支機構應用程式設定認證](../core/how-to-set-credentials-for-the-affiliate-application-using-the-client-utility.md)