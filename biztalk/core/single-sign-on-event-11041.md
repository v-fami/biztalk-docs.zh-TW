---
title: "單一登入： 事件 11041 |Microsoft 文件"
ms.custom: 
ms.date: 06/08/2017
ms.prod: biztalk-server
ms.reviewer: 
ms.suite: 
ms.tgt_pltfrm: 
ms.topic: article
ms.assetid: 186aefc4-fbb0-4f79-baa0-a9f4a7abd71a
caps.latest.revision: "7"
author: MandiOhlinger
ms.author: mandia
manager: anneta
ms.openlocfilehash: e8a1c0808a6b9d72bb9f8b4447848efc4835c131
ms.sourcegitcommit: cb908c540d8f1a692d01dc8f313e16cb4b4e696d
ms.translationtype: MT
ms.contentlocale: zh-TW
ms.lasthandoff: 09/20/2017
---
# <a name="single-sign-on-event-11041"></a>單一登入： 事件 11041
## <a name="details"></a>詳細資料  
  
|||  
|-|-|  
|產品名稱|企業單一登入|  
|產品版本|[!INCLUDE[btsSSOVersion](../includes/btsssoversion-md.md)]|  
|事件識別碼|11041|  
|事件來源|ENTSSO|  
|元件|不適用|  
|符號名稱|SSO_WARN_LOCAL_ACCOUNT|  
|訊息文字|SSO 服務正在使用本機帳戶執行。 這不建議使用，並且將限制 SSO 的功能。 請參閱您的文件 details.%r<br /><br /> SSO 服務帳戶： %1|  
  
## <a name="explanation"></a>說明  
 SSO 服務正在使用本機帳戶執行。 這不建議使用，並且將限制 SSO 的功能。  
  
## <a name="user-action"></a>使用者動作  
 如需詳細資訊，請參閱[SSO 安全性建議](../core/sso-security-recommendations.md)。