---
title: 單一登入： 事件 10772 |Microsoft Docs
ms.custom: ''
ms.date: 06/08/2017
ms.prod: biztalk-server
ms.reviewer: ''
ms.suite: ''
ms.tgt_pltfrm: ''
ms.topic: article
ms.assetid: 0d7fab9f-0a2a-4d75-b482-0a7e178d69e5
caps.latest.revision: 6
author: MandiOhlinger
ms.author: mandia
manager: anneta
ms.openlocfilehash: 78a17f0e9b4dd53680103bcb4cd0db3cc4613a0f
ms.sourcegitcommit: 266308ec5c6a9d8d80ff298ee6051b4843c5d626
ms.translationtype: MT
ms.contentlocale: zh-TW
ms.lasthandoff: 06/27/2018
ms.locfileid: "36976007"
---
# <a name="single-sign-on-event-10772"></a>單一登入： 事件 10772
## <a name="details"></a>詳細資料  
  
|                 |                                                                       |
|-----------------|-----------------------------------------------------------------------|
|  產品名稱   |                       企業單一登入                       |
| 產品版本 |      [!INCLUDE[btsSSOVersion](../includes/btsssoversion-md.md)]       |
|    事件識別碼     |                                 10772                                 |
|  事件來源   |                                ENTSSO                                 |
|    元件    |                                  不適用                                  |
|  符號名稱  |                        ENTSSO_E_NOT_SSO_ADMIN                         |
|  訊息文字   | 使用者不是指定的 SSO 系統管理員帳戶的成員。 |
  
## <a name="explanation"></a>說明  
 您必須是 SSO 系統管理員，若要執行這項功能。  
  
## <a name="user-action"></a>使用者動作  
 擁有您連絡 SSO 系統管理員的系統管理員協助。