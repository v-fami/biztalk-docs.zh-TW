...
### <a name="right-justified-values"></a>靠右對齊的值  
 ...
 輸入會靠右對齊的值，您必須在稱為 jdearglist.txt 檔案中輸入參數。 會讀取 jdearglist.txt，當您產生結構描述。 此文字檔中所列的值都會自動轉換為靠右對齊、並在左邊填滿空格的值。  
  
 您必須使用項目會描述這些參數，建立 jdearglist.txt 使用文字編輯器 中，並將它儲存在下列資料夾： %BizTalk_Install_Adapter%\config\JDE\  
  
 何處**BizTalk_Install_Adapter %** 是您安裝 BizTalk Adapter for JD Edwards OneWorld 的目錄。  
