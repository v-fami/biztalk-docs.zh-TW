...
## <a name="prerequisites"></a>必要條件  
 若要執行範例時，建議您有 Microsoft Office InfoPath 2010 或更新版本安裝。 若不使用 InfoPath，也可以執行範例，但是在此情況下，則無法透過 HTTP 配接器檢視經費支出報表或提交經費支出報表。  
...
3. 提升**部門**並**correlationID**訊息中的屬性文件，以便可以在連接埠篩選器中用來控制路由。  
...
|       屬性升級       |                                                                                                   -BizTalk Server 依賴屬性欄位，來進行路由而定。 辨別欄位可用於協調流程，不能用於路由。                                                                                                   |
|      訂閱篩選器       |                                                                                                          -訂用帳戶篩選器會執行擷取符合根據屬性欄位的一或多個準則的訊息路由。                                                                                                           |
| BizTalk 協調流程排程 |                                                                  -提供機會來將資訊新增至失敗的訊息。<br />-路由傳送至專屬位置，讓人為介入的失敗訊息。<br />-處理序重新提交經費支出報表。                                                                   |
|          XMLTransmit           |                                                                                                                -執行外寄 XML 訊息的基本組件。 PassThruTransmit 管線不會提供其他支援。                                                                                                                 |

## <a name="where-to-find-this-sample"></a>可在何處找到此範例  
 此範例位在 <`Samples Path>`\Messaging\ErrorHandling\\。  
...
2. 執行**Setup.bat**，會執行下列動作：  
   ...
   - 建立三個資料夾： **ExpenseReportIn**， **ExpenseReportOut**，並**ResubmittedReportIn**的下列路徑：  

      <`Samples Path`>**\Messaging\ErrorHandling**  

   - 複製並發佈 InfoPath 表單**Expense Report.xsn**並**Expense Report – Resubmit.xsn**到資料夾**C:\Temp\InfoPathForms**。  

   - 為此範例編譯 [!INCLUDE[btsVStudioNoVersion](../includes/btsvstudionoversion-md.md)] 專案。  

   - 虛擬目錄中，建立**ExpenseReports**。  

   - 建立新的 BizTalk 應用程式，稱為**Error Handling Sample**並部署到其中的範例組件。  
...
4. 若是使用 Internet Information Services (IIS) 7.0，您必須執行額外的設定步驟，針對此範例所使用之 [!INCLUDE[btsBizTalkServerNoVersion](../includes/btsbiztalkservernoversion-md.md)] HTTP 接收位置的對應虛擬目錄來調整設定。 請執行下列動作來設定 IIS：  
   ...
   3. 設定虛擬目錄**ExpenseReport**使用 HTTP 接收上一個步驟中建立的應用程式集區。  

      如果您還沒有 BTSHTTPReceive.dll ISAPI 延伸模組的網頁伺服器延伸，請建立並將它的狀態設定為「允許」來啟用它。 您可以在 IIS 7.0 中使用 IIS 管理主控台 (請直接底下**系統管理工具**或透過電腦管理主控台)，如下所示：  

   4. 依序展開**Internet Information Services 管理員**樹狀目錄中。  

   5. 按一下 **網頁服務延伸**資料夾。  

   6. 在 [管理] 主控台的右窗格中，按一下**加入新的 Web 服務延伸模組**。  

   7. 在 [**新的 Web 服務延伸模組**] 對話方塊中，按一下**新增**。  

   8. 在**Add file** ] 對話方塊中，按一下**瀏覽**以選取的檔案 <`BizTalkInstallPath>`**\HttpReceive\BTSHTTPReceive.dll**，然後按一下 [ **[確定]**.  

   9. 設定虛擬目錄**ExpenseReport**若要使用的本機路徑[!INCLUDE[btsBiztalkServerPath](../includes/btsbiztalkserverpath-md.md)]HTTPReceive  

      如需詳細資訊，請參閱 <<c0> [ 針對 HTTP 接收位置設定 IIS 如何](../core/how-to-configure-iis-for-an-http-receive-location.md)。  
...
1. 開啟 InfoPath 表單**Expense Report-John Doe.xml**。 看看**部門**表單內的欄位。 這個欄位應該設定為 [Marketing]。  

2. 在 InfoPath 視窗左上角，按一下**提交**或按**重新提交經費支出報表**。 等候確認視窗表示經費支出報表已成功重新提交。  

    -或-  

    複製並貼到這個檔案**ExpenseReportIn**資料夾。  

3. 您應該會看到新的檔案，放入**ExpenseReportOut**資料夾。 這個檔案是上面步驟中所提交的相同經費支出報表。  

   確認「沒有錯誤」案例之後，請依照下列程序使用不正確的經費支出報表執行範例，以觸發錯誤處理功能。  

#### <a name="to-trigger-error-handling"></a>若要觸發錯誤處理  

1. 開啟 InfoPath 表單**Invalid Expense Report-John Doe.xml**。 看看**部門**表單內的欄位。 這個欄位是設定為某個無效值。  
...
3. 您應該會看到新的檔案，稱為**ErrorReport_\<**<em>日期</em>_<em>時間</em>**\>.xml**放入**ExpenseReportOut**資料夾。  
