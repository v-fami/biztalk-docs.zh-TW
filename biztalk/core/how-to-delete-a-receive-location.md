...
-   您不能刪除接收埠的主要接收位置。 若嘗試這麼做，將收到一個錯誤訊息。 若要刪除此接收位置，您可以刪除接收埠 (這同時會刪除它所包含的所有接收位置)，或者您也可以指定其他接收位置做為主要接收位置，然後再刪除原始接收位置。 如需接收位置建立為主要接收位置的指示，請參閱 <<c0> [ 如何編輯接收位置屬性](../core/how-to-edit-the-properties-of-a-receive-location.md)。  
...
3. 按一下 **接收位置**，以滑鼠右鍵按一下接收位置，以刪除，然後按一下**刪除**。  
