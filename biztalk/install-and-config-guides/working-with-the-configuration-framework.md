...
## <a name="configuration-framework-command-line-parameters"></a>組態架構命令列參數  
 下表描述您可以在「組態架構」中的命令列執行的參數。  
...
## <a name="sample-configuration-xml-file"></a>範例組態 XML 檔案  
 以下是安裝所有功能的範例組態 XML 檔案。 若要建立您自己的組態檔，請使用 [!INCLUDE[btsBizTalkServerNoVersion](../includes/btsbiztalkservernoversion-md.md)] 上的 [自訂組態] 選項 ，確認所有元件順利完成，然後選擇 [組態] 視窗中的 [匯出組態]。 您即可使用匯出的組態檔來設定其他 [!INCLUDE[btsBizTalkServerNoVersion](../includes/btsbiztalkservernoversion-md.md)]。  
